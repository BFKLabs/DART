...
switch (length(varargin))
    ...
    case (1)        
        ...
        if (ischar(varargin{1}))
            isRTBatch = exist(varargin{1},'dir');
        else
            isRTBatch = false;
        end
end
...
[ok,h] = initGUIObjects(handles,isRTBatch);
if (~ok)
...
impDate = [2013 12 3 0 0 0];  % important date - Dec 2, 2013. First added
                              % update/output functionality. All zip files 
                              % before this date will not contain this
                              % functionality (won't consider for update)
...
% auto-starts the real-time batch processing (if enabled)
if (isRTBatch)            
    autoStartRTBatch(handles,varargin{1});
end
...
% --- automatically starts the real-time batch processing --- %
function autoStartRTBatch(handles,exeDir)

% global variables
global isBatch mainProgDir
mainProgDir = exeDir;

% loads the rt tracking data file and deletes it
tmpFile = fullfile(exeDir,'TempData.mat');
rtData = load(tmpFile);
% delete(tmpFile)        

% memory allocation
nExpt = length(rtData.Info);
bDataT = cell(nExpt,1);

% already recording, so disable the fly recording button
set(handles.buttonFlyRecord,'enable','off')
iProg = get(handles.buttonFlyTrack,'UserData');

% runs the fly-tracking GUI
buttonFlyTrack_Callback(handles.buttonFlyTrack, [], handles)
hTrack = getappdata(handles.figDART,'hTrack');
figure(hTrack)

% retrieves the data struct from the fly tracking GUI
[iData0,iMov0] = deal(getappdata(hTrack,'iData'),rtData.iMov);
[hTrackH,isBatch] = deal(guidata(hTrack),true);  
setappdata(hTrack,'iMov',iMov0)

% loops through each of the experiments that need to be processed
% initialising the solution file directories. created is a batch-processing
% file and a temporary solution file
for i = 1:nExpt
    % initialises the batch processing data struct
    bData = struct('MovDir',[],'SolnDir',[],'SolnDirName',[],...
                   'mName',[],'sName',[]);
    
    % sets the movie/solution file directory names
    bData.MovDir = fullfile(rtData.Info(i).OutDir,rtData.Info(i).Title);
    bData.SolnDir = fullfile(iProg.DirSoln,rtData.Info(i).OutDir(length(iProg.DirMov)+2:end));
    bData.SolnDirName = rtData.Info(i).Title;               
    bData.sName = fullfile(bData.MovDir,'Summary.mat');    
    bData.mName = setBPMovieNames(bData.MovDir,rtData.Video(i),rtData.Info(i));
    
    % creates the movie solution file directory
    solnDir = fullfile(bData.SolnDir,bData.SolnDirName);
    if (~exist(bData.MovDir,'dir')); mkdir(bData.MovDir); end
    if (~exist(solnDir,'dir')); mkdir(solnDir); end
    
    % saves a batch processing file to the directory
    save(fullfile(solnDir,'BP.mat'),'bData')
    if (i == 1)
        % if the first movie directory, then set the temporary file name
        tempName = [rtData.vPara.Name,rtData.vPara.vExtn];
        tempFile = fullfile(bData.MovDir,tempName);
    else
        % otherwise, copy the file from its original location to the new
        % movie directory
        copyfile(tempFile,bData.MovDir);
    end
    
    % loads the temporary file data and outputs the solution file
    sFileNw = fullfile(solnDir,[rtData.vPara.Name,'.soln']);
    [~,iData] = loadImgData(hTrackH,tempName,bData.MovDir,0,1,iData0,iMov0);
    outputBatchSolnFile(sFileNw,solnDir,iData,iMov0,[]);
        
    % if the first directory, then update
    if (i == 1)        
        mOpenFcn = getappdata(hTrack,'menuOpenSoln_Callback');
        setappdata(hTrack,'iData',iData); isBatch = false;                        
        e = struct('fDir',solnDir,'fName',[rtData.vPara.Name,'.soln']);                 
        mOpenFcn([],e,hTrackH); isBatch = true;
    end
        
    % sets a copy of the batch processing data struct
    bDataT{i} = bData;
end

% runs the batch processing
set(hTrackH.menuBatchProcess,'enable','on')
batchProcessMovies(hTrackH,[],cell2mat(bDataT))

% wait until the tracking GUI is closed
uiwait(hTrack)
...
% global variables
global impDate isFull isAnalyDir mainProgDir 
...
if (~isa(eventdata,'char'))
    ...
    if (zIndex == 0)
        ...
        % otherwise, determine if the new zip file is valid (must be after the
        % major update made on 2nd Dec 2013)
        zFile = fullfile(zDir,zName);        
        zData = dir(zFile);

        % checks to see if the update file is valid 
        if (calcTimeDifference(datevec(convertDate(zData.date)),impDate)<0)      
            % if not, then exit the function
            eStr = sprintf('Error! Zip File Must Be Later That %s',datestr(impDate));
            waitfor(errordlg(eStr))
            return
        end
    end
...
if (zIndex == 0)
    ...
    [tmpDir,zFile] = deal(fullfile(mainProgDir,'Temp Files'),fullfile(zDir,zName));
    mkdir(tmpDir)
    
    % sets whether the full file is being output
    uChoice = questdlg('Do you want to output the A) full code or B) partial code',...
                       'Program Output Type','Full','Partial','Full');
...
if (isFull)
    sName = [sName;{'Test Files','Para Files','Analysis Functions'}'];
else
...
% -------------------------------------------------------------------------
function menuProgDiagnostic_Callback(hObject, eventdata, handles)

% runs the diagnostic tool GUI
DARTDiagnostic(handles.figDART);
...
if (~isdeployed)
    ...
    updateSubDirectories(fullfile(mainProgDir,'Test Files'),'remove')
    updateSubDirectories(fullfile(mainProgDir,'Para Files'),'remove')
    
    % removes the xlwrite java files to the path
    mainDir = mainProgDir;
    jDirXL = fullfile(mainDir,'Code','Common',...
                            'File Exchange','xlwrite','poi_library');
    if (exist(jDirXL,'dir'))
        try
            javarmpath(fullfile(jDirXL,'poi-3.8-20120326.jar'));
            javarmpath(fullfile(jDirXL,'poi-ooxml-3.8-20120326.jar'));
            javarmpath(fullfile(jDirXL,'poi-ooxml-schemas-3.8-20120326.jar'));
            javarmpath(fullfile(jDirXL,'xmlbeans-2.3.0.jar'));
            javarmpath(fullfile(jDirXL,'dom4j-1.6.1.jar'));
        end
    end
    
    % removes the heap java files to the path
    jDirHC = fullfile(mainDir,'Code','Common','File Exchange','jheapcl');
    if (exist(jDirHC,'dir'))
        try; javarmpath(jDirHC); end
    end      
    
    % removes the conditional check table java files to the path
    jDirCCT = fullfile(mainDir,'Code','Common','Utilities','CondCheckTable');
...
if (isHG1v)
    naddStr = 'HG2';
    nwDir = find((~(strcmp(fName,'.') | strcmp(fName,'..') | ...
                    strcmp(fName,'toolboxes') | cellfun(@(x)(~isempty(...
                    strfind(x,'_mcr'))),fName))) & isDir);
else
    naddStr = 'HG1';
    try
        nwDir = find((~(strcmp(fName,'.') | strcmp(fName,'..') | ...
                        strcmp(fName,'toolboxes') | cellfun(@(x)(contains(...
                        x,'_mcr')),fName))) & isDir);    
    catch
        nwDir = find((~(strcmp(fName,'.') | strcmp(fName,'..') | ...
                        strcmp(fName,'toolboxes') | cellfun(@(x)(~isempty(...
...
for i = 1:length(nwDir)
    ...
    try
        validDir = ~strcmp(fName{nwDir(i)},naddStr) && ...
                   all(cellfun(@(x)(~contains(nwDirName,x)),uDir));    
    catch
        validDir = ~strcmp(fName{nwDir(i)},naddStr) && ...
                   all(cellfun(@(x)(isempty(strfind(nwDirName,x))),uDir));        
    end
...
function [ok,h] = initGUIObjects(handles,isRTBatch)
...
if (~exist(paraDir,'dir'))
    eStr = sprintf(['The parameter files sub-directory ("Para Files") is not ',...
            'present in the location where you are attempting to run DART ',...
            'from.\n\nMove the DART executable file (DART.exe) or Matlab entry file ',...
            '(DART.m) to where this directory is located and then restart the program']);
...
if (~exist(fullfile(mainDir,'DART.m'),'file') && ~isdeployed)
    eStr = {'DART is being run from the incorrect directory.';...
            'Alter the Matlab path to the DART program directory and restart'};
...
% creates a loadbar (only if not real-time batch processing)
if (~isRTBatch)
    % adds in the progress dialog menu
    if (~isdeployed)
        addpath(fullfile(mainDir,'Code','Common','File Exchange','ProgressDialog'));
        addpath(fullfile(mainDir,'Code','Common','Progress Bars'));
    end
...
if (~isdeployed)    
    ...
    updateSubDirectories(fullfile(mainDir,'Test Files'),'add')
    ...
    jDirXL = fullfile(mainDir,'Code','Common',...
                            'File Exchange','xlwrite','poi_library');
    if (exist(jDirXL,'dir'))
        javaaddpath(fullfile(jDirXL,'poi-3.8-20120326.jar'),'-END');
        javaaddpath(fullfile(jDirXL,'poi-ooxml-3.8-20120326.jar'),'-END');
        javaaddpath(fullfile(jDirXL,'poi-ooxml-schemas-3.8-20120326.jar'),'-END');
        javaaddpath(fullfile(jDirXL,'xmlbeans-2.3.0.jar'),'-END');
        javaaddpath(fullfile(jDirXL,'dom4j-1.6.1.jar'),'-END');
    end
...
if (exist(defFile,'file'))
    ...
    eStr = [{'The DART Program Data File Has Not Been Initialised Or Is Missing'};...
            {'DART Will Now Automatically Create The Default Program Data File.'}];
...
% ------------------------------------- %
% --- AUTOMATIC PROGRAM SCAN UPDATE --- %
% ------------------------------------- %

% searches for any newer versions of DART in the search directory (only if
% the directory exists)
if (isfield(ProgDef,'DART') && ~isdeployed)
    if (exist(ProgDef.DART.DirVer,'dir'))
        % searches the directory to see if there is a newer file
        logFile = fullfile(mainProgDir,'Para Files','Update Log.mat');
        if (exist(logFile,'file'))
            % loads the log file and determines the data
            A = load(logFile);
            
            % determines all the zip files in the directory
            snFileC = findFileAll(ProgDef.DART.DirVer,'*.zip');                        
            
            % calculates the time differences
            if (~isempty(snFileC))
                B = cellfun(@(x)(dir(x)),snFileC,'un',0);
                fDate = cellfun(@(x)(datevec(datenum(...
                                     convertDate(x.date)))),B,'un',0);
                dT = cellfun(@(x)(calcTimeDifference(x,A.Time)),fDate);

                % determines if there are any newer versions
                if (any(dT > 0))
                    % hides the loadbar and prompts the user if they wish
                    % to update to the latest version
                    set(h.Control,'visible','off');                    
                    uChoice = questdlg(['A newer version of DART has been ',...
                                        'detected. Would you like to update DART?'],...
                                       'Update DART?','Yes','No','Yes');
                    if (strcmp(uChoice,'Yes'))
                        % deletes the loadbar
                        try; delete(h); end
                        
                        % updates the program to latest version
                        [~,imx] = max(dT);
                        menuUpdateProg_Callback([], snFileC{imx}, handles)                                                

                        % returns a flag value and exits the function
                        ok = false;
                        return                        
                    else
                        % makes loadbar visible again
                        set(h.Control,'visible','on');  
                        uistack(h.Control,'top');
                    end
                end
...
% --------------------------------------- %
% --- BUTTON PROPERTY INITIALISATIONS --- %
% --------------------------------------- %

% removes the button image directory (if it exists)
if (exist(fullfile(paraDir,'Images'),'dir'))
    rmvButtonImgDir(paraDir)
end
...
for i = 1:length(BData)
    ...
    if (i ~= length(BData))       
        ...
        if (isempty(uData))
            ...
            set(hButton,'Enable','on','UserData',uData)
        end
    end
end
...
if (isdeployed)
    ...
    set(handles.menuProgCode,'enable','off')
    set(handles.menuConfigSerial,'enable','on')
else
    ...
    if (ispc)
        ...
        set(handles.menuDeployExe,'enable','on')    
        set(handles.menuConfigSerial,'enable','on')
    else
...
if (~exist(pFile,'file'))
    ...
    if ~isfield(A.bgP,'pPhase')   
        [A.bgP,isChange] = deal(DetectPara.initDetectParaStruct(),true);
    end
    ...
    if ~isfield(A,'sDev')
        ...
        isChange = true;
        A.sDev = {'STMicroelectronics STLink Virtual COM Port', ...
                 'STMicroelectronics Virtual COM Port'};
    end
    
    % determines if the optogenetics piece-wise polynomials are set
    if ~isfield(A,'fOpto')
        % if not, then calculate their values from the digitised curves        
        [A.fOpto,isChange] = deal(initOptoIntensityCurves(paraDir),true);
    end   
