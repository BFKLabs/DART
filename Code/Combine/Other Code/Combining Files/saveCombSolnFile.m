...
% hasAxR = isfield(snTot,'AxR');
pW = (length(fieldnames(snTot))-1) + (oPara.outY + calcPhi);

% creates the waitbar figure
if (nargin == 5)
    [wStr,wOfs] = deal(getappdata(h,'wStr'),1+(nargin==6));
else
    wStr = {'Overall Progress','Current Experiment Progress','Output Data Field'};
    [h,wOfs] = deal(waitbarFig(wStr,'Saving Experimental Solution Files'),0);
end
    
% updates the waitbar figure and creates the x-position movie
waitbarFig(1+wOfs,'Creating X-Position Movie...',1/pW,h);
...
if (oPara.outY)
    ...
    waitbarFig(1+wOfs,'Creating Y-Position Movie...',2/pW,h);
...
if (calcPhi)
    ...
    % updates the waitbar figure
    waitbarFig(1+wOfs,'Creating Orientation Angle Movie...',3/pW,h);
...
%     if (hasAxR)
%         % updates the waitbar figure
%         waitbarFig(1+wOfs,'Creating Object Aspect Ratio Movie...',3/pW,h);
% 
%         % sets the new tar files
...
for i = 1:nField
    % updates the waitbar figure
    wStrNw = sprintf('%s - "%s"',wStr{1},fStr{i});
    if (waitbarFig(1+wOfs,wStrNw,(i+1+(oPara.outY+calcPhi))/pW,h))
        % if the user cancelled, then delete the solution file and exit
        ok = false;
        delete('Data.mat')
        return
    else
        waitbarFig(2+wOfs,'Output Field To File...',0,h);
    end
    ...
    if (i == 1)
        ...
        eStr = sprintf('save(tarFiles{1},''-struct'',''snTot'',''%s'',''-append'')',...
            fStr{i});
    end
    
    % resaves the solution file
    eval(eStr);
    waitbarFig(2+wOfs,'Field Output Complete!',1,h);
end

% sets the new indices
isOut = [true;repmat(reshape(isOut,length(isOut),1),1+(oPara.outY+calcPhi),1)];
