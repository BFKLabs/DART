...
set(hGUI.figUndockPlot,'visible','off')
set(hGUIM.figFlyAnalysis,'visible','off')
...
set(handles.buttonSaveImage,'enable',eStr{1+(~isempty(sDir) || ~isNew)})
set(handles.editSubDirName,'enable',eStr{1+isNew})
...
if (chkDirString(dirName) || isempty(dirName))
    % checks if the new string is not empty
    if (isempty(dirName))
        set(handles.buttonSaveImage,'enable','off')
    else
        set(handles.buttonSaveImage,'enable','on')
    end
            ...
            otherwise % case is the other image types            
                try
                    ...
                    eStr = {sprintf('Error outputting the image file:\n\n');...
                            sprintf(' => %s\n',imgName);...
                            'Ensure that the image file is closed and the filename is valid.'};
...
set(hGUI.figUndockPlot,'visible','on');
