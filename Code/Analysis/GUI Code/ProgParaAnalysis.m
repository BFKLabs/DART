function varargout = ProgParaAnalysis(varargin)
% Last Modified by GUIDE v2.5 16-Jan-2014 00:17:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ProgParaAnalysis_OpeningFcn, ...
    'gui_OutputFcn',  @ProgParaAnalysis_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ProgParaAnalysis is made visible.
function ProgParaAnalysis_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for ProgParaAnalysis
...
if (isempty(ProgDef))
    ProgDef = struct('DirSoln',[],'DirComb',[],'OutFig',[],'OutData',[],...
                     'DirFunc',[],'TempFile',[]);
end
...
% UIWAIT makes ProgParaAnalysis wait for user response (see UIRESUME)
set(hObject,'WindowStyle','modal');
uiwait(hObject);

% --- Outputs from this function are returned to the command line.
function varargout = ProgParaAnalysis_OutputFcn(hObject, eventdata, handles)
...
if (isempty(hDART))
    ...
    ProgDefFull.Analysis = ProgDefNw;
    setappdata(hDART.figDART,'ProgDef',ProgDefFull)        
    set(hDART.buttonFlyAnalysis,'UserData',ProgDefNw)
...
wStr = {'DirSoln','DirComb','OutFig','OutData','DirFunc','TempFile','TempData'};

% sets the call back function for all the
for i = 1:length(wStr)
    % updates the objects callback function
    hObj = eval(sprintf('handles.button%s;',wStr{i}));
    bFunc = @(hObj,e)ProgParaAnalysis('setDefDir',hObj,[],guidata(hObj));
