...
set(hObject,'visible','off'); pause(0.05);
...
if nRC < blkSz 
    ...
    for i = 1:nBlk
        ...
        % outputs the data to file
        if (i == 1)
            % case is the first data block
            xlwrite(fFile,nwBlk,sName);
        else
...
if (iData.nTab == 1)
    set(handles.menuDeleteTab,'enable','off')
    set(handles.menuMoveTab,'enable','off')
end
...
set(findall(iData.tData.hTab{jTab},'type','uitable'),'visible','on')
...
if (nargin == 1)
    ...
    hTab1 = findall(handles.panelMetricInfo,'type','uitable','UserData',mSel);    
    if (isHG1)
        set(hTab0,'visible','off')
        set(hTab1,'visible','on')
    end
...
if (isHG1)
    set(handles.tableGroupInc,'Visible',eStr{1+showTable(1)})
    set(handles.tableExptInc,'Visible',eStr{1+showTable(2)})
end
...
set(getSheetTableHandle(handles),'visible','off');
...
if (iData.nTab == 1)
    set(handles.menuDeleteTab,'enable','on')
    set(handles.menuMoveTab,'enable','on')
end
...
set(handles.menuSave,'enable',eStr{1+any(~cellfun(@isempty,iData.tData.Data))})
...
if (nargin == 2); set(hTab,'Visible','on'); end
...
if (any(hasTest))
    ...
    set(handles.radioStatTest,'enable','off')
end

% sets the enabled properties of the metric data radio button
set(handles.radioMetricData,'enable',eStr{1+any(metType(:))})    
...
if (any(any(metType(:,4:5))))
    ...
    if (isempty(lStr))
        ...
        set(handles.textUnits,'String',' ','Enable','off');
    else
...
for i = 1:length(mInd)
    ...
    % creates the table object
    hTable{i} = uitable(handles.panelMetricInfo,'position',tPos,'ColumnFormat',cForm,...
                'CellEditCallback',eFcn,'CellSelectionCallback',sFcn,...
                'ColumnName',cName,'UserData',mInd(i),'visible','on','Data',dD{i},...
                'ColumnWidth',cWid{1+(mInd(i)>1)},'RowName',[],'ColumnEditable',cEdit);                                 
    
    % makes the table invisible (if the tab is not selected)
    if (isHG1)
        set(hTable{i},'visible',eStr{1+(mInd(i)==mInd(1))});
    else
...
set(handles.checkSepByExpt,'enable',eStr{1+mltExp},'UserData',uD{1+mltExp})
set(handles.checkSepByDay,'enable',eStr{1+mltDay},'UserData',uD{1+mltDay})    
set(handles.checkSepByApp,'enable',eStr{1+mltApp},'UserData',uD{1+mltApp})
set(handles.checkNumGroups,'enable',eStr{1+mltGrp})    
checkOtherFormat(handles.checkSepByApp,'1')
...
if (isempty(iSel) || (nList == 0))
    % no selection made
    set(handles.buttonMoveUp,'enable','off')
    set(handles.buttonMoveDown,'enable','off')    
else
    % update based on the current selection
    set(handles.buttonMoveUp,'enable',eStr{1+(~any(iSel==1))})
    set(handles.buttonMoveDown,'enable',eStr{1+(~any(iSel==nList))})
end
...
for i = 1:length(hCheck)
    ...
    switch (get(hCheckNw,'tag'))
        case ('checkSepByExpt')
            ...
            set(hCheckNw,'enable',eStr{1+(isOK&&(sum(iData.expOut(:,iData.cTab))>1))})            
        ...
        case ('checkSepByApp')
            isOK = all(iSelT ~= [5:8 nMetG]) && (getSelectedIndexType(handles)==2);
            set(hCheckNw,'enable',eStr{1+(isOK&&(sum(iData.appOut(:,iData.cTab))>1))})            
        case ('checkNumGroups')
            % determines
            switch (iSelT)
            ...
            set(hCheckNw,'enable',eStr{1+hasGrp})                                        
    end
end
...
set(handles.textUnits,'enable',eStr{1+tOK})
set(handles.popupUnits,'enable',eStr{1+tOK})
...
[isOKE,isOKD,eStr] = deal(iSelT == 2,any(iSelT == [2 4 6]),{'off','on'});
set(handles.checkSepByDay,'enable',eStr{1+(isOKD && uDataD && iData.sepDay)})
set(handles.checkSepByExpt,'enable',eStr{1+(isOKE && uDataE && iData.sepExp)})
set(handles.checkSepByApp,'enable',eStr{1+(uDataA && (nApp > 1))})
set(handles.checkNumGroups,'enable',eStr{1+(~any(iSelT == nMetG))})
