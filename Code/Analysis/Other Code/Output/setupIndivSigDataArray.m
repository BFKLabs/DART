...
% [outType,tSp,hasTSP] = deal(sepDay + 1,{'Day',''},false);
% 
...
% ii = cellfun(@(x)(any(strcmp(xDepT,x))),field2cell(xVar0,'Var'));
% 
...
% % = 1 - all day combined
% % = 2 - individual days
% nDayT = sepDay*(nDay-1)+1;
% [YR,iFly] = reduceDataArray(Y(:,outType),appOut,expOut,iOrder,nDayT,hasTSP);
% 
...
if any(ii)
    ...
    gStr = getStructField(plotD,xDep{find(ii,1,'first')}{2});
    nGrp = length(gStr);
    
    jj = strcmp(field2cell(xVar0,'Var'),xDep{find(ii,1,'first')}{1});
    tStr = {xVar0(jj).Name};
    ...
    jj = strcmp(field2cell(xVar0,'Var'),xDep{1});
    tSp{end} = xVar0(jj).Name;
...
% else
%     xVarP = field2cell(pData.oP.xVar,'Var');
%     xDepS = cellfun(@(x)...
%                 (x{1}),field2cell(pData.oP.yVar(mIndG),'xDep'),'un',0);
%     tStr = pData.oP.xVar(strcmp(xDepS{1},xVarP)).Name;
% end
...
% % --- removes the genotype groups that are not included
% function [YR,iFly] = reduceDataArray(Y,appOut,expOut,iOrder,nDayT,hasTSP)
% 
% % memory allocation
...
%             isOK = true;
%             if (i == 1) && isOK
%                 iFly{iApp,iExp} = kk{iApp}(1:length(kk{iApp})/nDayT(iExp));
%             end
%         end        
%     end
%     
