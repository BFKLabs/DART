...
if (~iscell(mName))
    waitbarFig(1,sprintf('%s (%s)',wStr{1},'Loading Experimental Solution File'),0,h);    
    A = untar(mName,TempDir);    
else    
    waitbarFig(1,sprintf('%s (%s)',wStr{1},'Copying Experimental Solution File'),0,h);
...
for i = 1:nSoln
    ...
    waitbarFig(1,wStrNw,(i+1)/(nSoln+2),h);
    ...
    if (~ok)
        ...
        if ((~isequal(nOk,isOk)) && (isequal(isOk,nFly)))
            ...
            snTot{i}.appPara.flyok = cellfun(@(x)(true(x,1)),num2cell(isOk),'un',0);
        end
    end
end
...
waitbarFig(1,'Multi-Experimental Solution File Load Complete!',1,h);
close(h);
