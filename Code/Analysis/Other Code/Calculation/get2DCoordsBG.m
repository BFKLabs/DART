...
for i = 1:nApp
    ...
    indC{i} = cellfun(@(x)(find(cellfun(@(y)((x>=y(1)) && (x<=y(2))),xEnd))),xMean{i});
        
    % optimises the circle centre/radii
    [pCF,pC{i},RadNw] = optCirclePara(iMov,indC{i});
    PxMn = num2cell([floor(min(PxNw{i},[],1))',ceil(max(PxNw{i},[],1))'],2);
    PyMn = num2cell([floor(min(PyNw{i},[],1))',ceil(max(PyNw{i},[],1))'],2);
    
    %
    PxMn = cellfun(@(x)([x([1 2 2 1]),mean(x)]),PxMn,'un',0);
    PyMn = cellfun(@(x)([x([1 2 2 1]),mean(x)]),PyMn,'un',0);
    
    % determines the sub-region row indices for each fly in the group
    indR{i} = cellfun(@(x,y)(find(cellfun(@(z)(...
                any(inpolygon(x,y,z(:,1),z(:,2)))),pCF))),PxMn,PyMn);  
...
if (nApp == 1)
    [dPx,dPy,Rad,pC,indR,indC] = deal(dPx{1},dPy{1},Rad{1},pC{1},indR{1},indC{1}); 
end