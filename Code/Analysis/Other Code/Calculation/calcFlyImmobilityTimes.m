...
indNR = cellfun(@(x)...
    ([x(2) find(T<(T(x(2))+cP.tNonR),1,'last')]),indB,'un',0);
...
if isfield(cP,'moveOnly')
    if cP.moveOnly
        % removes any of the flies that have not reacted to the stimuli 
        % within the time, tReact
        isReact = sum(calcDistanceRange(PxR,PyR) >= cP.dMove,1) >= nGrpMin;
        iMove(~isReact) = NaN;
    end
end
...
% --- determines the array index, Px, where the fly has moved more 
%     than a distance, dMove --- %
...
iGrp = getGroupIndex(dRng > dMove);
if isempty(iGrp)
    % if there was no movement, then set a NaN values for the index
    iMove = NaN;
else
    % searches to determine if the movement value is not anomalous
    ii = find(cellfun(@length,iGrp) > nGrpMin,1,'last');
    if isempty(ii)
        % if none of the groupings are long enough, then flag no movement
        iMove = NaN;
    else
        % otherwise, return the final index of the feasible grouping
        iMove = iGrp{ii}(end);
    end
end
