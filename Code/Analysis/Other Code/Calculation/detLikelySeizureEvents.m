...
% memory allocation
[nFile,nFly] = deal(size(ind,1),getFlyCount(iMov));
...
if (nargin == 1)
    ...
    h = waitbarFig(wStr,'Detecting Seizure Events'); 
else
...
for iApp = 1:nApp  
    ...
    wStrNw = sprintf('%s (%i of %i)',wStr{1},iApp,nApp);
    if (waitbarFig(1,wStrNw,iApp/nApp,h))
    ...
    if (iMov.ok(iApp))
        ...
        for iFly = 1:nFly(iApp)
            % updates the waitbar figure
            wStrNw = sprintf('%s (%i of %i)',wStr{2},iFly,nFly(iApp));
            if (waitbarFig(2,wStrNw,iFly/nFly(iApp),h))
                % if the user cancelled, then exit the function
                [indF,Pmet] = deal([]);
                return
            end
            ...
            if (iMov.flyok(iFly,iApp))                                
                ...
                for iFile = 1:nFile
                    % updates the waitbar figure
                    wStrNw = sprintf('%s (Video %i of %i)',wStr{3},iFile,nFile);
                    if (waitbarFig(3,wStrNw,iFile/nFile,h))
                        % if the user cancelled, then exit the function
                        [indF,Pmet] = deal([]);
                        return
                    end
...
if (~waitbarFig(3,'Analysis Complete!',1,h))
