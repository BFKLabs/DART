...
if (nargin < 3)
    h = waitbarFig({'Initialising'},'Retrieving Sleep Intensity Data');
end
...
waitbarFig(1+wOfs,'Determining Time Bin Indices...',1/(2+nApp),h);
...
if (nargout > 2)  
    ...
    if (~isfield(cP,'tBefore') && ~isfield(cP,'tAfter'))
        ...
        eStr = 'Error! For stimuli response, you must include the tBefore or tAfter variables.';
        waitfor(errordlg(eStr,'Missing Parameter Error','modal'));        
        [Ycount,YcountR,Ybin] = deal([]);
        return
    ...
    elseif (~isfield(cP,'tAfter'))
        ...
        iTa1 = cellfun(@(x)(find(Ttot>(x-(cP.tBefore*60+cP.nAvg+1)),1,'first')),Ts,'un',0);
    ...
    ii = (~cellfun(@isempty,iTs) & ~cellfun(@isempty,iTa1) & ~cellfun(@isempty,iTa2));    
    
    % sets the indices of the after stimuli signal points and also set the
    % time signal values
    [iTa1,iTa2,iTs] = deal(cell2mat(iTa1(ii)),cell2mat(iTa2(ii)),cell2mat(iTs(ii)));
    indS = num2cell([iTa1,iTa2],2);
    Tsig = cellfun(@(x)(max(0,roundP(Ttot(x(1):x(2))-Ttot(x(1))))),indS,'un',0);
else
...
% memory allocation
A = cellfun(@(x)(repmat({zeros(1+cP.sepDN,nGrp)},nDay,length(x))),flyok,'un',0);
...
for i = 1:nApp
    ...
    wStrNw = sprintf('Setting Binned Signals (Apparatus %i of %i)',i,nApp);
    if (waitbarFig(1+wOfs,wStrNw,0.5*(1+wOfs)*(i+1)/(2+nApp),h))
        ...
        if (nargout > 2)
            ...
            for j = 1:length(indB)
                ...
                if (any(jj))
                    ...
                    if (~isempty(Px))
                        Xnw{j} = setBinnedSignals(Px(indSnw,jj),Tsig{j},nSig);
                    end

                    % calculates the binned y-values (if they are present)
                    if (~isempty(Py))
                        Ynw{j} = setBinnedSignals(Py(indSnw,jj),Tsig{j},nSig);
                    end                
                end
            end
            ...
            for j = 1:numel(iYnwT)
                ...
                if (~isempty(iYnwT{j}))
                    ...
                    if (~isempty(Px))
                        if (cP.moveOnly)                
                            Xbin{i}{j} = cellfun(@(x)(XbinT{j}(:,x(iRnwT{j}(:,x)))),ii,'un',0); 
                        else
                            Xbin{i}{j} = cellfun(@(x)(XbinT{j}(:,x)),ii,'un',0);
                        end                
                    end            
                    ...
                    if (~isempty(Py))
                        if (cP.moveOnly)                
                            Ybin{i}{j} = cellfun(@(x)(YbinT{j}(:,x(iRnwT{j}(:,x)))),ii,'un',0); 
                        else
                            Ybin{i}{j} = cellfun(@(x)(YbinT{j}(:,x)),ii,'un',0);
                        end                
                    end
                end
            end
...
if (nargin < 3)
    ...
    waitbarFig(1+wOfs,'Time Binning Signals Complete!',1,h);
end