...
if (isempty(h))
    ...
    [h,wOfs] = deal(waitbarFig(wStr,'Speed Calculations'),0);    
else
...
wFunc = getappdata(h,'updateBar');
if (wOfs > 0); wFunc(2+wOfs,'',0,h); end

% determines the bin indices and removes and bins that have not matches
wFunc(1+wOfs,wStr,0.125,h); pause(0.01)
...
for iApp = 1:nApp
    ...
    wStrNw = sprintf('Calculating Mid-Line Crossings (Group %i of %i)',iApp,nApp);
    if (wFunc(1+wOfs,wStrNw,0.25+0.75*(iApp/nApp),h))
        [TBin,YBin,YRaw] = deal([]);
        return
    end
    ...
    YBin{iApp} = cellfun(@(x)(60*sum(YRaw{iApp}(x))/diff(TT(x([1 end])))),indB);
end
