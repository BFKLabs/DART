...
blkInfo = reduceStimBlockInfo(snTot(1).sTrainEx.sTrain(1).blkInfo);

% calculates the signal duration and the time from the start of the signal
% stimuli train to the next train
Tsig = arrayfun(@(x)(x.sPara.tOfs),blkInfo(2:end));
TsigR = [0;Tsig;sum(Tsig([1,end]));1e10];
...
for i = 1:nApp
    ...
    if ~isempty(snTot.Px{i})
        ...
        if snTot.iMov.is2D; Py = snTot.Py{i}(:,flyok{i}); end
                
        % calculates the pre-stimuli immobility times and the post-stimuli
        % reactivity times (if a fly is already "active" before the stimuli
        % then 
        ...
        indReact0 = cellfun(@(x,y)(detStimReactGroup(...
            x,y,tImmobTol,TsigR)),tImmobGrp,tReactGrp,'un',0);  
        
        % fills any empty values within the array
...
iLvl = cellfun(@(x)(find(x>=TsigR,1,'last')),num2cell(tReact(isImmob)));
indReact(isImmob) = iLvl;