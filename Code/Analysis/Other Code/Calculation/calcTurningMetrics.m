...
if (nargin < 3)
    wStr = {'Analysing Apparatus','Calculating Turn Statistics',...
            'Turn Angle Calculations'};
    [h,wOfs] = deal(waitbarFig(wStr,'Turn Analysis'),0);
else
    [wStr,wOfs] = deal(getappdata(h,'wStr'),nExp>0);
end
...
for i = 1:nApp
    ...
    wStrNw = sprintf('%s (Group %i of %i)',wStr{1},i,nApp);
    if (waitbarFig(1+wOfs,wStrNw,i/nApp,h))
    ...
    if (snTot.appPara.ok(i))
        % updates the waitbar figure
        waitbarFig(2+wOfs,'Calculating Relative 2D Coordinates',1/(length(fok{i})+1),h);
        waitbarFig(3+wOfs,wStr{3},0,h);
        pause(0.05);
        ...
        for j = 1:nFly  
            % updates the waitbar figure
            wStrNw = sprintf('%s (Fly %i of %i)',wStr{2},j,nFly);
            if (waitbarFig(2+wOfs,wStrNw,(j+1)/(nFly+1),h))
                % if the user cancelled, then exit the function
                tData = [];
                return
            else
                % otherwise, reset the waitbar figure
                waitbarFig(3+wOfs,'Initial Turn Detection',0,h);
            end            
            ...
            % updates the waitbar figure
            waitbarFig(3+wOfs,'Classifying Turn Events',0.9,h);
...
% initialisations          
try
    [wStr,nTurn,ok] = deal(getappdata(h,'wStr'),length(iFrm),true);     
...
for k = 1:nTurn
    ...
    if (~isW(iFound))
        ...
        % updates the waitbar figure
        wStrNw = sprintf('%s (%i%s Complete)',wStr{3},roundP(100*k/nTurn),char(37));
        if (waitbarFig(3+wOfs,wStrNw,0.4*(isNorm+k/nTurn),h))
            % if the user cancelled, then exit the function
            ok = false;
            [dtTurn,iTurn,pTurn,dTurn] = deal([]);
            return
...
if (isNorm); waitbarFig(3+wOfs,'Calculating Turn Angles',0.85,h); end

% calculates the angle of the turn
pTurn = cellfun(@(x)(integral(@(y)(ppval(pP,y)),x(1),x(2))),num2cell(tTurn,2));     
...
if (isNorm)
    ...
    if (size(tTurn,1) > 1)
        ...
        % determines which turn events are close enough to be combined and have the
        % same turn direction (i.e. slight pause within a turn that can be removed)
