...
% global variables
global szDel mainProgDir bufData pAR
...
setappdata(hObject,'pData',[]);
setappdata(hObject,'hSolnT',[])
setappdata(hObject,'hTube',[]);
setappdata(hObject,'hMark',[]);
setappdata(hObject,'hDir',[]);
setappdata(hObject,'hMainGUI',[])
setappdata(hObject,'vidTimer',[])
...
switch length(varargin) 
    ...
    case {3,4} % case is calibration
        ...
        if isempty(hGUI)
            ...
            % sets the input argument and the open GUI (makes invisible)
            reopenGUI = varargin{2};  
            setappdata(hObject,'hGUIOpen','figFlyRecord') 

            % closes the GUI (if running calibration from Fly Record GUI
            if reopenGUI; setObjVisibility(hGUI.figFlyRecord,'off'); end
        ...
        % sets the GUI re-opening flag
        setappdata(hObject,'reopenGUI',reopenGUI)
        
    otherwise % case is any other number of input arguments
        % displays an error message
        eStr = 'Error! Incorrect number of input arguments. Exiting Tracking GUI...';
...
setappdata(hObject,'cType',cType)        

% adds the ImageSeg/image processing code directories
if isempty(getappdata(hObject,'hGUIOpen'))
    wState = warning('off','all');
    ...
    updateSubDirectories(fullfile(mainProgDir,'Test Files'),'add')
...
setappdata(hObject,'PPDef',iData.ProgDef);
set(handles.imgAxes,'DrawMode','fast');

% sets all the functions
setappdata(hObject,'dispImage',@dispImage);
setappdata(hObject,'checkFixRatio_Callback',@checkFixRatio_Callback);
setappdata(hObject,'setupDivisionFigure',@setupDivisionFigure);
setappdata(hObject,'removeDivisionFigure',@removeDivisionFigure);
setappdata(hObject,'deleteAllMarkers',@deleteAllMarkers);
setappdata(hObject,'initMarkerPlots',@initMarkerPlots);
setappdata(hObject,'checkShowTube_Callback',@checkShowTube_Callback);
setappdata(hObject,'menuViewProgress_Callback',@menuViewProgress_Callback);
setappdata(hObject,'menuOpenSoln_Callback',@menuOpenSoln_Callback);
setappdata(hObject,'menuVideoFeed_Callback',@menuVideoFeed_Callback)
setappdata(hObject,'menuRTTrack',@menuRTTrack_Callback)
setappdata(hObject,'checkLocalView_Callback',@checkLocalView_Callback)
setappdata(hObject,'checkSubRegions_Callback',@checkSubRegions_Callback)
setappdata(hObject,'checkShowMark_Callback',@checkShowMark_Callback)
setappdata(hObject,'checkShowAngle_Callback',@checkShowAngle_Callback)
setappdata(hObject,'getMarkerProps',@getMarkerProps)
setappdata(hObject,'FirstButtonCallback',@FirstButtonCallback)
setappdata(hObject,'LastButtonCallback',@LastButtonCallback)
setappdata(hObject,'PrevButtonCallback',@PrevButtonCallback)
setappdata(hObject,'NextButtonCallback',@NextButtonCallback)
setappdata(hObject,'CountEditCallback',@CountEditCallback)
setappdata(hObject,'ImageParaCallback',@ImageParaCallback)
setappdata(hObject,'updateAllPlotMarkers',@updateAllPlotMarkers)
setappdata(hObject,'updateVideoFeedImage',@updateVideoFeedImage)
...
switch length(varargin)
    case {0,1} % case is the normal tracking (0 input for command-line, 1 thru DART)        
        isCalib = false;
    ...
    case 3 % case is the full calibration (thru Fly Record)
        isCalib = true;
        ...
        figPos0 = get(handles.figFlyTrack,'position');
    ...
    case 4 % case is the calibration test (thru command-line or DART)       
        isCalib = true;
        if ~isempty(hGUI)
            hMainGUI = hGUI.figFlyRecord;   
        else
        ...
        % retrieves/sets/updates the scale factor
        if (cType == 1) && (~isempty(hGUI))
            rtP = getappdata(hGUI.figFlyRecord,'rtP');
            iData.exP.sFac = rtP.trkP.sFac;
            set(handles.editScaleFactor,'string',num2str(iData.exP.sFac))        
            setappdata(hObject,'rtP',rtP)
            setappdata(hObject,'iStim',getappdata(hMainGUI,'iStim'))        
            ...
            % sets the sub-movie data struct
            iMov = getappdata(hGUI.figFlyRecord,'iMov');
            if isempty(iMov)
                % if the sub-movie data struct is empty, then initialise
                iMov = initMovStruct(iData);       
            end                     
        else
        ...
        setappdata(hObject,'objIMAQ',[])
        setappdata(hObject,'hMainGUI',hMainGUI)
        setappdata(hObject,'isTest',true)        
        setappdata(hObject,'iData',iData)                 
        setappdata(hObject,'iMov',iMov)
        
        % initialises the GUI properties        
        setTrackGUIProps(handles,'InitGUICalib'); pause(0.01); 
%         set(handles.textFrameSizeS,'string',nwStr)   
...
if setMovie
    ...
    mType = '*.avi;*.AVI;*.mj2;*.mp4;*.wmv';
    mStr = 'Movie Files (*.avi, *.AVI, *.mj2, *.mp4, *.wmv';
        ...
        if ~isSolnLoad                   
            if iMov.isSet
                ...
                if ~strcmp(uChoice,'Yes')
                    % re-initialises the sub-window data struct and disables
                    % the detect tube button
                    iMov.isSet = false;
                    iMov = initMovStruct(iData);                    
                else
                    ...
                    pFile = fullfile(iData0.ProgDef.TempFile,'Progress.mat');
                    if exist(pFile,'file')
                        delete(pFile)                            
                    end                
...
if cType > 0
    setappdata(handles.figFlyTrack,'fPosNew',[])
end
...
if loadImgData(handles, ldData.name, ldData.dir, setMovie, isSolnLoad)
    ...
    setappdata(handles.figFlyTrack,'bgObj',CalcBG(handles))
    ...
    if ~isSolnLoad
        ...
        setappdata(handles.figFlyTrack,'iData',iData0);          
        resetHandleSnapshot(hProp0,handles.figFlyTrack);
    end      
end
...
% retrieves the image mesh data and sets it into the main ImageSeg handle
wState = warning('off','all');
...
if isfield(solnData,'stimP')
    iData.stimP = solnData.stimP;
...
if menuOpenMovie_Callback(handles.menuOpenMovie, solnData.fData, handles)
    ...
    setappdata(handles.figFlyTrack,'pData',pData)
...
% retrieves the image data/mesh structs
[iData,a] = deal(getappdata(handles.figFlyTrack,'iData'),1);
...
pData = getappdata(handles.figFlyTrack,'pData');
iMov = getappdata(handles.figFlyTrack,'iMov');
...
iData = getappdata(handles.figFlyTrack,'iData');
[iData.ProgDef,isSave] = ProgParaTrack(handles.figFlyTrack,iData.ProgDef);
...
if isCalib
    ...
    rtP = getappdata(handles.figFlyTrack,'rtP');
    iMov = getappdata(handles.figFlyTrack,'iMov');    
    cType = getappdata(handles.figFlyTrack,'cType');    
...
if strcmp(uChoice,'Yes')  
    ...
    hGUIOpen = getappdata(handles.figFlyTrack,'hGUIOpen');
    reopenGUI = getappdata(handles.figFlyTrack,'reopenGUI');   
    ...
    if isCalib
        ...
        hMain = getappdata(handles.figFlyTrack,'hMainGUI');
        ...
        if ~isempty(hMain)
            ...
            hasDetect = initDetectCompleted(iMov);            
            setObjEnable(hMainH.toggleStartTracking,hasDetect)        
            setObjEnable(hMainH.menuRTPara,hasDetect)        
        end
        
        % determines if the tracking GUI is open
        hTrack = getappdata(handles.figFlyTrack,'hTrack');            
        if ~isempty(hTrack)
            % if so, then delete the tracking GUI
    ...
    hSolnT = getappdata(handles.figFlyTrack,'hSolnT');
    if ~isempty(hSolnT)
        try; delete(hSolnT); end
    end
...
if strcmp(get(hObject,'checked'),'off')
    ...
    setappdata(handles.figFlyTrack,'hSolnT',FlySolnView(handles));
    iMov = getappdata(handles.figFlyTrack,'iMov');
    ...
    try
        delete(getappdata(handles.figFlyTrack,'hSolnT'))
    end    
    
    % removes the check and GUI handle
    set(hObject,'checked','off')
    setappdata(handles.figFlyTrack,'hSolnT',[])
...
set(handles.figFlyTrack,'CurrentAxes',handles.imgAxes)
...
[iMov,isChange] = WindowSplit(handles);
...
if isChange           
    ...
    if isCalib
        ...
        if cType == 1
            ...
            setappdata(handles.figFlyTrack,'rtP',rtP)

            % enables the real-time parameter menu item
            setObjEnable(handles.menuRTPara,'on')
        else
            %
            setTrackGUIProps(handles,'UpdateSnapshotSelection')
        end
    else
    ...
    if isCalib
        ...
        % if the video feed was previously set to being on, then restart
        % the update timer. otherwise, reset the last static image
        if strcmp(get(handles.menuVideoFeed,'checked'),'on')
            start(vidTimer)            
        else
...
[vGrp,isChange] = VideoSplit(handles.figFlyTrack);
if isChange
...
if strcmp(get(hObject,'checked'),'on')
    % timer object is on, so turn off video timer
    if ~isempty(vidTimer)
        stop(vidTimer)
        set(hObject,'checked','off')        
        setObjEnable(handles.menuVideoProps,~isTest)    
    ...
    % alters the video feed status
    if ishandle(handles.editFrameStat)
        set(setObjEnable(handles.editFrameStat,'on'),'BackgroundColor','r')
        set(setObjEnable(handles.textStatusStr,'on'),...
                                                'string','Video Feed Off')  
    else
        set(handles.textVidStatusL,'ForegroundColor','r')
        set(handles.textVidStatus,'string','Off','ForegroundColor','r')
    end
    ...
    % alters the video feed status
    if ishandle(handles.editFrameStat)
        set(setObjEnable(handles.editFrameStat,1),'BackgroundColor','g')
        set(setObjEnable(handles.textStatusStr,1),'string','Video Feed On')
    else
        set(setObjEnable(handles.textVidStatusL,1),'ForegroundColor','g')
        set(setObjEnable(handles.textVidStatus,1),...
                            'string','On','ForegroundColor','g')
    end
...
% retrieves the video timer object
hMainGUI = getappdata(handles.figFlyTrack,'hMainGUI');

% runs the video parameter sub-GUI
VideoPara(guidata(hMainGUI))
...
rtP = getappdata(handles.figFlyTrack,'rtP');
rtD = getappdata(handles.figFlyTrack,'rtD');
iMov = getappdata(handles.figFlyTrack,'iMov');
iStim = getappdata(handles.figFlyTrack,'iStim');
dInfo = getappdata(handles.figFlyTrack,'objDACInfo');
...
if isChecked    
    ...
    hTrack = getappdata(handles.figFlyTrack,'hTrack');
    if ~isempty(hTrack)
        setappdata(handles.figFlyTrack,'hTrack',[])
        try; delete(hTrack); end
    end     
    ...
    setappdata(handles.figFlyTrack,'fPosNew',fPosNaN);
        
    % check to see that the devices have all been turned off correctly
    forceStopDevice(handles.figFlyTrack)
    ...
    setappdata(handles.figFlyTrack,'rtD',initRTDataStruct(iMov,rtP))
    ...
    hTrack = TrackingStats(handles.figFlyTrack);
    setappdata(handles.figFlyTrack,'hTrack',hTrack); 
    set(handles.figFlyTrack,'CurrentAxes',handles.imgAxes);            
end  
...
TrackingPara(handles.figFlyTrack)
...
iData = getappdata(handles.figFlyTrack,'iData');
iData.cFrm = str2double(get(handles.frmCountEdit,'string'));
setappdata(handles.figFlyTrack,'iData',iData)
...
if ~updateTube
    ...
    hTube = getappdata(handles.figFlyTrack,'hTube');
...
if chkEditValue(nwVal,[1 iData.nFrm],1)
    iData.cStp = nwVal;
    setappdata(handles.figFlyTrack,'iData',iData);
else
    set(hObject,'string',num2str(iData.cStp));
end

% --- callback function for the first frame/sub-movie button --------------
function FirstButtonCallback(hObject, eventdata, handles, varargin)
...
if ~isempty(varargin)    
    hFig = findall(0,'tag','figDirectDetect');
else
...
switch get(hObject,'UserData')
    ...
    case 'Sub' % case is the sub-movie
        ...
        iMov = getappdata(handles.figFlyTrack,'iMov');
        set(handles.checkReject,'value',~iMov.ok(1));        
end

% updates the data struct with the new value
eval(sprintf('iData.%s = 1;',pStr));
...
switch get(hObject,'UserData')        
    ...
    case 'First' % case is the movie frame (Direct Detect)
        ...
        setTrackGUIProps(varargin{1},'UpdateFrameSelection',1)        
...
% updates the data struct with the new value
eval(sprintf('iData.%s = %i;',pStr,nwVal));
setappdata(hFig,'iData',iData);
...
function PrevButtonCallback(hObject, eventdata, handles, varargin)
...
% % sets the GUI figure handle
if ~isempty(varargin)    
    [hFig,isMain] = deal(findall(0,'tag','figDirectDetect'),false);
else
    [hFig,isMain] = deal(handles.figFlyTrack,true);
end
...
if nwVal > 1
    % updates the data struct with the new value
    eval(sprintf('iData.%s = max(1,%i);',pStr,nwVal-cStp));
    setappdata(hFig,'iData',iData);    
    ...
    switch get(hObject,'UserData')      
        ...
        case 'Sub' % case is the sub-movie
            iMov = getappdata(handles.figFlyTrack,'iMov');
            set(handles.checkReject,'value',~iMov.ok(iData.cMov));
...
if nwVal < mxVal
    % updates the data struct with the new value
    eval(sprintf('iData.%s = %i;',pStr,nwVal+cStp));
    setappdata(hFig,'iData',iData);    
...
switch get(hObject,'UserData')       
    case {'Frame','DD'} % case is the movie frame
        [pStr,nwLim] = deal('iData.cFrm',[1,iData.nFrm]);
                
    case 'Sub' % case is the sub-movie
        iMov = getappdata(handles.figFlyTrack,'iMov');
        [pStr,nwLim] = deal('iData.cMov',[1,iData.nMov]);
        set(handles.checkReject,'value',~iMov.ok(iData.cMov));        
end
...
if isCalib 
    [iMov,eStr] = deal(getappdata(handles.figFlyTrack,'iMov'),{'off','on'});
    setObjEnable(handles.menuRTTrack,eStr{1+iMov.isSet})
    if strcmp(get(handles.menuVideoFeed,'checked'),'on')   
        start(vidTimer)
    end
end   
...
iData = getappdata(handles.figFlyTrack,'iData');
hTube = getappdata(handles.figFlyTrack,'hTube');
[ii,jj,vStr,mlt] = deal([1 2 2 1],[1 1 2 2],{'off','on'},1);
...
for i = 1:length(hTube)
    if ~showUpdate
        % sets the x/y offset based on the detection type
        if strcmp(Type,'General')
            % case is for general region automatic detection
            if isLocalView
            ...
            % sets the x-coordinates of the tube
        ...
        for j = 1:length(hTube{i})
            ...
            if strcmp(Type,'General')
                % case is for general region automatic detection
                x = iMov.autoP.X{j,i} - xOfs;
                y = iMov.autoP.Y{j,i} - yOfs;

                % sets the final tube outline x/y coordinates
                [xTube,yTube] = deal(x([(1:end),1]),y([(1:end),1]));
            else
                % case is for the other detection types
                if isColGroup(iMov)
    ...
    cellfun(@(x)(set(x,'Visible',vStr{isShow+1})),hTube{i})
end
...
% determines if the direct detection GUI is open
hasDD = ~isempty(findall(0,'tag','figDirectDetect'));
...
if isCalib || hasDD
    if isfield(handles,'menuRTTrack')
        ...
        iMov = getappdata(handles.figFlyTrack,'iMov');
        updateAllPlotMarkers(handles,iMov,true,hasDD)
    end
else
    dispImage(handles)
end
...
if isCalib
    ...
    % if the tube regions are missing, then add them
    if isempty(getappdata(handles.figFlyTrack,'hTube'))
        initMarkerPlots(handles,1)    
    end
else
...
bgObj = getappdata(handles.figFlyTrack,'bgObj');
bgObj.openBGAnalysis();
...
if strContains(iMov.bgP.algoType,'single')
...
% sets the frame status colour/string based on the frame stats
if ~isempty(iData.stimP)
    % sets the global movie frame index
    cFrmG = iMov.sRate*(cFrm-1) + ((1:iMov.sRate)-1) + iData.Frm0;  
    cFrmG = max(1,cFrmG(1)):min(length(iData.stimFrm),cFrmG(end));
    
    switch max(iData.stimFrm(cFrmG))
        case 0 % case is a normal frame
            [sStrNw,bCol] = deal('Normal','g');
            
        case 1
            ii = find(iData.stimFrm((cFrmG(end)+1):end)==3,1,'first');
            [frmNext,frmStr] = deal(roundP(ii/iMov.sRate,1),'Frame');
            if frmNext > 1; frmStr = [frmStr,'s']; end
            
            [sStrNw,bCol] = deal(sprintf('Stimulus In %i %s',frmNext,frmStr),'y');

        case 2
            [sStrNw,bCol] = deal('Inter-Stimulus Pause',[255 102 0]/255);
        
        case 3
            [sStrNw,bCol] = deal('Stimulus Event','r');
    end

    % updates the status properties/fields
    if ishandle(handles.textStatusStr)
        if ~isBGCalc
            set(handles.textStatusStr,'String',sStrNw,'Enable','on')
            set(handles.editFrameStat,'BackgroundColor',bCol,'Enable','inactive')
        else
            set(handles.textStatusStr,'String',sStrNw)
        end
    end
else
    % updates the status properties/fields
    if ~isCalib
        set(handles.textStatusStr,'String','N/A','Enable','off')
        set(handles.editFrameStat,'Enable','off')    
    end
end
...
function updateAllPlotMarkers(handles,iMov,hasImg,hasDD)
...
nApp = length(hMark);
if isCalib || hasDD
    fPosTmp = getappdata(handles.figFlyTrack,'fPosNew');
    if isempty(fPosTmp); return; end        
end
...
if ~isempty(hMark)    
    for i = 1:nApp
        ...
        if isCalib || hasDD        
            updateIndivMarker(handles,hMark{i},hDir{i},fPosTmp,i,pltLoc,pltAng,true)                     
        else
            updateIndivMarker(handles,hMark{i},hDir{i},pData,i,pltLoc,pltAng) 
        end        
    end
end
...
while (iFrm + cStp) <= iData.nFrm
    ...
    if ~get(handles.toggleVideo, 'value')
        stopMovie(handles)
        isComplete = false;
        return
    else
...
[nApp,isCG] = deal(iMov.nRow*iMov.nCol,isColGroup(iMov));
...
hAx = handles.imgAxes;
set(handles.figFlyTrack,'CurrentAxes',hAx)
...
for i = 1:nApp         
    ...
    for j = 1:length(hMark{i})
        ...
        [hTStr,hMStr] = deal(sprintf('hTube%i',i),sprintf('hMark%i',i));
        hDStr = sprintf('hDir%i',i);
        
        % sets the plot colour for the tubes
        [pCol,fAlpha,edgeCol,pMark,mSz] = getMarkerProps(iMov,i,j);
        
        % determines if general automatic detection was used
        if strcmp(Type,'General')
            % if so, then 
            [x,y] = deal(iMov.autoP.X{j,i},iMov.autoP.Y{j,i});
            [xTube,yTube] = deal(x([1:end,1]),y([1:end,1]));
        else
            % otherwise, set the tube region patch based on storage type
            if isCG
                x = iMov.xTube{i}(j,:) + xOfs;
        ...
        hMark{i}{j} = plot(NaN,NaN,'Color',pCol,'Marker',pMark,'tag',hMStr,...
                        'MarkerSize',mSz,'LineWidth',lWid,'Parent',hAx);
...
setappdata(handles.figFlyTrack,'hMark',hMark);
setappdata(handles.figFlyTrack,'hTube',hTube);
setappdata(handles.figFlyTrack,'hDir',hDir);
...
for i = 1:length(hMark)
    ...
    hTube = findobj(handles.imgAxes,'tag',sprintf('hTube%i',i));
    if ~isempty(hTube); delete(hTube); end
    
    % deletes any previous fly markers
    hMark = findobj(handles.imgAxes,'tag',sprintf('hMark%i',i));
...
setappdata(handles.figFlyTrack,'hMark',[]);
setappdata(handles.figFlyTrack,'hTube',[]);
setappdata(handles.figFlyTrack,'hDir',[]);
...
if ~strcmp(Type,'General')
    ...
    for i = 1:(nRow-1)        
        for j = 1:nCol
            ...
            set(hLineNw,'xData',xPltH(j+(0:1)),'yData',yPltH*[1 1],'Visible','on');
        end
    end
    ...
    if isMove                      
        ...
        switch Type
            case 'General'
                ...
                if isempty(hInner)
                    hold(hAx,'on')                                        
                    line(iMov.autoP.XG{i},iMov.autoP.YG{i},...
                                'color','g','tag','hInner','UserData',i);
                    hold(hAx,'off')
                else
...
if exist(progFile,'file')
    ...
    switch uChoice
        case 'Manually'
            ...
            ProgDef = ProgParaTrack(handles.figFlyTrack,[],1);            
        case 'Automatically'
...
if any(~isExist)
    ...
    if nargin == 1
        ProgDef = ProgParaTrack(handles.figFlyTrack,ProgDef,1);    
    end
end
...
iMov = struct('pos',[1 1 1 1],'posG',[],'Ibg',[],'ddD',[],...
              'nRow',[],'nCol',[],'nTube',[],'nTubeR',[],'dTube',false,...              
              'iR',[],'iC',[],'iRT',[],'iCT',[],'xTube',[],'yTube',[],...
              'sgP',[],'Status',[],'tempName',[],'autoP',[],'pStats',[],'bgP',[],...
              'isSet',false,'ok',true,'tempSet',false,'isOpt',false,...
              'sRate',5,'nDS',1,'rot90',false,'calcPhi',trkP.calcPhi,'vGrp',[]);           
iMov.sgP = iData.sgP;          
...
if nargin == 1
    exP = struct('FPS',NaN,'sFac',1,'nTubeR',20,'nTubeC',2);    
end
...
for i = 1:length(a)
    ...
    % value
    hEdit = findobj(handles.figFlyTrack,'UserData',a{i});
...
setappdata(handles.figFlyTrack,'vidTimer',vidTimer)
...
if isCalib
    ...
    fAlpha = 0.1*(1 + (iMov.Status{i}(j)~=1));
    pCol = eval(sprintf('mPara.%s.pCol',cStr{iMov.Status{i}(j)+1}));
    pMark = eval(sprintf('mPara.%s.pMark',cStr{iMov.Status{i}(j)+1}));
    mSz = eval(sprintf('mPara.%s.mSz',cStr{iMov.Status{i}(j)+1}));    
end
