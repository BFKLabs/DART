...
switch (typeStr)
    ...
    case ('InitGUICalib') % case is initialising the GUI     
        ...
        if isTest
            sStr = 'Video Feed Off';    
            setObjEnable(handles.menuWinsplit,'off')
        else
            sStr = 'Video Feed On';
        end
    	...
    	set(handles.editFrameStat,'enable','on','BackgroundColor','g')
    	set(handles.textStatus,'enable','on')
    	set(handles.textStatusStr,'enable','on','string',sStr)
        
        if isTest
            set(handles.menuVideoFeed,'checked','off','enable','off')
            set(handles.menuVideoProps,'visible','off')
    ...
    case ('PostImageLoad') % case is the successful movie load
    	...
    	set(handles.frmCountEdit,'string',...
            	num2str(iData.cFrm),'enable','on'); 
    	set(handles.editFrameRate,'string',...
            	num2str(roundP(iData.exP.FPS,0.01)),'enable','inactive')
    	set(handles.editFrameStep,'string',num2str(iData.cStp),'enable','on')
    	set(handles.textFrameStep,'enable','on')

        set(handles.menuWinsplit,'enable','on')
        dispImage(handles);      
...
%                     % old solution file - needs to be recalculated,
%                     % although can still view solution
%                     iData.Status = 0;                    
%                     setDetectEnable(handles,'on',[1:2 4])                      
%                     setDetectEnable(handles,'off',5) 
%                     setMenuEnable(handles,'off',6)
%                 else
                    % can view everything
                    iData.Status = 2;
...
%                 end
    ...
    case ('PreTubeDetect') % case is before detecting the tube regions
        ...
        set(handles.checkShowTube,'value',0,'enable','off')
        set(handles.checkShowMark,'value',0,'enable','off')
        set(handles.checkShowAngle,'value',0,'enable','off')
        setDetectEnable(handles,'off')         
    ...
    case ('PostTubeDetect') % case is after detecting the tube regions
        ...
        if isCalib
            ...
            set(handles.menuBatchProcess,'enable','on')            
            set(handles.frmCountEdit,'string',num2str(iData.cFrm))                
        end                           
    ...
    case ('PreFlyDetect') % case is after detecting the fly locations 
        ...
        set(handles.menuFile,'enable','off')
        setMenuEnable(handles,'off',[4 6 7])
        
    case ('PostFlyDetect') % case is after detecting the fly locations 
        % enables the tube region outline markers
        setDetectEnable(handles,'on',1:2) 
        if (iMov.calcPhi); setDetectEnable(handles,'on',3) ; end
        set(handles.menuFile,'enable','on')
        setMenuEnable(handles,'on')    

        % if there is more than one image-stack analysed, then 
        set(handles.checkShowTube,'value',1)
        if (pData.nCount > 0)
            set(handles.checkShowMark,'value',1)
            if (iMov.calcPhi)
                set(handles.checkShowAngle,'value',1)
            end
        end                    
    ...
    case ('PreBatchProcess') % case is before the batch processing
        ...
        set(handles.menuViewProgress,'enable','on')        
    ...
    case ('PostBatchProcess') % case is after the batch processing        
        ...
        set(handles.editFrameStat,'enable','inactive');
        set(hMenu,'enable','on');
    ...
    case ('UpdateFrameSelection') % case is updating the frame selection buttons
        ...
        if isfield(handles,'menuManualReseg')
            ...
            elseif (~isfield(pData,'fPos'))
                ...
                if (isnan(pData.fPos{j0}{i0}(iData.cFrm,1)))
                    % turns off the manual segmentation menu item
                    setMenuEnable(handles,'off',7)                
                else
                    % turns off the manual segmentation menu item
            ...
            set(handles.buttonUseCurrent,'enable','off')
            
            % determines if the current frame is the analysed frame
            if iData.iFrm == cFrm
                % enables all radio button and marker checkbox
                set(handlesM.checkShowMark,'enable','on')
                setPanelProps(handles.panelImageType,'on')
                                
            else
                % disables the associated radio/checkbox objects
                set(handles.radioHorizRes,'enable','off')
                set(handles.radioVertRes,'enable','off')                
                set(handles.radioMetrics,'enable','off')
                set(handles.radioRawMetric,'enable','off')
                set(handlesM.checkShowMark,'enable','off')
    ...
    case ('EnableAppSelect')
        ...
        if (isempty(findobj(0,'tag','figManualReseg')) ...
            ...
            set(handles.checkReject,'enable','on')
        end
...
%             % check to see if any buttons need to be enabled/disabled
%             set(handles.frmCountEdit,'enable','on')
...
%                 
%                 % turns off the movie play button
%                 set(handles.frmPlayButton,'enable','off');
%             else
%                 % turns on the movie play button
%                 set(handles.frmPlayButton,'enable','on');
%                 if (pVar{1} == 1)
%                     % if the new frame index is the first frame, enable the
%                     % next/last buttons and disable the 1st/last buttons
%                     setFrmEnable(handles,'off',1:2);
...
%         end
    ...
    case ('UpdateImage&Data') % case is updating the image/data fields
%         dispImage(handles)
%         setImgData(handles)
        
    case ('UpdateImage&Props') % case is updating the image/data fields
%         setObjProps(handles,iData);
%         dispImage(handles,2)
...
for i = 1:length(hTextL)
    ...
    set(hTextL(i),'enable',state);
    set(hTextS(i),'enable',state);
...
if (iMov.isSet && (nargin == 1))
    ...
    set(handles.textRowCount,'string',num2str(iMov.nRow),'enable','on')
    set(handles.textColCount,'string',num2str(iMov.nCol),'enable','on')
    ...
    set(handles.textRowCount,'string','','enable','off')
    set(handles.textColCount,'string','','enable','off')
    
    % enables the check boxes/buttons in the frame
    set(handles.checkSubRegions,'enable','off','value',0)
%     set(handles.checkLocalView,'enable','off','value',0)
    set(handles.checkReject,'enable','off','value',0)    
end
...
% --- sets the object properties based on the solution status -------------
function setObjProps(handles,iData)

% loads the data struct (if not provided)
if (nargin == 1)
    iData = getappdata(handles.figFlyTrack,'iData');
end

% checks to see only the solution has
if (isempty(iData.I))
    isSoln = true;
else
    isSoln = false;
...
% disables all the analysis buttons, menu items, display marker checkboxes
% and ROI frame objects
setAnalysisEnable(handles,'off')
setMenuEnable(handles,'off')
setDispEnable(handles,'off',1:7,0)
setMeshEnable(handles,'off')

% updates the ImageSeg objects based on their
switch (iData.Status)
    case (-1) % program is initialised (i.e., no image loaded)
        setFrmEnable(handles,'off')
        setObjEnable(handles.menuImageHist,'off')
    case (0) % nothing is set, so only enable the set mesh button
        setObjEnable(handles.menuImageHist,'on')
        setMeshEnable(handles,'on',1:3)
    otherwise
        % otherwise, enable the analysis menu item
        setObjEnable(handles.menuImageHist,'on')
        for i = 1:iData.Status
            switch (i)
                case (1) % case is the mesh has been set
                    % enable all mesh objects, the outline analysis button
                    % and the save solution, program default and
                    % segmentation parmeter menu items
                    setMenuEnable(handles,'on',1)
                    setAnalysisEnable(handles,'on',1)                    
                    if (isSoln)
                        % if only the solution is loaded, then disable the
                        % global radio button
                        setDispEnable(handles,'on',2)
                        setMeshEnable(handles,'on',[1:2 5])
                    else
                        % if both are loaded, then enable both buttons
                        setDispEnable(handles,'on',1)
                        setMeshEnable(handles,'on')
                    end
                case (2) % case is the outlines have been calculated
                    % enable the growth cone outline button and enable the
                    % growth cone bearing analysis button
                    setDispEnable(handles,'on',3)
                    setMenuEnable(handles,'on',[2 5])
                    setAnalysisEnable(handles,'on',2)
                case (3) % case is the axon bearings have been set
                    % enable the axon bearing check box and axon rotation
                    % menu item
                    setDispEnable(handles,'on',6)
                    setMenuEnable(handles,'on',3)
                    setAnalysisEnable(handles,'on',3)                    
                    if (~isSoln)
                        setMenuEnable(handles,'on',4)                        
                    end
                case (4) % case is the growth cone centres have been set
                    % growth cone centre has been calculated, so enable the
                    % growth cone center check box
                    setDispEnable(handles,'on',5)
                    setMenuEnable(handles,'on',[6 7])
            end
        end
end

% check to see if the pippete location has been set
iSoln = getappdata(handles.figFlyTrack,'iSoln');
if (isstruct(iSoln))
    if (~isempty(iSoln.PP))
        % calculates the local frame index
        cFrmL = calcLocalFrame(handles.frmCountEdit,iData);
        if (~isempty(iSoln.PP(cFrmL).pPos))
            % if so, enable the save axon file menu item            
            if (~isSoln)
                % if the solution is set, then enable the target location
                % check
                setDispEnable(handles,'on',7)
            end
        end
    else
        % otherwise, reset the target location check box
        setDispEnable(handles,'off',7,true)
    end
end

% if the image stack has been loaded, then enable the image shift
if ~isempty(iData.I)
    setMenuEnable(handles,'on',8)
end
