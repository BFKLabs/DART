...
if (chkEditValue(nwVal,nwLim,1))
    ...
    if (eval(pStr) ~= nwVal)
        ...
        set(handles.buttonSplitAxis,'enable','on')
...
set(handles.buttonCombine,'enable','off')
...
% sets the sub-index 
[nReg,eStr] = deal(size(sPara.pos,1),{'off','on'});
set(hGUI.textSubInd,'visible',eStr{1+(nReg>1)})
set(hGUI.popupSubInd,'visible',eStr{1+(nReg>1)})
...
if (nReg > 1)
    ...
    set(hGUI.menuSaveSubConfig,'enable','on')
    ...
    set(hGUI.menuSaveSubConfig,'enable','off')
    hPanel = findall(hGUI.panelPlot,'tag','subPanel');
    if (~isempty(hPanel)); delete(hPanel); end
end
...
set(hGUI.figFlyAnalysis,'visible','on')
...
set(findobj(hLine,'tag','end point 2'),'visible','off')
...
if ((hasCtrl) && (size(sPara.pos,1) > 1))    
    ...
    if (any(ii))
        ...
        set(handles.buttonCombine,'enable',vStr{1+(sum(ii)>1)})
    else
...
set(handles.buttonSplitAxis,'enable','off')
set(handles.buttonCombine,'enable','off')
set(handles.buttonClear,'enable',eStr{1+(size(sPara.pos,1)>1)})
...
set(hGUI.menuFile,'enable',state)
set(hGUI.menuPlot,'enable',state)
set(hGUI.menuGlobal,'enable',state)
...
if (strcmp(state,'off'))
    ...
    if (~isempty(hPara)); set(hPara,'visible',state); end    
end
...
if (nargin == 2)
    % makes the GUI invisible
    set(handles.figSplitPlot,'visible','off'); pause(0.05);
...
set(handles.tableAxisDim,'position',[X0 2.5*Y0+2*bPos(4) (pPosS(3)-2*X0) HT],'Data',Data)
...
if (nargin == 2)
    ...
    set(handles.figSplitPlot,'visible','on'); pause(0.05);
end
