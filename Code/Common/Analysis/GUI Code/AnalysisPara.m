...
set(hGUI.buttonUpdateFigure,'enable','off');
...
set(hGUI.buttonUpdateFigure,'enable','on');
pause(0.05);
...
if (Value.isComb)
    set(findobj(hP,'tag','nRow'),'string','1','enable','off')
    set(findobj(hP,'tag','nCol'),'string','1','enable','off')    
else
    set(findobj(hP,'tag','nRow'),'string',num2str(Value.nRow),'enable','on')
    set(findobj(hP,'tag','nCol'),'string',num2str(Value.nCol),'enable','on')
end
...
set(hGUI.listPlotFunc,'enable','inactive'); 
set(hGUI.popupPlotType,'enable','inactive'); 
set(hGUI.popupExptIndex,'enable','inactive'); 
...
% disables the listboxes
set(hGUI.listPlotFunc,'enable','on'); 
set(hGUI.popupPlotType,'enable','on'); 
set(hGUI.popupExptIndex,'enable','on'); 
...
%     set(handles.figAnalysisPara,'visible','off'); pause(0.05);    
...
if (strcmp(get(handles.figAnalysisPara,'visible'),'off'))
    set(handles.figAnalysisPara,'visible','on'); pause(0.05);
end
...
for i = 1:2
    if (isempty(hObj{i}))
         if (i == 1)
            ...
            set(handles.panelCalcPara,'visible','off');
        else
            % deletes the subplot panel
            set(handles.panelPlotPara,'visible','off');            
        end
    else
...
if (isempty(hObj{3}))
    ...
    set(handles.panelTimePara,'visible','off');
    set(handles.panelSubPara,'visible','off'); 
    set(handles.panelStimResPara,'visible','off'); 
else
...
if (nargin == 1)
    set(handles.figAnalysisPara,'visible','on')
end
resetObjPos(hFig,'height',Hfig) 
    ...
    for i = 1:length(p)
        if (~isempty(p(i).Enable))                        
            ...
            if (~isempty(isOn))
                ...
                set([hObj;hText],'enable',eStr{1+isOn});                          
            end
        end
    end
                    ...
                    switch (pType)
                        ...
                        case (2)
                            ...
                            lStr = {'Sleep Bouts','Sleep Duration',...
                                    ...
                                    'Response Amplitude','Inactivation Time Constant',...
                                    'Pre-Stim Avg Speed','Post-Stim Avg Speed',...
                            ...
                            hasStim = ~all(cellfun(@(x)(all(cellfun(@isempty,x))),field2cell(snTotT,'Ts')));
                            if (~hasStim || strcmp(gPara.movType,'Midline Crossing'))
                                lStr = lStr(1:4);
                            end
...
% makes the GUI invisible
if (nargin == 3)
    set(hPara,'visible','off'); pause(0.01);
end
