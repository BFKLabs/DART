...
set(hGUI.figFlyAnalysis,'visible','off')
...
set(handles.figOutputData,'visible','off'); pause(0.1);
...
set(handles.figOutputData,'visible','on'); pause(0.1);

% --- callback function for editing the apparatus inclusion checks --- %
function callbackApparatus(hObject,eventdata,handles)
...
if any(iData.appOut)
    set(handles.buttonOutput,'enable','on')
else
    set(handles.buttonOutput,'enable','off')
end
...
if (isa(eventdata,'char'))
    ...
    set(hMet,'enable','off','value',1)
    set(hType,'enable','off')        
...
h = waitbarFig({'Outputting Data To File...'},'Saving Analysis Data'); pause(0.5);
pStr = [pStr,');']; eval(pStr);

% updates and closes the data file
waitbarFig(1,'Data Output Complete!',1,h); pause(0.5);
close(h)
...
set(hBut{1},'enable',eStr{1+(iData.cInd>1)})
set(hBut{2},'enable',eStr{1+(iData.cInd<length(iData.nOut))})
...
if (isempty(iData.appName))
    ...
    tabFcn = @(hObj,e)OutputData('callbackApparatus',hObj,e,handles); 
...
if (any(cellfun(@(x)(any(strcmp(x,pData.Name))),pStr)))
    set(findobj(hApp,'style','radiobutton'),'enable','off')
end

% resets the locations of the other objects
dY = Bnew - appPos(4);
resetObjPos(hPanel,'bottom',dY,1);
set(handles.figOutputData,'visible','on')
