...
% Last Modified by GUIDE v2.5 05-Nov-2017 02:46:18
...
if (fIndex == 0)
    ...
    if (~all(diff(indFile) == 1))
        ...
        smFile = fullfile(fDir,getSummFileName(fDir));
        if (~exist(smFile,'file'))
            % if the summary file doesn't exist, then exit
            eStr = 'Error! Associated video solution summary file is missing.';
...
[snTot,iMov] = combineSolnFiles(sName,smFile);
if (isempty(snTot))
    % if the user cancelled, then exit the function
...
% prompts the user for the solution file directory
[fName,fDir,fIndex] = uigetfile({'*.ssol;','Single Experimental Solution Files (*.ssol)'},...
...
if (fIndex == 0)
    ...
    if (iscell(fName))
        ...
        if (~ok)
            ...
            [sNameNw,sNameF] = deal({[getFileName(sNameF),'.ssol']},{sNameF});
            setappdata(handles.figFlyAnalysis,'sName',sNameNw)
        end        
    end
end
...
% prompts the user for the solution file directory
[fName,fDir,fIndex] = uigetfile({'*.msol;','Multi-Experimental Solution File (*.msol)'},...
                            'Set The Multi-Solution File',dDir);
if (fIndex == 0)
    % if the user cancelled, then exit
    return
else
...
% prompts the user for the solution file directory
[fName,fDir,fIndex] = uigetfile({'*.spp','Subplot Configuration File (*.spp)'},...
                        'Select The Subplot Configuration File',dDir);
if (fIndex == 0)
    % if the user cancelled, then exit
    return
else
    % makes the figure invisible
    set(handles.figFlyAnalysis,'visible','off'); pause(0.05);
    ...
    set(handles.textSubInd,'visible','on')
    ...
    set(handles.menuSaveSubConfig,'enable','on')
    set(handles.figFlyAnalysis,'visible','on');     
end
...
qStr = {'This action will clear any currently calculated data and can''t be undone.';...
        '';'Are you sure you wish to continue?'};
uChoice = questdlg(qStr,'Continue Temporary Data Load?','Yes','No','Yes');
if (~strcmp(uChoice,'Yes')); return; end

% prompts the user for the solution file directory
[tName,tDir,fIndex] = uigetfile({'*.tdat;','Temporary Data File (*.tdat)'},...
                                 'Temporary Data File File',dDir);
if (fIndex == 0)
    % if the user cancelled, then exit
    return
else    
...
set(handles.menuSaveTempData,'enable','on')
...
[fName, fDir, fIndex] = uiputfile( ...
        {'*.csv','CSV-file (*.csv)';'*.mat','MAT-file (*.mat)'}, ...        
         'Set The Stimuli Data Output File',dFile);
if (fIndex == 0)
    % if the user cancelled, then exit the function
    return
else
...
% 
Stim = field2cell(field2cell(snTot,'iExpt',1),'Stim');
[Ts,Tf] = field2cell(snTot,{'Ts','Tf'});
Ts = cellfun(@(x)(cell2mat(x(~cellfun(@isempty,x)))),Ts,'un',0);
Tf = cellfun(@(x)(cell2mat(x(~cellfun(@isempty,x)))),Tf,'un',0);

% removes the empty stimuli data arrays
ii = ~cellfun(@isempty,Stim);
if (any(~ii))
    ...
    mStr = sprintf('The following experiments have missing stimuli data:\n\n');    
    for i = 1:length(exptEmpty)
        mStr = sprintf('%s => %s\n',mStr,exptEmpty{i});
    end    
    mStr = sprintf(['%s\nRe-check the relevant solution files to ensure ',...
                    'have been created correctly.'],mStr);
...
% removes the empty stimuli data arrays
[Stim,Ts,Tf,exptName] = deal(Stim(ii),Ts(ii),Tf(ii),exptName(ii));

%
switch (fExtn)
    case ('.mat')
        % saves the stimuli data to file
        save(fFile,'exptName','Stim','Ts','Tf');
    case ('.csv')
        % saves the csv stimuli data to file
        saveCSVStimData(fFile,Stim,Ts,Tf,exptName);
end
...
% prompts the user for the solution file directory
[fName,fDir,fIndex] = uiputfile({'*.spp','Subplot Configuration File (*.spp)'},...
                        'Set The Subplot Configuration Output File',dDir);
if (fIndex == 0)
    % if the user cancelled, then exit
    return
else
...
% prompts the user for the solution file directory
[tName,tDir,fIndex] = uiputfile({'*.tdat;','Temporary Data File (*.tdat)'},...
                                 'Temporary Data File File',dDir);
if (fIndex == 0)
    % if the user cancelled, then exit
    return
...
sData.fName = getFinalDirString(getappdata(handles.figFlyAnalysis,'fNameFull'));
sData.sName = getappdata(handles.figFlyAnalysis,'sName');

% creates the load bar
h = ProgressLoadbar('Outputting Temporarily Calculated Data To File...');

% saves the data to file
save(tName,'plotD','pData','gPara','sPara','sData');
...
iData = getappdata(handles.figFlyAnalysis,'iData');
[iData.ProgDef,isSave] = ProgParaAnalysis(handles.figFlyAnalysis,iData.ProgDef);
...
if (~isa(eventdata,'char'))
    uChoice = questdlg('Are you sure you want to clear all the locally stored analysis data?',...
                       'Clear Analysis Data?','Yes','No','Yes');
    if (strcmp(uChoice,'Yes'))
        resetAll = true;        
    else
        return        
...
set(handles.menuSaveTempData,'enable','off'); 
...
uChoice = questdlg('Are you sure you want to reset the Global Parameters?',...
...
if (strcmp(get(hObject,'state'),'on'))
    set(datacursormode(gcf),'DisplayStyle','window','enable','on')    
else
...
[sInd,eStr] = deal(str2double(lStr{iStr}),{'off','on'});
...
if ((~isempty(sPara.pData{sInd})) && ~any(isnan(sPara.ind(sInd,:))))
    ...
    [lStr,fName] = deal(get(handles.listPlotFunc,'string'),sPara.pData{sInd}.Name);
    ...
    set(handles.menuUndock,'enable','on')
    set(handles.menuSaveData,'enable','on')
    set(handles.buttonUpdateFigure,'enable','on')
    ...
    set(handles.menuSaveData,'enable','off')
    set(handles.listPlotFunc,'value',1)  
    try; set(hPara,'visible','off'); end
    
    % enables the undocking menu item
    if (size(sPara.pos,1) == 1)
        set(handles.menuUndock,'enable','off')
    else
        set(handles.menuUndock,'enable',eStr{1+any(~isnan(sPara.ind(:)))})        
    end      
    
    % disables the update button
    gCol = (240/255)*[1 1 1];
    set(handles.buttonUpdateFigure,'enable','off','BackgroundColor',gCol)
end
...
if (get(hObject,'value'))    
    ...
    set(handles.listExptSel,'value',1,'string','Multi Experiment',...
                    'Position',[lPos(1) B0 lPos(3) H0],'enable','inactive',...
                    'Style','popupmenu')     
    ...
    if (~isa(eventdata,'char'))
        if (any(xor(iData.indExpt,indExpt0)))
            ...
            uChoice = questdlg('Are you sure you want to reset the experiment list?',...
                       'Reset Experiment List?','Yes','No','Yes');
            if (strcmp(uChoice,'Yes'))
                % resets the plot data struct
                menuResetData_Callback(handles.menuResetData, 'Multi', handles)             
            else
...
% the list box
switch (lStr{indNw})
    case ('Multi-Experiment Analysis') % case is multi-solution
        A = pDataT.Multi;
        eStr = 'inactive';
    case ('Experiment Analysis (Population)') % case is single-solution population analysis
        A = pDataT.Pop;
    case ('Experiment Analysis (Individual)') % case is single-solution individual analysis
        A = pDataT.Indiv;      
end
...
if (isempty(A))
    ...
    set(handles.listPlotFunc,'string',lStrNw,'enable','on'); pause(0.1);        
    
    % sets the function stack into the GUI
    setappdata(handles.figFlyAnalysis,'fcnStack',[{' '},field2cell(A,'fcn')]);        
end
...
set(handles.popupExptIndex,'enable',eStr)
listPlotFunc_Callback(handles.listPlotFunc, '1', handles)
...
if (isempty(fIndNwT))
    ...
    if (isempty(pData{pInd}{abs(fIndNwT),eInd}.fDesc))
        ...
        set(handles.textFuncDesc,'string','<No Function Description Given>',...
                    'enable',eStr{1+(sign(fIndNwT) > 0)});
    else
        % otherwise, set the function description
        set(handles.textFuncDesc,'string',pData{pInd}{abs(fIndNwT),eInd}.fDesc,...
                    'enable',eStr{1+(sign(fIndNwT) > 0)});
    end        
end
...
if (isShowPara)
    ...
    resetRecalcObjProps(handles,'No')
    set(handles.buttonUpdateFigure,'enable','off')        
...
if (all([eInd,fInd,pInd] > 0))
    ...
    if (~isempty(plotD{pInd}{fInd,eInd}))               
        ...
        set(handles.menuUndock,'enable','on')
        set(handles.menuClearPlot,'enable','on')
        set(handles.menuZoom,'enable','on')                
        set(handles.menuDataCursor,'enable','on')                        
        if (isShowPara); set(hPara,'visible','on'); end
        ...
        set(handles.menuZoom,'enable','off')                
        set(handles.menuDataCursor,'enable','off')   
...
if (isShowPara); set(hPara,'visible','on'); end
set(handles.menuSaveData,'enable','off')
set(handles.menuClearPlot,'enable','off')
...
set(handles.menuUndock,'enable','on')
set(handles.menuClearPlot,'enable','on')
set(handles.menuResetData,'enable','on'); 
set(handles.menuSaveTempData,'enable','on'); 
...
if (isempty(ProgDefNew))
    ...
    if (~isfield(ProgDefNew,'TempData'))
        ...
        nwDir = fullfile(fileparts(ProgDefNew.TempFile),'3 - Temporary Data');    
        if (~exist(nwDir,'dir'))
...
set(handles.listPlotFunc,'value',[],'visible','off')

% makes the panel description object invisible
set(handles.textFuncDesc,'visible','off')
set(handles.textFuncDescBack,'visible','off')

% disables the save menu item
set(handles.menuOpenTempData,'enable','off')
set(handles.menuSaveData,'enable','off')
set(handles.menuSaveStim,'enable','off')
set(handles.menuSaveTempData,'enable','off')
set(handles.menuSaveSubConfig,'enable','off')
set(handles.menuClearPlot,'enable','off')
set(handles.menuUndock,'enable','off')
set(handles.menuResetData,'enable','off')
set(handles.menuSplitPlot,'enable','off')
...
switch (sType)
    case ('Experiment') % case is a single experiment solution file(s)        
        if (length(snTot) == 1)
            ...
            set(handles.textSolnCount,'string',num2str(length(snTot(eInd).T)))
        else
            ind = 1:length(snTot);
...
appPara.Name = cellfun(@(x)(sprintf('Apparatus #%i',x)),...
...
set(handles.menuZoom,'enable','off') 
set(handles.menuDataCursor,'enable','off') 
set(handles.menuGlobal,'enable','on') 
set(handles.menuSplitPlot,'enable','on')
set(handles.menuSaveData,'enable','off')
set(handles.menuSaveTempData,'enable','off')
set(handles.menuSaveSubConfig,'enable','off')
set(handles.menuOpenSubConfig,'enable','on')
set(handles.menuOpenTempData,'enable','on')

% removes any existing panels/plots
hPanel = findall(handles.panelPlot,'tag','subPanel');
if (~isempty(hPanel)); delete(hPanel); end    
set(handles.popupSubInd,'visible','off'); 
set(handles.textSubInd,'visible','off')
...
if (pInd == 3)
    ...
    set(handles.toggleExptSel,'visible','on','value',0)
    set(handles.listExptSel,'visible','on')
    ...
    set(handles.listExptSel,'visible','off'); 
    set(handles.toggleExptSel,'visible','off'); pause(0.05);        
    ...
    if (isMulti)
        ...
        set(hPopup,'string',sName,'enable','on')
        setSolnInfo(handles,'Experiment')
    else
        % sets the table strings but makes the popup menu inactive
        set(hPopup,'string',sName,'enable','inactive','value',1)
...
if (isfield(snTot(eInd),'iMov'))
    ...
    [nFly,nApp] = deal(max(getFlyCount(snTot(eInd).iMov)),length(snTot(eInd).Px));
else
...
if (pInd == 3)
    % sets the stimuli count string
    nStim = sum(cellfun(@(x)(sum(~cellfun(@isempty,x))),field2cell(snTot,'Ts')));
    if (nStim == 0)
        % no stimuli events
        stimStr = 'Not Applicable';
        set(handles.menuSaveStim,'enable','off')
    else
        % otherwise, set the number of stimuli events
        stimStr = num2str(nStim);
        set(handles.menuSaveStim,'enable','on')
    end
    ...
    set(handles.textStimCount,'string',stimStr)
    
    % calculates the sum of the experiment durations    
    iData = getappdata(hFig,'iData'); ii = iData.indExpt;
    ...
    if (length(snTotNw.iExpt) > 1)
        ...
        if (any(cellfun(@(x)(strContains(x,'Stim')),Type)))
            ...
            stimStr = num2str(sum(cellfun(@length,snTot.Ts)));
            set(handles.menuSaveStim,'enable','on')
        else
        ...
        switch (snTotNw.iExpt.Info.Type)
            ...
            case {'RecordStim','StimRecord'}
                ...
                stimStr = num2str(length(cell2mat(snTot(eInd).Ts)));
                set(handles.menuSaveStim,'enable','on')
            case ('RTTrack')
...
% enables the listboxes/popup menus
set(handles.listPlotFunc,'enable',state); 
set(handles.popupPlotType,'enable',state); 
...
if (nargin == 2)
    if (strcmp(state,'on'))
        ...
        if (pInd ~= 3)        
            set(handles.popupExptIndex,'enable','on'); 
        end
    else
        set(handles.popupExptIndex,'enable',state); 
    end
end
