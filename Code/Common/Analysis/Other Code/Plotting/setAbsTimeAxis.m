...
[lMax,hMin,dumT,xL,iStep] = deal(6,6,clock,get(hAx,'xlim'),1);
...
switch (nargin)
    case {2,4}
        Ts = deal(0);
    otherwise
        % calculates the start time of the experiment (in relation to the start
        % of the day, tDayH) which is given in seconds)
        if (~isempty(snTot))
...
for i = 1:length(xTick)
    if (isHours)
        tLblStr{i} = datestr([dumT(1:2) sec2vec(Ts+tTick(i)+tDayH)],'HHPM');
    else
        tLblStr{i} = datestr([dumT(1:2) sec2vec(Ts+tTick(i)+tDayH)],'HH:MMPM');
    end        
end
