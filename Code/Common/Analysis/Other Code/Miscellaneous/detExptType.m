...
if (length(snTot) > 1)
    % multiple solution files. all solutions must have min of 1 stim event
    Ts = field2cell(snTot,'Ts');
    isStim = any(cellfun(@(x)(~isempty(cell2mat(x))),Ts));
else
    % one solution file
    isStim = ~isempty(cell2mat(snTot.Ts));
end
