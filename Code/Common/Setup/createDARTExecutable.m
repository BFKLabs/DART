...
if (isempty(fDir))
    ...
    eStr = 'Error! No analysis functions were selected. Unable to create executable...';
    ...
    if (any(~isDef))                
        ...
        cpyFile = cellfun(@(x,y)(fullfile(x,y)),fDir(~isDef),fName(~isDef),'un',0);
...
uChoice = questdlg({'Which type of executable do you wish to create:';'';...
                    ' * Console Application (creates command window on opening)';...
                    ' * Windows Standalone Application (no command window on opening)'},...
...
toolStr = '-N -p daq -p imaq -p images -p instrument -p optim -p stats -p curvefit';
...
for i = 1:length(fStr)
    switch (fStr{i})
        case ('DART.fig')
            addStr = sprintf('%s -a ''%s''',addStr,fullfile(progDir,fStr{i}));
        case ('Para Files')
            addStr = sprintf('%s -a ''%s''',addStr,fullfile(progDir,fStr{i})); 
        otherwise
