...
for i = 1:nApp
    ...
    if (~isempty(h))
        ...
        if (waitbarFig(wOfs+1,wStrNw,i/nApp,h))
            ok = false; return;
        end
    end
    ...
    if (iMov.ok(i))
        ...
        for j = 1:nTube    
            ...
            if (~isempty(h))
                ...
                if (waitbarFig(wOfs+2,wStrNw,j/nTube,h))
                    ok = false; return
                end
            end            
...
% global variables
global wOfs
waitbarFig(wOfs+3,'Determining Correct Orientation Angles',0,h);

%
[axR,fPos,Phi] = deal(pData.axR{iApp}{iFly},pData.fPosL{iApp}{iFly},Phi*pi/180);
...
if (all(B == 0))
    ...
    for i = 1:length(jGrp)     
        ...
        if (waitbarFig(wOfs+3,wStrNw,i/length(jGrp),h))
            PhiBF = [];
            return
        end
