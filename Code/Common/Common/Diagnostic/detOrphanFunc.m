...
if (nargin == 2)   
    ...
    wStr1 = cellfun(@(x)(sprintf('Level %i',x)),num2cell(2:nLevelMx)','un',0);
    wStr0 = [{'Determining M-File/Figure Names'};wStr1];
    h = waitbarFig(wStr0,'Function Dependency Determination'); pause(0.1);
...
if (nLevel == 0)
    ...
    waitbarFig(j,sprintf('Level %i',j),0,h);
end
