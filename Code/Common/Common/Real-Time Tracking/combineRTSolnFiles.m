...
if (nargin == 1)
    ...
    h = waitbarFig(wStr,'Solution File Output',false);
    pause(0.05);
end
...
for i = 1:nSoln
    ...
    if (nargin == 1)
        ...
        waitbarFig(1,wStrNw,i/(nSoln+1),h);            
    end
    ...
    [pData.nApp,pData.nTube] = deal(length(iMov.iR),getFlyCount(iMov));
...
if (nargin == 1)    
    waitbarFig(1,'Solution File Output Complete!',1,h);            
    pause(0.05); close(h)
end
...
switch (pStr)
    ...
    case ('exP') % case is the experimental info data struct
        p = struct('FPS',[],'sFac',[],'nTubeR',NaN,'nTubeC',NaN);        
end