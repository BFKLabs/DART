...
set(hObject,'visible','on','position',[Lnw Bnw figPos(3:4)]); 
...
if (strcmp(get(hObject,'label'),'Display Criteria Markers'))
    ...
    set(hCrit,'visible',vStr{1+(~isCheck)})    
else
    ...
    set(hExLoc,'visible',vStr{1+(~isCheck)})        
end
...
if (getappdata(handles.figTrackStats,'isC2A'))
    ...
    set(hCrit,'visible',vStr{1+isCheck},'yData',yy([1 1 2 2]))
    set(handles.menuDispCrit,'enable',vStr{1+(~isnan(yData))})
end
...
pPosNw = [(X0+(X0+dPos(3))*isChk(1)),Y0+max(0,isChk(1)*dPos(4)-pPos(4)),pPos(3:4)];

% updates the position of the 
set(handles.panelTrackStats,'visible',eStr{1+isChk(1)},'position',dPosNw)
set(handles.panelAxisPlot,'visible',eStr{1+isChk(2)},'position',pPosNw)
...
nFlyR = getFlyCount(iMov,1);
[aok,Stim,eStr] = deal(find(iMov.ok),rtP.Stim,{'off','inactive'});
[nApp,nFly,vStr] = deal(length(aok),max(nFlyR),{'off','on'});
...
elseif (isC2A)
    ...
    set(handles.menuDispCrit,'enable',vStr{1+rtP.popSC.isVtol})
elseif (isExLoc)
...
[nApp,nFlyR] = deal(length(aok),getFlyCount(iMov,1));
