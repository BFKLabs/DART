...
try
    if (nargin < 4)  
        ...
        h = waitbarFig(wStr,'Loading Movie Positions');
    else
        wStr = getappdata(h,'wStr');
        wOfs = 1 + (length(wStr) > 2);
    end
catch
...
for i = 1:nFile
    ...
    wStrNw = sprintf('Loading Movie Positions (Apparatus %i of %i)',i,nFile);
    if (waitbarFig(1+wOfs,wStrNw,i/(nFile+1),h))
    ...
    try
        ...
        if (strContains(ME.message,'Out of memory'))
            % updates the waitbar figure
            wStrNw = sprintf('Retry Loading Movie Positions (Apparatus %i of %i)',i,nFile);
            waitbarFig(1+wOfs,wStrNw,i/(nFile+1),h)
...
if (nargin < 4)
    ...
    waitbarFig(1+wOfs,'Movie Position Loading Complete!',1,h);
end   
