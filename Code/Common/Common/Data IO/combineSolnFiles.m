...
function [snTot,iMov] = combineSolnFiles(varargin)
...
% sets the solution/summary file names
if (nargin == 1)
    % loads the summary file and sets the solution file directory
    movDir = varargin{1};
    smFile = fullfile(movDir,getSummFileName(movDir));    
    if (exist(smFile,'file'))
        % loads the summary file
        smData = load(smFile);
        
        % sets the solution file names from the base video name
        bName = sprintf('*%s*',smData.iExpt.Info.BaseName);
        sFile = dir(fullfile(movDir,bName));
    else
        % sets the solution file names from the wild-card search
        sFile = dir(fullfile(movDir,'*.soln'));
    end        

    % sets the solution file names to be loaded    
    if (isempty(sFile))
        % if there are no valid solution files, then output an error
        eStr = 'The selected directory does not contain any valid solution files.';
        ...
        % otherwise, set the solution file names
        sName = cellfun(@(x)(fullfile(movDir,x)),field2cell(sFile,'name'),'un',0);
    end
else
    % otherwise, set the input arguments    
    [sName,smFile] = deal(varargin{1},varargin{2});
    smData = load(smFile);
end
...
if (exist(smFile,'file'))
    ...
    if (any(allNaN))        
        ...
        for i = reshape(find(allNaN),1,sum(allNaN))
            ...
            elseif (allNaN(i-1))
                ...
                T0(i) = T0(i-1) + length(smData.tStampV{i-1})/iExpt.Video.FPS + Tp;
            else
    ...
    T0 = cellfun(@(x)(x(1)),tStampV(1:min(length(sName),length(tStampV))));
    Tf = cellfun(@(x)(x(end)),tStampV(1:min(length(sName),length(tStampV))));
    ...
    % searches the stimuli parameter struct to determine if there are any
    % random signal parameters set. is so, neglect the time stamps
    ii = cellfun(@isempty,field2cell(smData.iStim,'iPara'));
    if (all(ii) || (strcmp(iExpt.Info.Type,'RTTrack')))
        % circadian rhythm expt, so no stimuli
        hasRand = false;
    else
        % otherwise, determine if there were any random components
        smData.iStim.iPara = smData.iStim.iPara(~ii);
        iPara = getStructFields(smData.iStim.iPara{1},[],1);
        hasRand = any(field2cell(iPara,'isRand',1));    
    end
    
    % sets the stimulus time-stamp array
    if (~isempty(smData.tStampS) && ~hasRand)                
    ...
    T0expt = tStampV{1}(1);
    T0 = datevec(addtodate(datenum(iExpt.Timing.T0),floor(T0expt),'second'));                    
    if (i0 ~= 0)
...
elseif (strcmp(iExpt.Info.Type,'RecordOnly') && (~isempty(tStampS)))
    % case is the experiment originally set as a record experiment, but has
    % had the stimuli time stamps added in through SyncSummary
    iExpt.Info.Type = 'RecordStim';
end
...
snTot = orderfields(struct('T',[],'Ts',[],'Tf',[],'Px',[],'Py',[],'Phi',[],'AxR',[],...
                           'isDay',[],'sgP',sgP,'iExpt',iExpt,'appPara',[],...
                           'pMapPx',[],'pMapPy',[],'pMapPhi',[],'pMapAxR',[],...
                           'iMov',[],'Type',1));           
           
% sub-struct memory allocation           
[snTot.T,snTot.Ts,snTot.Tf,Px,Py] = deal(cell(nFile,1)); 
...
if (~isempty(hh))
    ...
    try
        ...
        [h,wOfs] = deal(waitbarFig(wStr,'Combining Solution Files'),0); 
    end
else
...
for i = 1:nFile
    ...
    if (waitbarFig(1+wOfs,wStrNw,0.5*(i/nFile),h))
        [snTot,iMov] = deal([]);
        return        
    end        
    ...
    if (isempty(A.pData))            
        ...
        ii = find(cellfun(@(x)(strContains(x,vFile)),sName));
        ...
        if (i == 1)
            ...
            sgP = struct('sRate',a.iMov.sRate,'fRate',a.exP.FPS,'sFac',a.exP.sFac);
        ...
        for j = 1:nApp                         
            ...
            Px{i}{j} = cell2mat(cellfun(@(x)(x(:,1)*sgP.sFac),...
                            fPos{j},'un',0));
            Py{i}{j} = cell2mat(cellfun(@(x)((x(:,2)+yOfs(j))*sgP.sFac),...
                                fPos{j},'un',0));      
...
if (~isempty(T0))
    for i = 1:nFile
        ...
        if (waitbarFig(1+wOfs,wStrNw,0.5*(1+(i/nFile)),h))
            [snTot,iMov] = deal([]);
            return        
        end
        
        if (isOK(i))
            % sets the day/night boolean flags
            TT = cellfun(@(x)(addtodate(datenum(T0),...
                            roundP(x,1),'second')),num2cell(snTot.T{i}));
            TTv = datevec(TT);
            snTot.isDay{i} = (TTv(:,4) >= hDay) & (TTv(:,4) < (hDay + 12));            
            
            % sets the stimuli time stamps for the current video
            if ((~hasRand) && (~isempty(tStampS)))
                if (i == find(isOK,1,'last'))
                    indStim = (tStampS > snTot.T{i}(1)) & (tStampS < snTot.T{i}(end));
                else
                    indStim = (tStampS > snTot.T{i}(1)) & (tStampS < snTot.T{i+1}(1));
                end
                                
                if (any(indStim))
                    % if there are any stimuli present within the movie,
                    % then set the start/finish times of the stimuli
                    snTot.Ts{i} = tStampS(indStim);
                    nTnw = length(snTot.Ts{i});
                    TsigNw = cellfun(@(x)(x(end)),iExpt.Stim(1).Tsig(indStim));
                    snTot.Tf{i} = snTot.Ts{i} + reshape(TsigNw,nTnw,1);
                end        
            end
        end
    end
...
if (~isfield(iMov,'flyok'))
    iMov.flyok = true(max(getFlyCount(iMov,1)),nApp);
end
...
elseif (any(~isOK))
    ...
    [snTot.T,snTot.Ts] = deal(snTot.T(isOK),snTot.Ts(isOK));
    [snTot.Tf,snTot.isDay] = deal(snTot.Tf(isOK),snTot.isDay(isOK));
...
if (~isempty(iMov))
    if (isfield(iMov,'ok'))        
        ...
        % sets the position statistics struct (if it exists)
        if (isfield(iMov,'pStats'))
            iMov.pStats = iMov.pStats(ok0,:);
        end
...
% --- 
function iVid = getVideoFileIndex(fName)

% determines the index of the 
[~,AA,~] = fileparts(fName);
AR = regexp(AA,'\D','split');
[ARmx,imx] = max(cellfun(@length,AR));

% if there is no valid index, then exit the function
if (ARmx == 0)
    % if there are no valid solution files, then output an error
    eStr = sprintf(['The following file does not have a valid index:',...
                    '\n\n  => "%s.soln"'],fName);
    waitfor(errordlg(eStr,'Invalid Solution Files','modal'))

    % returns an empty array
    iVid = NaN;        
else
    iVid = str2double(AR{imx});
end    
