...
if (addHeader)
    ...
    DataH = {'Video Directory',NaN,bData.MovDir;
             ...
             'Start Time (min)',NaN,roundP(convertTime(pData.T(1),'sec','min'),0.01);
             'Final Time (min)',NaN,roundP(convertTime(pData.T(end),'sec','min'),0.01);
             'Apparatus Count',NaN,nApp;
             'Fly Count',NaN,nFly;
...
for i = 1:nApp
    ...
    VB{i} = Dnw./repmat(dTB{i},1,getAppFlyCount(iMov,i));
end
...
for i = 1:nApp
    ...
    if (addHeader)
        ...
        B = combineCellArrays({sprintf(...
                    'Apparatus #%i Avg Speed (mm/sec)',ind(i))},fStrH,0);
