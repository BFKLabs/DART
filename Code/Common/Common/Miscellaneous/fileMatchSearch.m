...
%     sub-directory, sDir --- %
function fMatch = fileMatchSearch(fData,sDir)

% initialisations
[p,tol,fMatch] = deal([1e10*ones(1,3) 3600 60 1],30,[]);
...
for i = 1:length(nwFiles)
    ...
    if (~(strcmp(nwFiles(i).name,'.') || strcmp(nwFiles(i).name,'..')))
        if (nwFiles(i).isdir)
            ...
            fMatch = fileMatchSearch(fData,fullfile(sDir,nwFiles(i).name));
            if (~isempty(fMatch))
                return;
            end
        else
            ...
            dDiff = abs(p.*(datevec(fData.datenum)-datevec(nwFiles(i).datenum)));
            if ((nwFiles(i).bytes == fData.bytes) && (sum(dDiff) <= tol))
                fMatch = nwFiles(i);
                fMatch.dir = sDir;
                return;
            end
