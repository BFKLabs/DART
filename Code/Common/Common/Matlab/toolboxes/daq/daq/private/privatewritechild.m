function privatewritechild(varargin)
%PRIVATEWRITECHILD Convert the channel or line to MATLAB Code.
%
...
%    $Revision: 1.14.2.7 $  $Date: 2009/12/31 18:44:52 $
...
if strcmp(syntax, 'named')
    ...
    if sum(strcmp(childname,''))
        throwAsCaller(MEXception('daq:privatewritechild:badtype',...
            'Object''s %ss must have %sName property values for the ''named'' SYNTAX.',type_child,type_child))
    else
...
for i = 1:length(child)
    ...
    i_child = feval(classname, structchild.uddobject(i));
    ...
    if i == 1
        ...
        case 'addline'
            ...
            childudd = feval('addchannel',parentudd,1);
            default_child = privateUDDToMATLAB(childudd);
        end
    ...
    case 'named'
        ...
        objname = allvalue.([type_child 'Name']);
...
while writtenLines < totalLines
    ...
    if ~isempty(hLine)
        hLine = [hLine{:}];
        validPorts = [validPorts [name num2str(portId) ';']]; %#ok<AGROW>
