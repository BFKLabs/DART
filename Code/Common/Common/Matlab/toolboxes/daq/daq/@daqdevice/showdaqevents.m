...
%    $Revision: 1.5.2.6 $  $Date: 2008/06/16 16:35:21 $
...
if isa(obj, 'daqdevice')
   ...
   try
      events = get( daqgetfield(obj,'uddobject'), 'EventLog');
   catch e
      error('daq:showdaqevents:unexpected', e.message);
   end
else
...
if nargin == 2
   if ischar(index) 
      ...
      index = find(strcmpi(index, {events.Type}));
   elseif iscell(index)
      % Initialize variables.
      tempIndex = [];
      for i = 1:length(index)
         % Loop through the specified events and find their indices.
         tempIndex = [tempIndex find(strcmpi(index{i}, {events.Type}))]; %#ok<AGROW>
      end
