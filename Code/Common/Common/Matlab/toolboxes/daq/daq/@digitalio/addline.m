...
%    length as HWLINE.  By default, lines are added from the first port.
% 
...
%    $Revision: 1.6.2.9 $  $Date: 2008/06/16 16:35:39 $
...
% Set the name of the channel property
s = struct(obj);
channelprop = s.info.child;

% Set the name of the channel name property
...
if isempty(port)
	port = -1; % daq will take care of assigning the ports
elseif port < 0 
    error('daq:addline:invalidport', 'An invalid PORT was specified.  PORT must be a non-negative number.');
end
...
for j = 1:length(port)
   for i = 1:length(hwline)
      try
         ...
         if port(j) == -1 
            uddlinei = addchannel(uddobj, hwline(i)); % No port specified.
         else   
            uddlinei = addchannel(uddobj, hwline(i), port(j));
         end
         linei = daqgate('privateUDDToMATLAB', uddlinei);
         line = [line; {linei}]; %#ok<AGROW>
         
         % Store what indices have been added
         lineindex = [lineindex get(uddlinei, 'Index')]; %#ok<AGROW>
         
         % Assign the direction.
         set(uddlinei, 'Direction', direction{1});
         ...
         if ~isempty(name) 
            set(uddlinei, channelnameprop, name{1});
            if length(name) > 1
               name(1) = [];
            end
         end
         ...
         error('daq:addline:unexpected', e.message);
      end
   end
end
