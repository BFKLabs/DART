...
%    boards associated with the National Instruments' Traditional analog input
%    object.  For 1 Mux board, 64 channels are available.  For 2
%    boards, 128 channels are available and for 4 boards 256 channels
%    are available.  This functionality is not available via the NIDAQmx 
%    interface.
%
...
%   $Revision: 1.5.2.7 $  $Date: 2008/08/08 12:50:39 $
...
if ~isa(object,'analoginput')
   error('daq:addmuxchannel:invalidobject', 'OBJECT must be a 1-by-1 analog input object.');
end

if ~strcmp(daqhwinfo(object,'AdaptorName'),'nidaq')
    if strcmp(daqhwinfo(object,'AdaptorName'),'nidaqmx')
        error('daq:addmuxchannel:invalidadaptor', 'The National Instrument NIDAQmx interface does not support multiplexers.');
    else    
        error('daq:addmuxchannel:invalidadaptor', 'ADDMUXCHANNEL only works with National Instrument objects using the Traditional interface.');
    end
end
