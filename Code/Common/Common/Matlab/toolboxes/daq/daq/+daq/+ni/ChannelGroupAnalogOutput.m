classdef (Hidden) ChannelGroupAnalogOutput < daq.ni.ChannelGroup
    %ChannelGroup Represents a NI-DAQmx task used for analog output
    ...
    methods
        function obj = ChannelGroupAnalogOutput(session,name)
            % Get the device name from the group name
            deviceID = sscanf(name,'ao/%s');
            obj@daq.ni.ChannelGroup(session,name,deviceID);

            obj.OutputBufferSize = 0;
            obj.IsSingleScanTiming = false;
            obj.AsyncIOChannel = daq.ni.AsyncIOOutputChannel(session, obj, 'mwnidaqmxao');
        end
        ...
        function startTrigger = getStartTriggerConfiguration(obj)
            ...
            if type == daq.ni.NIDAQmx.DAQmx_Val_None
                ...
                if status == daq.ni.NIDAQmx.DAQmxErrorCannotGetPropertyWhenTaskNotReservedCommittedOrRunnin ||...
                    ...
                    %default
                    startTrigger = [ '/', obj.getDeviceIDForSync() ,'/ao/StartTrigger'];
        ...
        function [scanClock] = getScanClockConfiguration(obj)
            ...
            if status==daq.ni.NIDAQmx.DAQmxErrorCannotGetPropertyWhenTaskNotReservedCommittedOrRunnin ||...
                ...
                if ~isempty(posQuestion)
                    terminal = [terminal(1:posQuestion-1) '/ao/SampleClock'];
                end
    ...
    methods(Hidden)
        ...
        function doConfigureScanClock(obj)
            ...
            % Some AO devices like USB-4331 have fixed update rates and
    ...
    methods (Hidden)
        function updateChannelMap(obj)
            updateChannelMapBase(obj,daq.internal.SubsystemType.AnalogOutput);
        end
    end
    ...
    methods (Access = protected)
        function resetImpl(obj)
            % Reset analog outputs to zero at daq.reset g699723
            [~,~,~] =...
