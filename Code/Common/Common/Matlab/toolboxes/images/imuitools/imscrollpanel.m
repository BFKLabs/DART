...
setappdata(hScrollpanel,'API',api);
    ...
    function replaceImage(varargin)
        ...
        [common_args,specific_args] = imageDisplayParseInputs(specificArgNames,varargin{:});
    ...
    function mag = validateInitialMagProperty(initMag)
        ...
        % case.
        mag = checkInitialMagnification(initMag,{'fit'},'replaceImage',...
    ...
    function scrollHorizontal(varargin)
        ...
        if ~isequal(hSliderHorPos,new_slider_pos)
            ...
            set(hAx,'Position', [xPos hAxesPos(2:4)]);
    ...
    function scrollVertical(varargin)
        ...
        if ~isequal(hSliderVerPos,new_slider_pos)
            ...
            hAxesPos = get(hAx, 'Position');
            yPos = findYPos;
            set(hAx,'Position',[hAxesPos(1) yPos hAxesPos(3:4)]);
    ...
    function resizeView(varargin)
        ...
        set(hAx,'Position', [xPos yPos hAxesPos(3:4)]);
...
imageWidth  = getImWidth(hIm);
imageHeight = getImHeight(hIm);
...
if ~isRSetImage(hIm) && (~isXDataDefault || ~isYDataDefault)  && ...
...
if isRSetImage(hIm)
    [~,imageHeight] = getSpatialDims(hIm);
    ydata = [1 imageHeight];
end
