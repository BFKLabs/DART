function data_streams = dicom_encode_attrs(attrs, txfr_syntax, uid_details)
...
[swap_file, swap_meta, swap_pixel] = determine_swap(txfr_syntax, uid_details);
...
% Group and Element
segment = dicom_typecast(uint16(attr.Group), 'uint8', swap);
segment = [segment dicom_typecast(uint16(attr.Element), 'uint8', swap)];
...
if ((len ~= dicom_undefined_length()) && (rem(len, 2) ~= 0))
    len = len + 1;
end
    
% Add the length and data to the segment.
segment = [segment dicom_typecast(len, 'uint8', swap)];

if (ischar(attr.Data))
    segment = [segment uint8(attr.Data(:)')];
else
    tmp = dicom_typecast(attr.Data, 'uint8', swap);
    segment = [segment tmp(:)'];
end

% Pad the data (if necessary) by adding a null byte.
if (rem(numel(segment), 2) ~= 0)
    segment(end + 1) = 0; %#ok<AGROW>
end



function [tf_file, tf_meta, tf_pixel] = determine_swap(txfr_syntax, uid_details)
...
tag = data(1:4);
tag = dicom_typecast(dicom_typecast(tag, 'uint16'), 'uint8', 1);

len = data(5:8);
len = dicom_typecast(dicom_typecast(len, 'uint32'), 'uint8', 1);
...
for p = 1:numel(struct_data)
    ...
    tmp = dicom_encode_attrs(struct_data(p), uid_details.Value, uid_details);
