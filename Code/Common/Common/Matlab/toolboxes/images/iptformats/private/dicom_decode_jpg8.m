...
if (fid < 3)
    error(message('images:dicom_decode_jpg8:CouldNotOpenFile', tmp_file))
end
...
if (count ~= length(comp_fragment))
    warning(message('images:dicom_decode_jpg8:dataTruncatedWhenWritingToTempFile'))
end
...
try
    ...
    warning(message('images:dicom_decode_jpg8:CouldNotDeleteTempFile', tmp_file))
end
