...
for sInd = 1:num_strels
    B = images.internal.morphmex(mex_method, B, getnhood(se(sInd)), getheight(se(sInd)), unpacked_M);
end
...
elseif strel_is_all_flat
    ...
    if useIPP3x3(A,se,input_is_2d,strel_is_single,strel_is_all_2d)
...
%==========================================================================
function TF = useIPP3x3(A,se,is2DInput,strelIsSingle,strelIsAll2D)
...
TF  = is3x3nhood && is2DInput && supportedType && strelIsAll2D ...
      && iptgetpref('UseIPPL'); 
%--------------------------------------------------------------------------
...
for k = 1:length(varargin)
    if ischar(varargin{k})
        string = validatestring(varargin{k}, allowed_strings, ...
