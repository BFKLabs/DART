...
if strcmp(method,'euclidean')
    ...
    if (nargout == 2)
        ...
        if(numOfElements <= intmax('uint32'))
            imgIdx = reshape(uint32(1:numOfElements), s);
        else
            imgIdx = reshape(uint64(1:numOfElements), s);
        end
        [D, IDX] = bwdistComputeEDTFT(BW, imgIdx);
        varargout{1} = sqrt(D);
        varargout{2} = IDX;                  
    else
    ...
    [weights,conn] = computeChamferMask(ndims(BW),method);
...
error(message('images:bwdist:internalError', upper( string )))
