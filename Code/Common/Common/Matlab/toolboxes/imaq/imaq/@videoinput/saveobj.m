...
if ~isempty(objsSaved)
    for i = 1:length(objsSaved)
        if isequal(objsSaved{i}, obj)
            ...
            warning(message('imaq:saveobj:recursive'));
            clear('oc');
            return;
        end
    end
...
% x(i).triggerProps = triggerconfig(uddobject).
...
propInfo = struct('uddobject', '', 'props', '', 'childprops', '', 'location', '', 'triggerProps', '');
...
for i = 1:numUDObjects
    ...
    elseif ishandle(currentObj)
        ...
        if ~localIsInvalidHandleSaved(currentObj),
            ...
            warnstate = warning('off', 'backtrace');
            oc = onCleanup(@()warning(warnstate));
            warning(message('imaq:saveobj:invalid'));
            clear('oc');
...
% Child objects cannot contain other objects.
...
propInfo(indexForProps).triggerProps = localFindTriggerProps(currentObj);
...
function triggerProps = localFindTriggerProps(currentObj)
% Finds the trigger properties for the UDD object currentObj
triggerPropNames = {'TriggerType', 'TriggerCondition', 'TriggerSource'};
triggerVals = get(currentObj, triggerPropNames);
triggerProps = cell2struct(triggerVals, triggerPropNames, 2);
...
% invalid, then all children in the array will be invalid
currentObj = imaqgate('privateGetField', value, 'uddobject');
