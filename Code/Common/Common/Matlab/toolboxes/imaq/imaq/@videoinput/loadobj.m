    ...
    if isstruct(propVals)
        ...
        try
            ...
            if ~isempty(aliases)
                ...
                tempobj = videoinput(constructorArgs{:}); %#ok<TNMLP>
                propInfo(i).obj = imaqgate('privateGetField', tempobj, 'uddobject');
            end
            ...
            try
                ...
                warning(message('imaq:loadobj:noInputRestore'));
                clear('oc');
            end
            
            % Reset the trigger configuration
            if (isfield(propInfo(i), 'triggerProps'))
                try
                    triggerVals = propInfo(i).triggerProps;
                    triggerconfig(propInfo(i).obj, triggerVals.TriggerType, ...
                        triggerVals.TriggerCondition, triggerVals.TriggerSource);
                catch %#ok<CTCH>
                    warnstate = warning('off', 'backtrace');
                    oc = onCleanup(@()warning(warnstate));
                    warning(message('imaq:loadobj:noTriggerRestore'));
                    clear('oc');
                end
            end
            ...
            if nChildren ~= length(propInfo(i).childprops),
                ...
                warnstate = warning('off', 'backtrace');
                oc = onCleanup(@()warning(warnstate));
                warning(message('imaq:loadobj:wrongDevice'));
                clear('oc');
            else
                for nChild = 1:nChildren,
                    ...
                    try
                        ...
                        warnstate = warning('off', 'backtrace'); 
                        oc = onCleanup(@()warning(warnstate));
                        warning(message('imaq:loadobj:noSourceRestore'));
                        clear('oc');
                    end
                end
            end
            ...
            % Invalid object.
            propInfo(i).obj = videoinput(propVals); %#ok<TNMLP>
        end
    else
        % Invalid object.
...
for i = 1:length(uddObjects);
    out(i) = propInfo(uddObjects{i}).obj; %#ok<AGROW>
end
    ...
    for j = 1:length(allLocations)
        ...
        if isimaqobj,
            if ~isempty(loc.ChildIndex)
                ...
                objToInsert = videoinput(currentObj); %#ok<TNMLP>
            end
        else
        ...
        if isnumeric(loc.Index)
            ...
            currentPropValue = localFillInArray(currentPropValue, objToInsert, loc.Index); %#ok<NASGU>
        else
            ...
            if (length(loc.Index) > 2) && (isnumeric(loc.Index{end}) && strcmp(loc.Index{end-1}, '()'))
                currentPropValue = localFillInStructOrCellWithArray(currentPropValue, objToInsert, loc.Index); %#ok<NASGU>
            else
            	currentPropValue = localFillInStructOrCell(currentPropValue, objToInsert, loc.Index); %#ok<NASGU>
            end
        end
...
if ~isempty(currentPropValue)
	...
	val = localFillInArray(val, obj, index{end}); %#ok<NASGU>
else
    val = localFillInArray('', obj, index{end}); %#ok<NASGU>
end
...
if index <= len
    ...
    for i = len:index-1
        currentPropValue = [currentPropValue obj]; %#ok<AGROW>
    end
end
