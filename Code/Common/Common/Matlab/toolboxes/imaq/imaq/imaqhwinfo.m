...
iatgeneral.ToolboxPreferencesManager.getOrResetInstance();
...
switch nInputs
    case 0
        ...
        % Warning added for support packages only when adaptor list is
        % empty. Do not want to warn even if they have one adaptor showing
        % up. 
        if isempty(out.InstalledAdaptors)
            warnSetting = warning('backtrace', 'off');
            warning(message('imaq:imaqhwinfo:additionalVendors'));
            warning(warnSetting);
        end
    otherwise
        ...
        if nInputs==2
            ...
            if (~isnumeric(field) && ~iscellstr(field))
                error(message('imaq:imaqhwinfo:fieldType'));
            elseif (numel(field)~=length(field))
                error(message('imaq:imaqhwinfo:matrixField'));
            end
        end
        
        adaptor = privateTranslateAdaptor(adaptor);
        ...
        try
            ...
            if isempty(matchIndex)
                error(message('imaq:imaqhwinfo:invalidAdaptor'));
            end
            ...
            if iscellstr(field),
                ...
                for i=1:length(field)
                    ...
                    if any(matchIndices)
                        ...
                        error(message('imaq:imaqhwinfo:invalidField', field{i}));
                    end
                end
                ...
                for i=1:length(field)
                    ...
                    if isempty(index),
                        error(message('imaq:imaqhwinfo:noDeviceID'));
                    else
