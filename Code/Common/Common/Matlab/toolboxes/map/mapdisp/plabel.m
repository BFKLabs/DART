...
if isinf(alt)
    if matlab.graphics.internal.isGraphicsVersion1
        %  If mstruct.galtitude is Inf, place the labels
...
alt = alt(ones(size(latline)));

if ~isempty(latline) && ~isempty(lonline)

	hndl0 = textm(latline,lonline,alt,labelstr,...
               'Color',fontcolor,...
               'FontAngle',fontangle,...
               'FontName',fontname,...
               'FontSize',fontsize,...
    ...
    if ~matlab.graphics.internal.isGraphicsVersion1
        ...
        map.graphics.internal.restackMapAxes(hndl0)
    end
