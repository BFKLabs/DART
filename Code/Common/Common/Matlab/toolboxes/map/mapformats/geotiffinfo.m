...
if isfield(tags, tagname) && isfield(tags.(tagname), keyname)
    ...
    if pcscode > 0      
        ...
        projcode = getDoubleValue(result, epsgname, gtiff_info.UserDefined);
...
function tiff_info = addGeoTiffTags(tiff_info, tags, geoTiffTags)
...
% Assign the values to the output structure. Use deal since tiff_info can
% be an array.
...
if isempty(ctTranslationMap) || ~isa(ctTranslationMap, 'containers.Map')
    ...
    % 9816 is Tunesia Mining Grid which has no CT projection, set to ''.
