...
% Determine the shape type of S and a handle to a write function.
...
switch(geometry)
    case 'point'
        if any(strcmp(classname, {'geopoint','mappoint'}))
...
% Loop through each feature and convert the requested attributes (defined
% by geostructFieldIndex) to values in a cell array. Each attribute is
% either a scalar numeric value, a value in a cell array, or a string
% value.
...
if isstruct(S)
    % Validate and convert to a dynamic vector.
    try
