...
if strcmp(nameOfEnclosingDirectory,'e000')
    if 'W' == DSI.LongitudeofNEcorner(end)
        ...
        warning(message('map:dted:correctingLongitudes', [ fullfile( nameOfEnclosingDirectory, name ), ext ]));
...
if (latlim(1)>maplatlim(2)+tolerance || ...
    ...
    warning(message('map:dted:limitsOutsideDataset', mat2str( [ maplatlim( 1 ), maplatlim( 2 ) ], 3 ), mat2str( [ maplonlim( 1 ), maplonlim( 2 ) ], 3 )));
    map=[];
    refvec = [];
    return
end
...
if any(negatives) && all(z(negatives) < -12000)
    ...
    warning(message('map:dted:twosComplementDetected'))
