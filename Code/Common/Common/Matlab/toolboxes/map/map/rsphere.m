...
%   if the string METHOD is 'mean' and the geometric mean, sqrt(a*b), if
%   METHOD is 'norm'.
%
...
%   if the string METHOD is 'mean' and the triaxial geometric mean,
%   (a^2 * b)^(1/3), if METHOD is 'norm'.
%
...
%   mean if the string METHOD is 'mean' and a geometric mean if METHOD is
%   'norm'.
%
...
%   R = RSPHERE('euler', ..., ANGLEUNITS) use the ANGLEUNITS string to
%   specify the units of the latitude and longitude inputs. ANGLEUNITS can
...
if ~strcmp(typeOfRadius,'euler')
    ellipsoid = checkellipsoid(varargin{2},'RSPHERE','ELLIPSOID',2);
    ...
    ellipsoid = checkellipsoid(varargin{6},'RSPHERE','ELLIPSOID',6);
