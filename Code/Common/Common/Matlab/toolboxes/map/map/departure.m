...
%   D = DEPARTURE(LON1,LON2,LAT,ELLIPSOID,ANGLEUNITS) use the input string
...
elseif nargin == 4
    if ischar(in4)
        ...
        ellipsoid = checkellipsoid(in4,'DEPARTURE','ELLIPSOID',4);
        units = 'degrees';
        useSphericalDistance = false;
    end
else
    ellipsoid = checkellipsoid(in4,'DEPARTURE','ELLIPSOID',5);
