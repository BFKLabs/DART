...
if isstruct(R)
    ...
    if isPlaid(R.LatMesh,R.LonMesh)
        lonmesh = R.LonMesh;
        ncols = size(lonmesh,2);
        % Apply geoquadline to 1-by-N curve running along the equator.
        [~,lonlim] = geoquadline(zeros(1,ncols),lonmesh(1,:));
        if isequal(lonlim,[-180 180]);
            % The mesh wraps all the way around in longitude. We want
            % limits correspond to the "seam" between the first and last
            % columns.
            lonlim = lonmesh(1,1) + [0 360];
        end
    else
        lonlim = [-Inf Inf];
    end
...
% [Avoid this: lon = interp2(R.LonMesh, x, y, '*linear');]
slon = interp2(sind(R.LonMesh),x,y,'*linear');
clon = interp2(cosd(R.LonMesh),x,y,'*linear');
