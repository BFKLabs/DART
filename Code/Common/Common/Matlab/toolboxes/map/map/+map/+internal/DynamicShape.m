classdef DynamicShape < map.internal.DynamicVector
    ...
    methods (Access = protected, Hidden = true)
                ...
                for k=1:numel(f)
                    ...
                    if isVertexValue(prop)
                        ...
                        if ischar(type)
                            ...
                            % value is {cell array of strings, string}
        ...
        %   self.Name = value       (where Name is a dynamic property)
            ...
            if usingVertexRules
                ...
                % Setting a feature property. Replace empty string
                % values.
                self = replaceEmptyStringInProperties(self);
            end
        end
        ...
        % This method is not invoked when assigning self.Name
            ...
            if  ~isscalar(in) && ...
                    ...
                    (iscellstr(in) && ~isempty(in))
                % IN is either:
                % 1) Not a cell and not scalar and is numeric or logical. 
                % 2) A non-empty cell array of strings
            ...
            % '' for strings) to match the length of that element.
            %
            if iscell(in) && isempty(in)
            ...
            if isfield(temp.pDynamicProperties, fname)
                ...
                % otherwise, replace the empty string values.
                if usingVertexRules
        ...
        % Validate dynamic property input values.
            ...
            if ~isVertexValue(in)
                ...
                if iscell(in)
                    % Determine if cell IN contains strings or cell arrays.
                    % Allow a cell array of : cell array of strings:
                    % { {strings}, {strings}, {strings}}
                    % or a cell array of : strings and cell array of strings:
                    % { 'string', {strings}, {strings}}
                    containsCells = cellfun(@iscell, in);
                    containsChars = cellfun(@ischar, in);
                    containsCellsOrStrings = containsCells | containsChars;
                    if any(containsCellsOrStrings)
                        % Validate cells and string content.
                        if ~all(containsCellsOrStrings)
                            % Must be cell array of strings or cell array
                            % of cell array of strings.
                        ...
                        elseif any(containsCells)
                            ...
                            if any(containsChars)
                                ...
                                % strings. To allow this behavior, and
                                % allow a vertex property to contain a cell
                                % array of strings mixed with cellstrs,
                                % remove the error and uncomment the other
                                % two lines.
                                error(message('map:DynamicShape:invalidCellStrs'))
                                % Wrap string elements as cells.
    ...
    methods (Access = private, Hidden = true)
        ...
        % Replace empty strings in property values with a feature
        % separator.
        ...
        % Synchronize the dynamic vertex properties.
            ...
            for k=1:numel(propnames)
                ...
                if isVertexValue(dynamic)
                    ...
                    % array. For string-valued dynamic properties, this
...
if ~isscalar(v) && ~isempty(v)
    ...
    % continue only if needed. If the cell array contains strings or cell
    ...
    for k=numel(v)-1:-1:1
        ...
        elseif isempty(v{k}) && ischar(v{k})
            % The empty string needs to be converted to cell value.
            v{k} = [{''} {''}];
...
if ~isempty(values) && iscell(values) && iscell(values{1})
    ...
    if ~ischar(v{1})
        ...
        % logical or numeric or a scalar string.
    ...
    elseif isscalar(v)
        ...
        % v{1} is a cell array of strings, return values
        vout = values;
    end
else
    vout = values;
...
% Replace empty string in cell array with empty cellstr:
% replace { ...'' ... } with { ... {''} ...}
...
% If c is a non-scalar cell array, with at least one non-string value,
% and it contains empty values, then replace those values with 0s.

if iscell(c) && ~isscalar(c) && ~iscellstr(c)
...
% Return true if the cell array, c, contains cell arrays of strings, or
% strings.

fcn = @(x) (iscellstr(x) || ischar(x));
...
%
% Upcast X to class double if the class of X is single and the class of
