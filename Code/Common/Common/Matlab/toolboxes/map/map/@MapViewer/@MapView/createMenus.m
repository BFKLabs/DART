...
ax_pos_pixels = getpixelposition(ax);
...
if ~matlab.graphics.internal.isGraphicsVersion1
    ...
    % Use the map viewer figure.
    hfig = this.Figure;
    ax = this.getAxes();
    
    % Use getpixelposition to determine the position of the lower left corner
    % of the axes relative to the figure.
    ax_pos = getpixelposition(ax, true);
    
    % Form the position rectangle of the selection rectangle relative to the
    % figure. The FigureSelectionBox field of the MapViewer object contains the
    % location of the selection rectangle as a set of vertices [x1 y1; x2 y2].
    rect_pos = [this.FigureSelectionBox(1,:), diff(this.FigureSelectionBox)];
    
    % We want the region inside the line, so we need to trim the width of
    % the selection rectangle to account for the width of the selection
    % rectangle lines.
    ...
    % Export an image frame of the area outlined by the selection rectangle.
    frame = getframe(hfig, rect_pos);
    exportFrameToFile(this, rect_pos_relative_to_axes, frame);
end
