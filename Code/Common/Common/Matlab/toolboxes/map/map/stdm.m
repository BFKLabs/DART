...
if m==1
	if n==1
		...
		lat=lat(:);
        lon=lon(:);
		latstd=norm(lat-latbar)*ellipsoid(1)/(n-1);
		lonstd=norm(departure(lon,lonbar*ones(n,1),lat,ellipsoid,'radians'))/(n-1);
	end
	...
	lonstd=zeros(1,n);
	for i=1:n
		latstd(i)=norm(lat(:,i)-latbar(i))*ellipsoid(1)/(m-1);
		lonstd(i)=norm(departure(lon(:,i),lonbar(i)*ones(m,1),lat(:,i),ellipsoid,'radians'))/(m-1);
	end
end
