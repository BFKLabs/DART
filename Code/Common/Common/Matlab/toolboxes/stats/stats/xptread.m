function [tbl, missing, fileHeader] = xptread(filename,varargin)
%XPTREAD Creates a table array from data stored in a SAS XPORT format file.
%
%   DATA = XPTREAD displays a dialog box for selecting a file, then reads
%   data from the file into a table array. The file should be in the SAS
%   XPORT format.
%
...
tbl = struct2table(obs);
tbl.Properties.Description = fileHeader.DataSetName;
...
tbl.Properties.VariableDescriptions = {var.Label};
if readObsFlag
    % We see if the first variable can be used as observations. If so, then
    % we remove it from the table and remove corresponding missing
    % values.
    try
        tbl.Properties.RowNames = obs.(var(1).vname);
        tbl.(var(1).vname) = [];
        missing(:,1) = [];
    catch theEx
        % If the variable is inappropriate for observations then warn but
        % keep going in the most obvious cases
        if strcmpi(theEx.identifier,'MATLAB:table:DuplicateRowNames')
            warning(message('stats:xptread:DuplicateObsnames'));
        elseif strcmpi(theEx.identifier,'MATLAB:table:InvalidRowNames')
