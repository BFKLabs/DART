...
%   $Revision: 1.1.6.3 $  $Date: 2007/11/09 20:32:06 $
...
for i = 1:length(someFunction)
    ...
    if(~isempty(handle)) && (isa(handle,'inline') || isa(handle,'function_handle'))
        cellArrayOfFcn{i} = handle;
    else
        msg = sprintf('The field ''%s'' must contain a function handle.',property);
        error('optim:createCellArrayOfFunctions:needFunctionOrCell',msg);
    end
end
