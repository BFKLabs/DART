...
if (nargin < 3)
    % if not provided, 
    [wStr,wOfs] = deal({'Current Experiment Progress','Output Data Field'},0);    
    h = waitbarFig(wStr,'Position Movie Setup');  
else
    [wOfs,wStr] = deal(1,getappdata(h,'wStr'));
end
...
for i = 1:nApp
    ...
    wStrNw = sprintf('%s (Apparatus %i of %i)',wStr{1},i,nApp);
    if (waitbarFig(1+wOfs,wStrNw,i/(1+nApp),h))
...
if (nargin < 3)
    ...
    waitbarFig(2,'Position Movie Creation Complete!',1,h);
end
...
for i = 1:nStack
    % updates the waitbar figure
    wStr = sprintf('Adding Movie Frame (%i of %i)',i,nStack);
    if (waitbarFig(2+wOfs,wStr,i/nStack,h))
