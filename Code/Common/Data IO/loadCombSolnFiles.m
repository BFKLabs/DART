...
switch (nargin)
    case (2) % case is loading from single combined solution files
        ...
        h = waitbarFig(wStr,'Loading Experimental Solution File');    
...
if (waitbarFig(1+wOfs,'Loading Base Data File...',p0,h))
    ...
    if (~isfield(snTot,'iMov'))        
        ...
        switch (nargin)
            case (2)
                eStr = {'This single experimental solution file (.ssol) has an obsolete format.';'';...
                        ['You will need to recombine the .ssol file before being ',...
                         'able to analyse the data within the DART Analysis GUI.']}; 
        end
            ...
            case (2)
                eStr = {'This single experimental solution file (.ssol) has an obsolete format.';'';...
                        ['You will need to recombine the .ssol file before being ',...
                         'able to analyse the data within the DART Analysis GUI.']};                    
            case (3)
                eStr = {['The current single experimental solution file (.ssol) within this ',...
                         'multi-experiment solution file (.msol) has an obsolete format.'];'';...
                        ['You will need to recombine the .ssol file(s) before being ',...
                         'able to analyse the data within the DART Analysis GUI.']};                     
            case (6)
                eStr = {['The current single experimental solution file (.ssol) within this ',...
                         'multi-experiment solution file (.msol) has an obsolete format.'];'';...
                        ['You will need to recombine the .ssol file(s) before being ',...
                         'able to combine the data within the DART Data Combining GUI.']};                                     
        end
...
waitbarFig(1+wOfs,'Recalculating X-Locations...',pS(1),h);
waitbarFig(2+wOfs,'Fly X-Location Reading',0,h);
...
if (isempty(snTot.Px))
    ...
    waitbarFig(2+wOfs,'Fly X-Location Reading Complete!',1,h);
end    

% determines if the Y-values need to be loaded (if there are any files)
if (anyY)
    % otherwise, load the fly locations
    waitbarFig(1+wOfs,'Recalculating Y-Locations...',pS(1)+pS(2),h);
    waitbarFig(2+wOfs,'Fly Y-Location Reading',0,h);    
    ...
    if (isempty(snTot.Py))
        ...
        waitbarFig(2+wOfs,'Fly Y-Location Reading Complete!',1,h);
    end            
else
...
if (anyP)
    % otherwise, load the fly locations
    waitbarFig(1+wOfs,'Recalculating Orientation Angle...',pS(1)+2*pS(2),h);
    waitbarFig(2+wOfs,'Fly Orientation Angle Reading',0,h);    
    ...
    if (isempty(snTot.Phi))
        ...
        waitbarFig(2+wOfs,'Fly Orientation Angle Reading Complete!',1,h);
    end            
end
...
% if (anyN)
%     % otherwise, load the fly locations
%     waitbarFig(1+wOfs,'Recalculating Object Size...',pS(1)+2*pS(2),h);
%     waitbarFig(2+wOfs,'Object Size Reading',0,h);    
...
%         return         
%     else
%         waitbarFig(2+wOfs,'Object Size Reading Complete!',1,h);
%     end            
% end
...
snTot = orderfields(snTot);
waitbarFig(1+wOfs,'Recalculation Complete!',1,h); pause(0.005);
...
if (isfield(snTot,'iMov'))
    if (snTot.iMov.nRow > 1) && (is2DCheck(snTot.iMov))    
        if (~iscell(snTot.iMov.iR{1}))
            ...
            % is obsolete if the row index range is greater than y-value range
            isObs = rMax > yRng;
        end        
    end
end