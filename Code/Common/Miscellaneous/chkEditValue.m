...
function ok = chkEditValue(nwVal,valLim,chkInt,isOdd)
...
elseif (nwVal < (valLim(1)-del))
    ...
    if (isinf(valLim(2)))
        if (chkInt || (isIntLim(1)))
            eStr = sprintf('Input value must be greater than or equal to %i.',valLim(1));
        else
            eStr = sprintf('Input value must be greater than or equal to %.3f.',valLim(1));
        end
    else
...
elseif (nwVal > (valLim(2)+del))
    ...
    if (isinf(valLim(1)))
        if (chkInt || (isIntLim(1)))
            eStr = sprintf('Input value must be less than or equal to %i.',valLim(2));
        else
            eStr = sprintf('Input value must be less than or equal to %.3f.',valLim(2));
        end
    else
        if (chkInt || (all(isIntLim)))
            eStr = sprintf('Input value must be between %i and %i.',...
...
if (~ok)
    waitfor(errordlg(sprintf('Error! %s',eStr),'Input Value Error','modal'));
end