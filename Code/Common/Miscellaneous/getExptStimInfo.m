...
function stimP = getStimInfo(sPara)
...
for iStim = 1:length(sPara.sTrain)
    ...
    for iCh = 1:length(indU)
        ...
        stimP = setStimInfo(stimP,tArrCh,tS0,tLim(iCh,:),iStim);        
    end
end
...
function stimP = setStimInfo(stimP,tArr,tS0,tLim,iStim)
...
stimPS = appendStimField(stimPS,chName,'Ts',tLim(1)+tS0);
stimPS = appendStimField(stimPS,chName,'Tf',tLim(2)+tS0);
