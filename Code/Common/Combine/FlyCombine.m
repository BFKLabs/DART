...
% Last Modified by GUIDE v2.5 11-Jun-2017 22:35:04
...
h = ProgressLoadbar('Initialising Combining GUI...');
...
switch (length(varargin))   
    case (0) % case is running full program from command line
        ...
        set(hObject,'name','DART Fly Experiment Data Output Program (Test Mode)') 
    ...
    otherwise % case is any other number of input arguments
        ...
        eStr = 'Error! Incorrect number of input arguments. Exiting Data Combining GUI...';
        waitfor(errordlg(eStr,'Data Combining GUI Initialisation Error','modal'))
...
set(handles.menuSaveSingleSoln,'enable','off')
cla(handles.imgAxes); axis(handles.imgAxes,'off')
...
set(hObject,'visible','on'); pause(0.1);
...
if (isempty(fFile))
    ...
    switch (length(fDir))
        case (0)
            return
        case (1)
            smFile = fullfile(fDir{1},getSummFileName(fDir{1}));
            [snTot,iMov] = combineSolnFiles(fFileGrp{1},smFile);    
        otherwise
...
% prompts the user for the solution file directory
[fName,fDir,fIndex] = uigetfile({'*.ssol;','Experimental Solution Files (*.ssol)'},...
...
% determines if there is 1 or 2 analysis required
...
if (isfield(snTot,'iMov'))
    ...
    if (is2D)
        wStr = sprintf(['Warning! The loaded experimental solution file ',...
                        'has an obsolete format.\n\nIt is highly recommended ',...
                        'that you recombine this experimental solution file ',...
                        'from the constituent video solution files']);
...
switch (indNw(2))
    case (1) % case is the apparatus name        
        if (~strContains(nwData,','))
            ...
            eStr = 'Apparatus names strings can''t contain a comma';
            waitfor(errordlg(eStr,'Apparatus Name Error','modal'))
...
resetLimitMarker(handles.imgAxes,[0 0],'Start')
set(handles.textStartTime,'string',datestr(iPara.Ts0,'mmm dd, YYYY HH:MM AM'))
...
iPara = getappdata(handles.figFlyCombine,'iPara');
[iPara.Tf,iPara.indF] = deal(iPara.Ts0,[length(snTot.T) length(snTot.T{end})]);
setappdata(handles.figFlyCombine,'iPara',iPara);

% resets to the finish time
resetPopupFields(handles.panelFinishTime,iPara.Tf)
resetLimitMarker(handles.imgAxes,xLimTot(2)*[1 1],'Finish')
set(handles.textFinishTime,'string',datestr(iPara.Tf0,'mmm dd, YYYY HH:MM AM'))
...
if (strcmp(get(get(hObject,'parent'),'tag'),'panelStartTime'))
    ...
    if ((dTS < 0) || (dTF < 0))
        ...
        resetLimitMarker(handles.imgAxes,dTS*[1 1]*Tmlt,'Start')
    end
else
    % calculates the time difference between the new time location and the
    % limit markers    
    ...
    if ((dTS < 0) || (dTF < 0))
        ...
        % resets the limit markers
        resetLimitMarker(handles.imgAxes,dTS0*[1 1]*Tmlt,'Finish')
    end
end
...
[hAx,xNew] = deal(handles.imgAxes,pNew(1,1));
...
switch (Type)
    case ('Start')
        ...
        hFinish = findobj(handles.imgAxes,'tag','Finish');        
        ...
        if (xNew == 0)
            ...
            iPara.indS(2) = find([snTot.T{iPara.indS(1)}*Tmlt;1e10] <= xNew,1,'last');                
        end
        ...
        if ((iPara.indS(1)*iPara.indS(2)) == 1)
            ...
            iPara.Ts = calcTimeString(T0,snTot.T{iPara.indS(1)}(iPara.indS(2)));
        end
        setappdata(handles.figFlyCombine,'iPara',iPara)
    ...
    case ('Finish')
        ...
        hStart = findobj(handles.imgAxes,'tag','Start');
        ...
        iPara.indF(2) = find([snTot.T{iPara.indF(1)}*Tmlt;1e10] <= xNew,1,'last');                
...
% clears the image axis
[hAx,isPlotAll] = deal(handles.imgAxes,false);
cla(hAx); axis(hAx,'on')
...
set(handles.popupAppPlot,'enable','on')
...
hold on
[xTick,yLim] = deal(get(hAx,'xtick'),[0,max(getFlyCount(iMov,1))]);
...
if (isPlotAll)
    hLbl = ylabel(hAx,'Apparatus Index','FontUnits','pixels');
else
    hLbl = ylabel(hAx,'Fly Index','FontUnits','pixels');
end

% updates the axis/label properties
set(hLbl,'fontweight','bold','fontsize',lblSize)
axis(hAx,'ij')
...
setColor(hLineS,'r');
set(hLineS,'tag',Type)
set(findobj(hLineS,'tag','bottom line'),'visible','off')
set(findobj(hLineS,'tag','top line'),'linewidth',lWidL)
set(findobj(hLineS,'tag','end point 1'),'hittest','off','linewidth',lWidM,'Markersize',mSizeM)
set(findobj(hLineS,'tag','end point 2'),'hittest','off','linewidth',lWidM,'Markersize',mSizeM)
...
[ok,nFly] = deal(getappdata(hGUIInfo,'ok'),getAppFlyCount(iMov,iApp));
...
if (~isempty(jj))
    for i = length(jj):-1:1
        ...
        if (~isempty(xPlt))
            xPlt = [xPlt(1:jj(i),:);NaN(2,size(xPlt,2));xPlt((jj(i)+1):end,:)];
        end
        % removes the gaps from the y-plot values
        if (~isempty(yPlt))
            yPlt = [yPlt(1:jj(i),:);NaN(2,size(yPlt,2));yPlt((jj(i)+1):end,:)];
        end        
    end
end
...
if (~isempty(appPara))    
    ...
    if (isPlotAll)
        ...
        if (any(aok))
            ...
            set(findobj(handles.imgAxes),'visible','on')            
            ...
            set(findobj(handles.imgAxes),'visible','off')
        ...
        if (aok(iApp))
            ...
            for i = 1:nFly
                ...
                if (~isempty(xPlt))                    
                    ...
                    yNw = (i + 0.5) - xPlt(:,i);
                    set(hPosNw,'LineWidth',0.5,'color','b','xdata',T*Tmlt,...
                           'yData',yNw,'visible',eStr{1+double(ok(i,iApp))});
                else
                    % otherwise, make the line invisible
                    set(hPosNw,'visible','off');                    
                end
                ...
                if (~isempty(yPlt))                    
                    % updates the plot data
                    yNw = (i + 0.5) - yPlt(:,i);
                    set(hPos2Nw,'LineWidth',0.5,'color','r','xdata',T*Tmlt,...
                           'yData',yNw,'visible',eStr{1+double(ok(i,iApp))});
                else
                    % otherwise, make the line invisible
                    set(hPos2Nw,'visible','off');                    
                end                
            end          
            ...
            x = findall(handles.imgAxes,'tag','top line');
            set(x,'ydata',yLim);
            
            % updates the end marker y-location
            y = findall(handles.imgAxes,'tag','end point 2');
            set(y,'ydata',yLim(2));
        else
            % if apparatus is rejected, turn off the image axis
            set(findobj(handles.imgAxes),'visible','off')
            set(hPos,'visible','off')
            set(hPos2,'visible','off')
        end
    end
end
...
h = waitbarFig({'Creating Fly Information Sub-GUI...'},...
                'Initialising Experimental Solution Data GUI',1);

% makes the GUI invisible
set(handles.figFlyCombine,'visible','off'); pause(0.01);

% initialises the parameter struct
iPara = initParaStruct(snTot);

% sets the visibility of the orientation angle menu item
set(handles.menuOrientAngle,'visible',eStr{1+isfield(snTot,'Phi')})
...
waitbarFig(1,'Initialising GUI Objects and Plot Markers...',0.8,h);
set(0,'CurrentFigure',handles.figFlyCombine)
set(handles.figFlyCombine,'CurrentAxes',handles.imgAxes)
initSolnInfo(handles)
initExptInfo(handles)

% updates the menu properties
set(handles.menuPlotData,'enable','on')
updateViewMenu(handles,handles.menuViewXData,1)
set(handles.menuViewYData,'enable',eStr{1+is2D})
set(handles.menuViewXYData,'enable',eStr{1+is2D})
...
set(handles.textStartTime,'string',datestr(iPara.Ts0,'mmm dd, YYYY HH:MM AM'))
set(handles.textFinishTime,'string',datestr(iPara.Tf0,'mmm dd, YYYY HH:MM AM'))
set(handles.toggleZoom,'enable','on')
set(handles.menuSaveSingleSoln,'enable','on')
...
set(handles.figFlyCombine,'visible','on'); pause(0.01);

% updates and closes the waitbar figure
waitbarFig(1,'GUI Initialisation Complete',1,h); pause(0.1);
try; close(h); end
...
for i = 1:nDir
    ...
    smFile = fullfile(fDir{i},getSummFileName(fDir{i}));
    [snTotT{i},iMovNw{i}] = combineSolnFiles(fFile{i},smFile);    
...
for i = 2:nDir
    ...
    if (waitbarFig(3,wStrNw,(i-1)/nDir,hh))
        % if the user cancelled, then exit the function
        [snTot,hh] = deal([]);
        return
    end
...
waitbarFig(3,'Post-Processing Complete!',1,hh);
close(hh); hh = [];
...
if (~isempty(getappdata(handles.figFlyCombine,'snTot')))
    ...
    set(handles.tableAppInfo,'Data',Data,'enable','on')
else
    rowName = cellfun(@(x)(sprintf('Group #%i',x)),...
...
%
fPos0 = get(handles.figFlyCombine,'position');

% retrieves the base table dimensions
while (W0 == 0)
    % retrieves the table dimensions
    [~,~,W0] = getTableDimensions(getappdata(handles.figFlyCombine,'jObjT'));
    
    % if the width is still zero, then make the GUI visible and try again
    if (W0 == 0)                
        isMove = true;
        resetObjPos(handles.figFlyCombine,'Left',-(fPos0(3)+10));
        set(handles.figFlyCombine,'visible','on'); pause(0.05);         
    end
end

% resets the object position
if (isMove)
    set(handles.figFlyCombine,'visible','off'); pause(0.05);         
    resetObjPos(handles.figFlyCombine,'Left',fPos0(1));
end
...
if (ismac)
    ...
    % case is using a PC
    [axSize,lblSize] = deal(16*fR,22*fR);    
end