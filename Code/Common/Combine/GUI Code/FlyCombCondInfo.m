...
try
    ...
    set(handles.figFlyInfoCond,'visible','on')
    jTab = getJavaTable(hSP);
end
...
[ok,eStr] = deal(getappdata(handles.figFlyInfoCond,'ok'),{'off','on'});
...
if isfield(hGUI,'figFlyCombine')
    ...
    if isPlotAll
        ...
        set(hPos,'visible',eStr{1+eventdata.NewData});        
    else
        ...
        if (iApp == indNw(2))
            ...
            if (appPara.ok(indNw(2)))
                ...
                set(hPos,'visible',eStr{1+newValue});
            end
        end
    end
else
...
for i = 1:nApp
    ...
    wStrNw = sprintf('Calculating Combined Dataset Metrics (Apparatus %i of %i)',i,nApp);
    waitbarFig(1,wStrNw,0.8*(i/(nApp+1)),h);
...
for i = 1:length(iMov.iR)
    Data((getAppFlyCount(iMov,i)+1):end,i) = {[]};
end
