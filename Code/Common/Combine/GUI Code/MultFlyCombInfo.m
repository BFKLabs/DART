...
if (~isempty(hGUIInfoM))
    set(hGUIInfoM,'visible','off')
end

% makes the main and current GUIs invisible
set(hGUI.figFlyCombine,'visible','off')
...
if (~isa(eventdata,'struct'))
    ...
    if (isempty(fFile))
        ...
        h = waitbarFig(wStr,'Experimental Solution File Loading');
...
for i = 1:nAdd
    % updates the waitbar
    if (~isMulti)
        if (waitbarFig(1,sprintf('%s (%i of %i)',wStr{1},i,nAdd),i/nAdd,h))
            % if the user cancelled, then exit the function
            return
        end
    end
...
if (isMulti)
    ...
    set(handles.buttonSolnAdd,'enable','off')
    
    % makes the GUI visible again
    pause(0.5);
    set(handles.figMultCombInfo,'visible','on'); 
else
    % closes the waitbar
    close(h); 
end
...
[fName,fDir,fIndex] = uigetfile({'*.msol','Multi-Experiment Solution Files (*.msol)'},...
                        'Select The Multi-Solution File',iProg.DirComb);
if (fIndex == 0)
...
h = waitbarFig(wStr,'Creating Multi-Experimental Solution File');
...
for i = 1:nFile
    ...
    % updates the waitbar figure
    waitbarFig(2,'Solution File Update Complete!',1,h);
end
...
set(hGUI.figFlyCombine,'Visible','on');

% makes the info GUI visible (if it exists)
hGUIInfoM = getappdata(hGUI.figFlyCombine,'hGUIInfo');
if (~isempty(hGUIInfoM))
    set(hGUIInfoM,'visible','on')
end
...
if (yMax == 2)
    ...
    set(handles.buttonSolnAdd,'enable','on')
    set(handles.buttonSolnRemove,'enable','off')
end
...
if (yMax == 2)
    ...
    % retrieves the indices of the selected item
    set(handles.buttonSolnAdd,'enable','off')
    set(handles.buttonSolnRemove,'enable','on')
end
...
uChoice = questdlg('Do you want to synchronise all the apparatus link indices?',...
    'Synchronise Link Indices','Yes','No','Yes');
if (~strcmp(uChoice,'Yes'))
    % if the user cancelled, then exit the function
    return
else
...
set(handles.buttonSolnReset,'enable','on')

% makes the GUI invisible
set(handles.figMultCombInfo,'visible','off'); pause(0.01);
...
if (iSolnAdd.nCount == 0)
    ...
    set(handles.textOutName,'enable','on')
    set(handles.tableAppName,'enable','on','visible','on')
...
if (iSolnAdd.nCount == 0)
    ...
    set(handles.buttonSolnReset,'enable','off')
    set(handles.menuSave,'enable','off')
...
% makes the GUI visible again
set(handles.figMultCombInfo,'visible','on'); pause(0.01);
...
if (all(cellfun(@isempty,appOut)))
    ...
    set(handles.buttonClearApp,'enable','off')
else
    % otherwise, enable the clear button
    set(handles.buttonClearApp,'enable','on')
end
...
if (~isempty(indSoln))
    if (all(iSolnAdd.indOut{indSoln} == 0))
        ...
        set(handles.buttonSyncInd,'enable','off')
    else
        % some cross-link indices have been set, so enable button
        if (any(find(iSolnAdd.indOut{indSoln}) > ...
                                min(cellfun(@length,iSolnAdd.indOut))))
            set(handles.buttonSyncInd,'enable','off')
        else
            set(handles.buttonSyncInd,'enable','on')
        end
    end
end
...
set(handles.figMultCombInfo,'visible','off')
set(handles.listSolnAll,'enable','on')
set(handles.listSolnAdded,'enable','on')
...
if (isShow)
    set(handles.figMultCombInfo,'visible','on'); pause(0.05);
end
...
if (nExp == 0)
    ...
    set(handles.listSolnAdded,'String','','max',2,'Value',[],'enable','off')
else
    % sets the listbox title/object properties
    setAddList(handles,iSolnAdd)
end
...
elseif ((iSolnAdd.nCount > 1) && (~isempty(indNw)))
    if (indNw(1) == 1)
        ...
        set(handles.buttonMoveUp,'enable','off');
        set(handles.buttonMoveDown,'enable','on');
    elseif (indNw(1) == iSolnAdd.nCount)
        % case is the last entry
        set(handles.buttonMoveUp,'enable','on');
        set(handles.buttonMoveDown,'enable','off');
    else
...
% makes the GUI invisible
set(handles.figMultCombInfo,'visible','off'); pause(0.05);
...
[nCount,eStr,col] = deal(iSolnAdd.nCount,{'off','on'},'rk');
appOut = getappdata(handles.figMultCombInfo,'appOut');
...
set(handles.listSolnAdded,'String',lStr,'enable','on')
set(handles.menuSave,'enable',eStr{all(isSet)+1})
...
if (isempty(ind))
    ...
    set(handles.tableAppLink,'Data',[],'enable','off')
    set(handles.textAppLink,'enable','off')  
    set(handles.buttonSyncInd,'enable','off')  
else
    ...
    % updates the table
    set(handles.textAppLink,'enable','on')
    ...
    appOut = getappdata(handles.figMultCombInfo,'appOut');
    if (all(cellfun(@isempty,appOut)))
        set(handles.tableAppLink,'enable','off')
    end
end
