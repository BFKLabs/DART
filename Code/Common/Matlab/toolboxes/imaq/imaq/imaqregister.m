...
%   (Windows), "/local/adaptors/adaptor.so" (Linux), or
%   "/local/adaptors/adaptor.dylib" (Mac), the adaptor base name is
%   "adaptor".
%
%   NOTE:  The adaptor shared library (a DLL on Windows) is not the same
%          as the driver shared library supplied by a hardware vendor (also
%          a DLL on Windows).  The adaptor shared library is specific to
%          the Image Acqusition Toolbox and will be specified as such by
%          the hardware vendor.
%
...
elseif (nargin == 2)
    ...
    if ( ~(isvector(action) && ischar(action)) )
        error(message('imaq:imaqregister:invalidaction'));
    end
...
if (nargin > 0);
    ...
    if ( ~(isvector(adaptor) && ischar(adaptor)) )
        error(message('imaq:imaqregister:invalidadaptorpath'));
    end
...
if ~exist(adaptorFile, 'file')
    error(message('imaq:imaqregister:filenotfound', adaptorFile));
end
...
if ( isempty(adaptorPath) || strcmp(adaptorPath(1), '.') )
    error(message('imaq:imaqregister:nonfullpath'));
end
...
% will resolve to the same name.  If this is the exact same path as an
% already registered adaptor, then don't error, just exit.
if any(strcmpi(fullAdaptorPath, adaptorFile))
    return
elseif any(strcmpi(oldAdaptors, adaptorName))
    error(message('imaq:imaqregister:alreadyregistered', adaptorName));
end

% Verify that it is a valid adaptor and not just any shared library.
if ~imaqmex('isvalidadaptor', adaptorFile)
    error(message('imaq:imaqregister:invalidadaptor', adaptorFile));
end
...
if any(adaptorFound)
    ...
    oc = onCleanup(@() warning(state));
    warning(message('imaq:imaqregister:unregisterfailed', adaptorFile));
    clear('oc');
end