...
% Error checking.  Since this is the non-object method of IMAQFIND,
% parameters must be PV pairs.  However, it is possible to specify the PV
% pairs as one or more structs so those need to be accounted for.
curArg = 1;
while (curArg <= nargin)
    if isstruct(varargin{curArg})
        % Since a struct only requires one parameter move to the next
        % parameter on the next pass through the loop.
        curArg = curArg + 1;
    else
        % If this is the last argument and it is not a struct, that is an
        % error.
        if (nargin == curArg)
            error(message('imaq:imaqfind:pvpair'));
        end
...
try
    ...
    throw(privateFixError(exception));
end
