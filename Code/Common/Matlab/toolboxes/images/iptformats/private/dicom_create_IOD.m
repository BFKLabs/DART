function [attrs, status] = dicom_create_IOD(IOD_UID, X, map, metadata, options)
...
if ((options.writeprivate) && (~isempty(private_tags)))
    ...
    new_attrs = process_private(private_tags, private_names, metadata, dictionary, options.txfr);
    attrs = add_to_IOD(attrs, new_attrs);
...
function [attrs, status] = process_modules(attr_details, metadata, dictionary)
...
for p = 1:numel(fields)
    ...
    tag = dicom_tag_lookup(fields{p}, dictionary);
...
function [attrs, status] = process_private(private_tags, private_names, metadata, dictionary, txfr)
...
for p = 1:numel(private_tags)
    attrs(p) = dicom_convert_meta_to_attr(private_names{p}, metadata, dictionary, txfr);
end
