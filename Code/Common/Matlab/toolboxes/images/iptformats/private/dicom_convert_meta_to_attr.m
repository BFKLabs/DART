function attr = dicom_convert_meta_to_attr(attr_name, metadata, dictionary, txfr)
%DICOM_CONVERT_META_TO_ATTR  Convert a metadata field to an attr struct.

%   Copyright 1993-2011 The MathWorks, Inc.

% Look up the attribute tag.
tag = dicom_tag_lookup(attr_name, dictionary);
...
% Add the attribute.
if (isempty(VR))
    attr = dicom_add_attr([], tag(1), tag(2), dictionary, data);
else
    attr = dicom_add_attr([], tag(1), tag(2), dictionary, data, VR);
end
...
function attrs = encode_SQ(SQ_struct, dictionary, txfr)
...
for p = 1:numel(items)
    ...
    attrs = dicom_add_attr(attrs, 'fffe', 'e000', dictionary, data);
    
end



function attrs = encode_item(item_struct, dictionary, txfr)
...
for p = 1:numel(attr_names)
    ...
    new_attr = dicom_convert_meta_to_attr(attr_names{p}, item_struct, dictionary, txfr);
