...
[attrs, settings] = dicomparse(fileDetails.name, ...
                               fileDetails.bytes, ...
                               machineEndian, ...
                               true, ...
                               dictionaryFile, ...
...
tag = dicom_typecast(metadata.InstanceData((1:4) + offsetDelta), ...
...
while (location < dataBytes)
    ...
    location = location + 4;
    len = dicom_typecast(InstanceData((1:4) + location), 'uint32', swapOffsetBytes);
