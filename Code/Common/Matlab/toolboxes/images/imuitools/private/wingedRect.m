...
draw_API.findSelectedVertex  = @findSelectedVertex;
    ...
    function initialize(h,translateRectFcn,sideResizeFcn,cornerResizeFcn)
        ...
        % Function scope. Determines whether rectangle is drawn to
        % indicate fixed aspect ratio.
        is_fixed_aspect_ratio = false;
        
        % Function scope. Determines whether rectangle is drawn to
        % indicate it is resizable.
        is_resizable = true;
        ...
        % Place the side markers below the side lines and put the
        % corners on top of everything else.  This makes sure the
        % ButtonDownFcns get hit when over the side markers and so
        % that the corners aren't obscured by the side lines.
        for i = 1:num_vert
        ...
        % Fix for g1007803: Loop over the corner markers after all side
        % markers and top lines are constructed so that all corners are on
        % top. This avoids the corner case where the first corner marker is
        % below the last side marker.
        for i = 1 : num_vert
