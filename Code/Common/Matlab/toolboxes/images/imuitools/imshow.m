...
%   IMG may also be a graphics file FILENAME.
%
...
%   spatial referencing object RX and colormap MAP.
%
...
%       imshow(I,RI);
%
%   See also IMREAD, IMREF2D, IMTOOL, IPTPREFS, SUBIMAGE, TRUESIZE, WARP, IMAGE,
%            IMAGESC.
...
if ~isempty(RI)
    if ~RI.sizesMatch(cdata)
        error(message('images:imref:sizeMismatch','ImageSize','imref2d'));
    end
...
if ~isequal(find(spatialArgPositions),2)
    error(message('images:imshow:incorrectImrefPosition','imshow(IMG,RI,___)'));
end

RI = varargin{2};
varargin(2) = [];
if isa(RI,'imref3d');
    error(message('images:imshow:imref3dNotValidRef'));
end
...
for i = 1:length(stringLocations)
    if strncmpi(varargin{i},'XData',length(varargin{i})) ||...
       strncmpi(varargin{i},'YData',length(varargin{i}))
        error(message('images:imshow:xyDataAndImref','''XData''','''YData'''));
    end
end
