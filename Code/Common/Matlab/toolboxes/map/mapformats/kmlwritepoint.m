...
%
%   KMLWRITEPOINT(FILENAME, LATITUDE, LONGITUDE) writes the latitude and
...
%
%   KMLWRITEPOINT(FILENAME, LATITUDE, LONGITUDE, ALTITUDE) writes the
%   latitude, longitude, and altitude points to disk in KML format. The
%   altitude values are interpreted as 'relativeToSeaLevel'.
%
%   KMLWRITEPOINT(..., Name, Value) specifies name-value pairs that set
...
%          displayed in the viewer as the label for the points. If the
%          value is a string, the name is applied to all points. If the
%          value is a cell array, it is the same size as LAT and LON. If
%          unspecified, Name is set to 'Point N' where N is the point
%          number.
%
%     Description
%          A string or cell array of strings, which specifies the contents
%          to be displayed in the point's description tag(s). The
%          description appears in the description balloon when the user
%          clicks on either the point name in the Google Earth Places panel
%          or clicks the placemark icon in the viewer window. If the value
%          is a string, the description is applied to all points. If the
%          value is a cell array, it is the same size as LAT and LON. Use a
...
%          points. If the value is a cell array, it is the same length as
%          LAT and LON.  If the icon filename is not in the current folder,
%          or in a folder on the MATLAB path, specify a full or relative
%          pathname. The string may be an Internet URL. The URL must
%          include the protocol type.
%
%     IconScale
...
%
%     Color
%          A MATLAB Color Specification (ColorSpec) for the icons (a
%          string, cell array of strings, or numeric array with values
%          between 0 and 1). If the value is a cell array, it has the same
%          length as LAT and LON. If the value is a numeric array, it is
%          size M-by-3 where M is the length of LAT or LON.
%
%     AltitudeMode
...
%      'Name', 'Machu Picchu');
%
%   See also KMLWRITE, KMLWRITELINE, SHAPEWRITE
