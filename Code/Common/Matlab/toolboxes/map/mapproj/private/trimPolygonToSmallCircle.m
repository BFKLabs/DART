...
% chosen to comfortably enclose the entire ring. Although points nearly at
% the limits in az were previously snapped to azLimit, it's necessary to
% snap following the wrapping and trimming steps above.
...
for p = 1:numel(cwIndex)
    ...
    for q = 1:numel(indx)
        ...
        if cwPolygonIsNested(k,containingPoly(j),az,rng,first,last)               
            ...
            map.internal.assert(isscalar(shift), ...
                'map:internalProblem:topology', 'Expected SHIFT to be a scalar.')
