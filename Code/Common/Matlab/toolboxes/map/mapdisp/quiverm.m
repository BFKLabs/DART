...
validateattributes(v, {'double','single'}, {'2d'}, 'QUIVERM', 'V', 4)

if length(lat) == 1 && size(lat,1) ~= size(u,1)
    error('map:quiverm:invalidLat', ...
        'Lat vector input must have row dimension of u.')
end

if length(lon) == 1 && size(lon,1) ~= size(u,2)
    error('map:quiverm:invalidLon', ...
