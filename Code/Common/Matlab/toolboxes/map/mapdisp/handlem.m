...
%   exactly match the string TAGSTR.
%
...
%   empty. If the the current axes or AXESH is not a map axes,
%   handlem('map') will error.
%
%  See also FINDOBJ
...
if ~ischar(object)
    ...
    % Ensure string input
    validateattributes(object,{'char'},{'row'},'','',1)
    ...
    switch method
       % If we reach this block, then children is non-empty.        
        ...
        case 'strmatch'
            ...
            h = children(strncmp(tag,tags,length(tag)));
            
        case 'findstr'
            tags = get(children,'Tag');
            if iscell(tags)
                % Multiple children
                h = children(~cellfun(@isempty,strfind(tags,tag)));
            else
                % Only child
                h = children(~isempty(strfind(tags,tag)));
            end
    end
end
