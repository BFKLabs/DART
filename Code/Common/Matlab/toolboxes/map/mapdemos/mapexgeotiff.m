...
%
% This example shows how to write data referenced to standard geographic
% and projected coordinate systems to GeoTIFF files, using |geotiffwrite|.
% The Tagged-Image File Format (TIFF) has emerged as a popular format to
% store raster data. The GeoTIFF specification defines a set of TIFF tags
% that describe "Cartographic" information associated with the TIFF raster
% data. Using these tags, geolocated imagery or raster grids with
% coordinates referenced to a Geographic Coordinate System (latitude and
% longitude) or a (planar) Projected Coordinate System can be stored in a
% GeoTIFF file.
%
% Copyright 2010-2013 The MathWorks, Inc.

%% Setup: Define a Data Folder and Filename Utility Function
% This example creates several temporary GeoTIFF files and uses the variable
...
% The contents of |datadir| are deleted at the end of the example.
...
subR.XWorldLimits = sort(xlim);
subR.YWorldLimits = sort(ylim);
...
%%
% Convert the referencing matrix to a map raster reference object. Use
% raster interpretation 'postings', because it's a USGS DEM. This
...
%%
% Uncompress the DEM data file to the |datadir| folder and read all the
% data from the file.
...
%%
% Write the data to a GeoTIFF file with |geotiffwrite|. For comparison,
...
%%
% When you read the file using |imread| the data values and class type are
% preserved. You can see that the data values in the TIFF file are not
% preserved.
...
% printing a table of the values.
...
% RGB image to an indexed image, by printing a table of values.
