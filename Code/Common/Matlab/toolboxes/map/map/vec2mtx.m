...
[lat,lon] = doInterpm(lat, lon, 0.9/sampleDensity(R), 'lin');
...
if (nargin == 3) || (nargin == 4 && fill)
    ...
    latlim = deriveLatitudeLimits(lat, bufwidth);
    lonlim = deriveLongitudeLimits(lon, bufwidth);
...
function latlim = deriveLatitudeLimits(lat, bufwidth)
...
function lonlim = deriveLongitudeLimits(lon, bufwidth)
...
lonlim = conditionLongitudeLimits(lonlim);

%-----------------------------------------------------------------------

function lonlim = conditionLongitudeLimits(lonlim)
%conditionLongitudeLimits Adjust wrapping of longitude-limit vector
%
%   LONLIM = conditionLongitudeLimits(LONLIM) adds or subtracts multiples of 360
...
%   lonlim(2), conditionLongitudeLimits assumes that the intention is to span a full
