...
%   meridian lable, parallel or meridian line (graticule), or map frame)
...
%   Input Arguments
%   ---------------
%   h -- Handle to a map axes to an object in a map axes. 
...
if ~isempty(ax) && ~isappdata(ax, 'HighLevelFunction')
    ...
    % Set the stacking order for the decorations. The map data will end up
    % above the frame and below everything else.
    %   It's OK if not all objects are present; uistack accepts empty input
    %   without error and performs no operation in such cases.
    % uistack(hFrame,'bottom')
    % uistack(hGraticule,'top')
    % uistack(hGraticuleLabels,'top')
    % uistack([hScaleRuler; hNorthArrow],'top')
    uistack([hNorthArrow; hScaleRuler; hGraticuleLabels; hGraticule],'top')
    
    % Stack all text map data objects above everything else. (All text
    % excluding the meridian and parallel labels, that is.)
    t = setdiff(findobj(ax,'Type','text'),hGraticuleLabels,'stable');
    try
        uistack(t,'top')
    catch exception
        if strcmp(exception.identifier,'MATLAB:uistack:ParentMustBeSame')
            arrayfun(@(ht) uistack(ht,'top'),t)
        else
            rethrow(exception)
        end
    end
