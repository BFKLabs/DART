classdef DynamicVector
    ...
    properties (Access = public)
        %Geometry - String defining type of geometry
        %
        %   Geometry is a string defining the type of geometry.
        Geometry = 'point'
    ...
    methods  
        ...
        %
        %   See also map.internal.DynamicVector/end.
        ...
        %   string, FIELDNAME, from the dynamic vector, V.
        %
        ...
        %   string, PROPNAME, from the dynamic vector, V.
        %
        %   V = rmprop(V, PROPNAMES) removes all the properties specified
        %   in the cell array, PROPNAMES, from the dynamic vector, V. If
        %   PROPNAMES contains a coordinate property an error is issued.
        %
        %   See also map.internal.DynamicVector/fieldnames, map.internal.DynamicVector/rmprop.
   
            try
                validateattributes(names, {'char', 'cell'}, {},  ...
                    'rmprop', 'PROPNAME');
            catch e
                throwAsCaller(e);
            end
            ...
            % Remove entries that are not strings and not unique.
            names = unique(names,'legacy');
        ...
        %
        %   TF = isprop(V, NAME) returns true if the value specified by
        %   the string, NAME, is a property of the dynamic vector, V.
        %
        %   TF = isprop(V, NAMES) returns true for each element of the cell
        %   array, NAMES, that is a property of V. TF is a logical array
        ...
        %   the string, NAME, is a dynamic property of the dynamic vector,
        %   V.
        %
        %   TF = isfield(V, NAME) returns true for each element of the cell
        %   array, NAMES, that is a dynamic property of V. TF is a logical
        %   array the same size as NAMES.
        %
    ...
    methods (Hidden = true)
        ...
        %   are replaced.
        %
        %   IN may be a numeric array, a logical array, a string, or a cell
        %   array of numeric, logical, or string values.
        %
    ...
    methods (Access = protected, Hidden = true)
        ...
        function validatesubsindex(self, s, k)
         % Validate subs index values: s.subs
            ...
            switch(s(k).type)
                case '.'
                    % obj.(s(1).subs) must contain a string. A cell array
                    % of strings is not permitted.
                    if ~ischar(s(k).subs), ...
        ...
        % replaced with standard linear indexing. If s.subs{2} is a string
        % or cell array, then remove all other fields from self.
        ...
        % a property name.
            ...
            if numout <= 1 && ~isempty(varargout) && iscell(varargout{1}) ...
                ...
                % are all logical or numeric or a scalar string.
        ...
        % Protected implementation for subsasgn. s contains strings for
        % property indexing. For example: self(2,'Name') = self(1,'Name')
            ...
            if ~isempty(in) || ischar(in)  % permit ''
                ...
                if usingCellNotation
                    ...
                    if ~ischar(in)
                        ...
                        % example: obj.PropertyName{5} = 'string', where
        ...
        % Return list of properties of self and the string,
        ...
        % The fields of LABELS contains either a string value or empty,
        % depending on whether a label is present for that field name.
...
elseif isCellType(S,name)
    % The elements in the field are strings or mixed type or non-scalar.
    % If the elements are column vectors, then transpose them.
    values = {S(:).(name)};
else
    values = [S(:).(name)];
end
...
% cell array of strings containing a list of field names that may be
% replaced. NEW is a cell array of strings containing new field names. OLD
% is a cell array of strings containing old field names. NEW and OLD must
% match in size.
%
...
if iscell(in)
    ...
    % Cell input must be a vector and all inputs must be strings. Inputs
    % in the cell array must be row vectors.
    ...
    % The input must be a string, numeric, or logical vector.
    validateattributes(in, {'numeric','char','logical'}, ...
        {'vector'}, classname);
end
...
% The last index may be a string or cell array of strings.

subs = s(1).subs;
if ~isempty(subs)
    % Determine if the first 1:N indices are valid. indices must be real,
    % positive, integer-valued numbers, logicals, or the string, ':'
    ...
    % string or cell array of strings, but only if it is referenced as the
    % last index and not the first and only index.
    subs = subs(end);
    v = subs{:};
    if ~(isIndex(v) || ...
    ...
    if iscell(v)
        % Last index must be a cell array of strings.
        if ~iscellstr(v), ...
...
% If V is a cell array containing [] values, then replace them with string
% '' values.
...
% scalar string value. Otherwise return the cell array.
...
% Obtain character string for display of map.internal.DynamicVector object
% using builtin. FIELDNAME is the field of S to convert to a string.
% FIELDSTR is the FIELDNAME of S converted to a string.

% Create a structure containing only the fieldname of S
printS = struct();
printS.(fieldname) = S.(fieldname); %#ok<STRNU>

% Obtain a character string representation using builtin.
...
% Each fieldname in INFO is either [] or contains a label string. FIRSTLINE
% is a string containing the first line to print.
