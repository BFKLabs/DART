        ...
        function h = ContourGroup(ax, Z, R, levels)
            ...
            if nargin ~= 0
                ...
                if matlab.graphics.internal.isGraphicsVersion1
                    h.FillColormap = jet(n+1);
                    h.LineColormap = jet(n);
                else
                    h.FillColormap = parula(n+1);
                    h.LineColormap = parula(n);
                end
...
% object h by scaling cmap, the colormap of the ancestral figure (relative
% to the color limits of the ancestral axes, if CLimMode is 'manual'). The
% result will be to assign one color per level to h.LineColormap and one
% color per interval to h.FillColormap. (Given n levels there are n + 1
% intervals.)
...
if ~isempty(hLine)
    ...
    n = numel(hLine);
    for k = n:-1:1
        p(k) = oneVertexPatch(hLine(k));
        
        % Push the new patch to the bottom in the list of axes
        % children, to keep it hidden under a line.
        c = get(ax,'Children');
        c = circshift(c,-1);
        set(ax,'Children',c)
    end
...
function p = oneVertexPatch(hLine)
...
% figure's colormap or axes' color limits.

f  = ancestor(h.HGGroup,'figure');
ax = ancestor(h.HGGroup,'axes');

if ~isempty(f) && ~isempty(ax)
    fListener  = addlistener(f,'Colormap','PostSet', @setColormapsCallback);
    axListener = addlistener(ax,'CLim','PostSet', @setColormapsCallback);
    ...
    function setColormapsCallback(~,~)
        if isvalid(h)
            g = h.HGGroup;
            if ishghandle(g) && strcmp(get(g,'BeingDeleted'),'off')
                f  = ancestor(h.HGGroup,'figure');
                if ~isempty(f)
                    cmap = get(f,'Colormap');
                    setContourGroupColormaps(h, cmap);
    ...
    function newParentCallback(~, ~)
        if strcmp(get(ax,'BeingDeleted'),'off') && isvalid(h)
            ...
            if ~isempty(f)
                ...
                % Start over again if the hggroup is re-parented to a new axes
                % or if the axes is re-parented to a new figure.
                delete(fListener)
                delete(axListener)
