...
%    $Revision: 1.11.2.7 $  $Date: 2008/06/16 16:34:42 $
...
for j = 1:length(propinfo)
   ...
   % G300754 If input type is the incorrect spelling of PseudoDifferential
   % correct it now.
   if strcmp(objtype, 'analoginput')
       if isfield(objinfo{1},'InputType')
           if strcmp(objinfo{1}.InputType, 'PsuedoDifferential') % Incorrect spelling
               objinfo{1}.InputType = 'PseudoDifferential'; % Correct spelling
           end
       end
   end
