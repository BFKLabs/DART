...
%    $Revision: 1.12.2.7 $  $Date: 2007/06/18 22:03:59 $
...
if isfield(recreate_parent_prop, 'TriggerType') &&...
   ...
   set(parent, 'TriggerType', recreate_parent_prop.TriggerType);
   set(parent, 'TriggerCondition', recreate_parent_prop.TriggerCondition);
   recreate_parent_pvpair = localRmField(recreate_parent_pvpair, ...
...
for i = 1:length(recreate_parent_pvpair)
   set(parent, recreate_parent_pvpair{i,1}, recreate_parent_pvpair{i,2});
end
   
% Reset the Handle property.
set(parent, 'Handle', handle);
...
if ~isempty(recreate_child_prop)
   ...
   for i = 1:length(recreate_child_prop)
      ...
      case 'Line'
         % Use UDD addchannel directly, because M addline forces user
         % to choose a Direction for the line.         
         parentudd = privateMATLABToUDD(parent);
         childudd = addchannel(parentudd, hwchan(i), recreate_child_prop(i).Port);
         newchild = privateUDDToMATLAB(childudd);
      end
      ...
      for j = 1:length(child_pvpair)
         set(newchild, child_pvpair{j,1}, child_pvpair{j,2});
      end
   end
end
...
if ~isempty(triggerChannel)
   ...
   set(parent, 'TriggerChannel', child([triggerChannel{:}]));
end
