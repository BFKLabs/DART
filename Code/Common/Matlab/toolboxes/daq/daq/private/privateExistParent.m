...
%    $Revision: 1.14.2.9 $  $Date: 2008/12/04 22:26:29 $
...
engine_parent_prop = get(engine_parent);
...
engine_child = get(engine_parent, child_name);
...
if isequal(engine_parent_prop_temp, recreate_parent_prop_temp) &&...
   ...
   % Reset the object to its not-running state.
   ...
   return;
    ...
    % This function is called only from load therefore the warning id will
    % reflect the load command rather than this private function.
   objectStoppedWarning1 = 'An existing Data Acquisition object has been stopped and its properties';
   objectStoppedWarning2 = 'have been updated to match the properties of an object being loaded.';
   objectStoppedID = 'daq:load:objstopped';
   propertyOverwriteWarning1 = 'The properties of an existing Data Acquisition object have been updated';
   propertyOverwriteWarning2 = 'to match the properties of an object being loaded.';
   objectOverwriteID = 'daq:load:objoverwrite';
   
   if isfield(engine_parent_prop, 'Running') &&...
         strcmpi(engine_parent_prop.Running, 'on')
      warning(objectStoppedID, '%s\n%s',objectStoppedWarning1, objectStoppedWarning2);
      stop(engine_parent);
   elseif iswarned(1) == 1 && all(iswarned)
      warning(objectOverwriteID, '%s\n%s',propertyOverwriteWarning1, propertyOverwriteWarning2);
   ...
   if isfield(recreate_parent_prop, 'InputType')
      if strcmp(recreate_parent_prop.InputType, get(engine_parent, 'InputType'))
   ...
   uddobj = daqgetfield(engine_parent,'uddobject');
   reset(uddobj);
   ...
   for i = 1:length(recreate_parent_pvpair)
      set(engine_parent, recreate_parent_pvpair{i,1}, recreate_parent_pvpair{i,2});
   end
   ...
   if ~isequal(engine_child_prop, recreate_child_prop)
      ...
      if ~isempty(recreate_child_prop)
         ...
         if ~isempty(engine_child) 
            ...
            % Get access to the engine_child's uddobject array.
            ...
            for j = 1:min(length(engine_child),length(recreate_child_prop))
               ...
               % Create the object.
               temp = feval(classname,engine_child_struct.uddobject(j));
               
               % Set the new object's properties.
               for k = 1:length(recreate_child_pvpair)
                  set(temp, recreate_child_pvpair{k,1}, recreate_child_pvpair{k,2});
               end
            end
         end
      end
      ...
      if diffchild >= 1 
         ...
         for j = 1:diffchild
            ...
            case 'Line'
               addedchild = feval(child_fcn, engine_parent, hwid(j), recreate_child_prop(j).Port, recreate_child_prop(j).Direction);
            end
            ...
            for k = 1:length(recreate_child_pvpair)
               set(addedchild, recreate_child_pvpair{k,1}, recreate_child_pvpair{k,2});
            end
         end
         
         channelAddedWarning1 = ['An existing Data Acquisition object has had ' lower(child_name) 's added to match'];
         channelAddedWarning2 = 'the properties of an object being loaded.';
         channelAddedID       = 'daq:load:channeladded';
   
         warning(channelAddedID, '%s\n%s',channelAddedWarning1, channelAddedWarning2);
      elseif diffchild <= -1
         ...
         channelDeletedWarning1 = ['An existing Data Acquisition object has had ' lower(child_name) 's deleted to match'];
         channelDeletedWarning2 = 'the properties of an object being loaded.';
         channelDeletedID = 'daq:load:channeldeleted';
         warning(channelDeletedID, '%s\n%s',channelDeletedWarning1, channelDeletedWarning2);
      end
   end
   
   % Reset the TriggerChannel property.
   if ~isempty(triggerChannel);
      child = get(engine_parent, child_name);
      set(engine_parent, 'TriggerChannel', child([triggerChannel{:}]));
   end
