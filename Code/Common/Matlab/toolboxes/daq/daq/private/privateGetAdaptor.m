...
%    $Revision: 1.8.2.6 $  $Date: 2011/10/31 06:07:10 $
...
if strcmp(subsystem,'analoginput')
    ...
    for i = 1:length(alladaptor)  
        if ~strcmp(alladaptor{i},'winsound')
            ...
            pinfo = propinfo(obj1, 'InputType');
            for itype = 1:length(pinfo.ConstraintValue)
                adaptorStr = [adaptorStr; {[alladaptor{i} ' ' id{i} ' (' pinfo.ConstraintValue{itype} ')']}];   
            end
...
if ~isempty(alladaptor)
   ...
   try
      ...
      if ~strcmp(subsystem, 'digitalio')
         ...
         % Determine the channel IDs.
         % Construct the channel names - based on the hardware ids.
         if strcmp(lower(alladaptor{1}), 'winsound')
            names = {'Channel1 (Left)';'Channel2 (Right)'};
         else
            if strcmp(subsystem, 'analoginput')
                % Set up the channel IDs. 
                pinfo=propinfo(obj,'InputType');
                if (strcmpi(pinfo.DefaultValue, 'Differential'))
                    % If the default InputType is Differential use the
                    % differential ids.
                    channelIDs = objInfo.DifferentialIDs;
                else
                    % For all other types ('NonReferencedSingleEnded'
                    % 'SingleEnded', 'PseudoDifferential') use the
                    % single ended ids. This assumes that any new
                    % InputTypes added will always have SingleEndedIDs.
