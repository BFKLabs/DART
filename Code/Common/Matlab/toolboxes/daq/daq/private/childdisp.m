...
%    $Revision: 1.13.2.9 $  $Date: 2011/10/31 06:07:07 $
...
if nargin==4,
   ...
   children = get(children,info.child); % MATLAB layer get
   ...
   for i = 1:length(children)
      try
         s=struct(get(children(i),'parent'));
         info=s.info;
         break;
      catch
      end
...
% Get the structure information of the channels:
...
for i=1:nprops,
   ...
   for j=1:ValSets,
      % If the handle is not valid, do nothing otherwise get the property 
      % information.
      try
         ValsGrp{j,i} = get(Hstruct.uddobject(j),childprops{i});
