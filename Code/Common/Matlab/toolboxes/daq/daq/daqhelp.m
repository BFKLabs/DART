...
daqroot = which(['daqmex.' mexext], '-all');
...
case 1
   ...
   if isdir && isempty(ext)
      ...
      elseif strcmp(name(1), '@')
         ...
         fullpath{1} = [daqroot filesep '@' name filesep 'Contents.m'];
         fullpath{2} = [daqroot filesep '@' name filesep name '.m'];
      end
   end
   
    % Special check for CLEAR, LOAD, and SAVE help.
    % The help for these are privateXYZ.M files.
    indices = strmatch(name, {'clear', 'load', 'save'});
    if ~isempty(indices)
        name = ['private' name];
    end
   ...
   if ~isempty(ext) && ~strcmp(ext, '.m') 
      try
         out = daqgate('privatePropDesc', ext(2:end),name);
      catch e
...
if isempty(fullpath{1}) && (nargin == 2 || (nargin == 1 && isempty(path)))
    ...
    try
        out = daqgate('privatePropDesc',name,path);
    catch e
        if isempty(path)
    ...
    if ~isempty(device)
        try
            ...
            error('daq:daqhelp:invalidproperty','The ''%s'' property is invalid for ''%s'' objects.', name, driver);
        end;
    end
    if nargout == 0
        fprintf('\n');
...
elseif isempty(fullpath{1}) && nargin == 1
   ...
   % If the name is daqmex, the fullpath may be to the mex file.  Therefore
   % need to replace the mex extension with a .m so the correct help is called.
   if strcmp(name, 'daqmex')
      fullpath = strrep(fullpath, ['.' mexext], '.m');
   end
...
if any(strcmpi(name, {'analoginput', 'analogoutput', 'digitalio'}))
   out = 'daqdevice';
   return;
elseif any(strcmpi(name, {'aichannel', 'aochannel', 'dioline'}))
   out = 'daqchild';
end
