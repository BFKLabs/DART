classdef (Hidden) AnalogInputVoltageChannel < daq.AnalogInputChannel
    ...
    methods(Hidden)
        function obj = AnalogInputVoltageChannel(session,deviceInfo,id)
            %AnalogInputVoltageChannel All settings & operations for an analog input voltage channel added to a session.
            ...
            obj@daq.AnalogInputChannel(session,deviceInfo,id,'Volts');
    ...
    methods (Access = protected)
        ...
        function measurementTypeDisplayText = getMeasurementTypeDisplayHook(obj) %#ok<MANU>
            ...
            measurementTypeDisplayText = 'Voltage';
        end
    end
    ...
    properties(Hidden)
        ...
        %TerminalConfigInfo The current input type (single ended/differential) as an
        %enumeration
        InputType;
        TerminalConfigInfo;
    end
    ...
    methods
        ...
        function set.TerminalConfig(obj,value)
            try
                ...
                try
                    ...
                    obj.channelPropertyBeingChanged('TerminalConfigInfo',newValue)
                    % Keep the hidden and visible properties in sync
                    obj.TerminalConfigInfo = char(newValue);
                    obj.InputType = char(newValue);
                    obj.TerminalConfigInfo = newValue;
                    obj.PropertyChangeInProgress = false;
                catch e
        ...
        function set.InputType(obj,value)
            try
                ...
                try
                    obj.PropertyChangeInProgress = true;
                    newValue = daq.InputType.setValue(value);
                    % Really, we only change the underlying hidden property
                    % with the enumeration -- and we only report that change
                    obj.channelPropertyBeingChanged('TerminalConfigInfo',newValue)
                    % Keep the hidden and visible properties in sync
    ...
    methods(Hidden)
        function channelDescriptionText = getChannelDescriptionHook(obj) %#ok<MANU>
            ...
            % operation
            channelDescriptionText = 'analog input voltage channel';
        end
    end
end
