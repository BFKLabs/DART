classdef (Hidden) AudioChannel < daq.Channel
    %AudioChannel All settings & operations for an audio channel added to a session.
    %    This class should be specialized for each class of possible audio channel. 
    %    Vendors may further specialize those to implement additional behaviors.
    ...
    methods(Hidden)
        function obj = AudioChannel(subsystemType, session, deviceInfo, id, supportedRanges)
            %AudioChannel All settings & operations for an audio channel added to a session.
            %    AudioChannel(SUBSYSTEMTYPE,SESSION,DEVICEINFO,ID,SUPPORTEDRANGES)
            %    Create an audio channel with SUBSYSTEMTYPE, SESSION,
            %    DEVICEINFO, and ID (see daq.Channel), with an vector of
            %    ranges SUPPORTEDRANGES of type daq.Range.
    ...
    methods (Access = protected)
        ...
        function measurementTypeDisplayText = getMeasurementTypeDisplayHook(obj) %#ok<MANU>
            ...
            measurementTypeDisplayText = 'Audio';
        end		
