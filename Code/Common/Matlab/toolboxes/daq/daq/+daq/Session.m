classdef Session < daq.internal.BaseClass
    ...
    methods
        function [el] = addlistener(varargin)
            ...
            if nargin == 3 && strcmp(varargin{2},'DataAvailable')
    ...
    methods(Sealed)
        function [channel,index] = addAnalogInputChannel(obj,varargin)
            ...
            try
                ...
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.AnalogInput,false,varargin{:});
            catch e
        ...
        function [channel,index] = addAnalogOutputChannel(obj,varargin)
            ...
            try
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.AnalogOutput,false,varargin{:});
            catch e
                % Rethrow any errors as caller, removing the long stack of
                % errors -- capture the full exception in the cause field
                % if FullDebug option is set.
        ...
        function [channel,index] = addDigitalChannel(obj,varargin)
            ...
            try
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.DigitalIO,false,varargin{:});
            catch e
                % Rethrow any errors as caller, removing the long stack of
                % errors -- capture the full exception in the cause field
                % if FullDebug option is set.
        ...
        function [channel,index] = addCounterInputChannel(obj,varargin)
            ...
            try
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.CounterInput,false,varargin{:});
            catch e
        ...
        function [channel,index] = addCounterOutputChannel(obj,varargin)
            ...
            try
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.CounterOutput,false,varargin{:});
            catch e
                % Rethrow any errors as caller, removing the long stack of
                % errors -- capture the full exception in the cause field
                % if FullDebug option is set.
        ...
        function [channel,index] = addAudioInputChannel(obj,varargin)
            ...
            try
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.AudioInput, false, varargin{:} );
            catch e
                % Rethrow any errors as caller, removing the long stack of
                % errors -- capture the full exception in the cause field
                % if FullDebug option is set.
        ...
        function [channel,index] = addAudioOutputChannel(obj,varargin)
            ...
            try
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.AudioOutput, false, varargin{:} );
            catch e
                % Rethrow any errors as caller, removing the long stack of
                % errors -- capture the full exception in the cause field
                % if FullDebug option is set.
    ...
    methods(Sealed, Hidden)
        function [channel,index] = addDigitalGroup(obj,varargin)
            % Add a digital group of lines to the session
            %
            % addDigitalGroup(DEVICEID,CHANNELID,MEASUREMENTTYPE) adds
            % a group of lines, specified by CHANNELID on device DEVICEID to
            % the session.  CHANNELID may be a string, or cell array of
            % strings. MEASUREMENTTYPE may be 'Input' or 'Output'.
            %
            % addDigitalGroup(DEVICEID,CHANNELID,MEASUREMENTTYPE,...)
            % additional parameters can be specified, and are vendor
            % specific.  See the vendor specific documentation for details.
            %
            % [CHANNEL,INDEX] = addDigitalGroup(...)
            % addDigitalGroup optionally returns CHANNEL, which is an
            % object representing the channel that was added.  It also
            % returns INDEX, which is the INDEX into the Channels array
            % where the channel was added.
            %
            % Example:
            %    s = daq.createSession('ni');
            %    s.addDigitalGroup('cDAQ1Mod2', 'Port0/Line0:3', 'OutputOnly');
            %    s.outputSingleScan(12);
            %
            % See also addDigitalChannel, removeChannel,
            % daq.getDevices
            try
                if nargin < 4
                    obj.localizedError('daq:Session:notEnoughArgForAddChannel')
                ...
                % addChannelInternal to do the real work
                [channel,index] = obj.addChannelInternal(...
                    daq.internal.SubsystemType.DigitalIO,true,varargin{:});
            catch e
                % Rethrow any errors as caller, removing the long stack of
                % errors -- capture the full exception in the cause field
                % if FullDebug option is set.
    ...
    methods(Sealed)
        ...
        function stop(obj,noWait)
            ...
            %
            % If there is data that has been acquired, but the
            % DataAvailable event has not fired, it will fire to deliver
            % any data that has not been delivered.
            %
    ...
    events
        ...
        % Example:
        %
        % See also: daq.DataAvailableInfo, handle.addlistener, startBackground
        DataAvailable
        ...
        % Example:
        %
        % See also: handle.addlistener, startBackground
        DataRequired
        
        % ErrorOccurred is fired when a hardware error occurs on a device.
        %
        % Listeners on the ErrorOccurred event of the daq.Session object will
        % receive a call to their listener function with a
        % daq.ErrorOccurredInfo object as the second parameter (EVENTINFO).
        %
        % Example:
        %
        % See also daq.ErrorOccurredInfo, handle.addlistener, startBackground
        ErrorOccurred
    end
    ...
    methods(Hidden)
        function obj = Session(vendorInfo,initialRate)
            ...
            if ~isempty(obj.RateLimitInfo)
                ...
                initialRate = max(initialRate,obj.RateLimitInfo.Min);
                initialRate = min(initialRate,obj.RateLimitInfo.Max);
            end
            
            obj.Rate = initialRate;
    ...
    methods
        ...
        function set.DurationInSeconds(obj,newValue)
            try
                ...
                try
                    ...
                    % duration
                    numberOfScans = uint64(ceil(newValue * obj.Rate));
                    ...
                    if numberOfScans < obj.MinimumNumberOfScans
                        ...
                        obj.localizedWarning('daq:Session:durationIncreased',num2str(newValue))
                    end
        ...
        function set.IsContinuous(obj,newValue)
            try
                ...
                obj.sessionPropertyBeingChangedHook('IsContinuous',newValue)
                if logical(newValue)
    ...
    % Hidden public methods, which are used to provide transition from
    % version 2 API.
    methods(Hidden)
        function addchannel(obj,varargin)
            obj.getLocalizedException('daq:Session:v2addChannelCommand').throwAsCaller();
        end
        
        function start(obj,varargin)
            obj.getLocalizedException('daq:Session:v2startCommand').throwAsCaller();
        end
        
        function getdata(obj,varargin)
            obj.getLocalizedException('daq:Session:v2getdataCommand').throwAsCaller();
        end
        
        function putdata(obj,varargin)
            obj.getLocalizedException('daq:Session:v2putdataCommand').throwAsCaller();
        end
        
        function getsample(obj,varargin)
            obj.getLocalizedException('daq:Session:v2getsampleCommand').throwAsCaller();
        end
        
        function putsample(obj,varargin)
            obj.getLocalizedException('daq:Session:v2putsampleCommand').throwAsCaller();
        end
        
        function getvalue(obj,varargin)
            obj.getLocalizedException('daq:Session:v2getvalueCommand').throwAsCaller();
        end
        
        function putvalue(obj,varargin)
            obj.getLocalizedException('daq:Session:v2putvalueCommand').throwAsCaller();
        end
    end
    ...
    methods (Sealed, Hidden)
        ...
        function [newChannels,indexNewChannels] = doAddChannel(...
                obj,subsystem,isGroup,deviceID,channelID,measurementType,varargin)
            ...
            if isempty(deviceID) || ~ischar(deviceID)
            ...
            if isempty(channelID) ||...
                    ~isvector(channelID) ||...
                    ~(daq.internal.isNumericNum(channelID) || ischar(channelID) || iscellstr(channelID)) ||...
                    (isnumeric(channelID) && any (channelID < 0))
            ...
            if subsystem == daq.internal.SubsystemType.DigitalIO
                ...
                if isGroup
                    if numel(channels) > 52
                        obj.localizedError('daq:Session:digitalGroupExceeds52Lines')
                    end
                    [~, ia] = unique(channels, 'first');
                    repeatChannels = channels(~ismember(1:numel(channels), ia));
                    if ~isempty(repeatChannels)
                        obj.localizedError('daq:Session:digitalGroupRepeatChannelsSpecified',...
                            obj.renderCellArrayOfStringsToString(repeatChannels, ''', '''));
                    end
                end
            end
            ...
            if ~isempty(obj.RateLimitInfo)
                ...
                % session.  Since the channels have to be added from the
                % same device, we only need to check index 1
                newChannelRateLimit = newChannels(1).getSubsystem().RateLimitInfo;
                if newChannelRateLimit.Min > obj.RateLimitInfo.Max ||...
                    ...
                    if (obj.RateLimitInfo.Min == 0) && (obj.RateLimitInfo.Max == 0) ...
                            && any(~[newChannels.OnDemandOperationsSupported])
                        obj.localizedWarning('daq:Session:incompatibleDevices',...
                            obj.renderCellArrayOfStringsToString({newChannels.ID}, ','),...
                            deviceID)
                    end
                end
            end
        ...
        function doRemoveChannel(obj,index)
            ...
            % G642605 If there are no longer any output channels reset
            % the duration to 1 second which will set the NumberOfScans
            % appropriately.
            if obj.Channels.countOutputChannels() == 0 && isa(channelToRemove,'daq.AnalogOutputChannel')
                obj.DurationInSeconds = 1;
            end
        ...
        function flushOutputData(obj,fireWarning)
            ...
            if obj.ScansQueued ~= 0 && fireWarning
        ...
        function [data,triggerTime] = doInputSingleScan(obj)
            numberOfInputChannels = obj.Channels.countInputChannels();
            if numberOfInputChannels == 0
                obj.localizedError('daq:Session:noInputChannels')
            end
            
            if any(~[obj.Channels.OnDemandOperationsSupported])
            ...
            if daq.internal.getOptions().FullDebug
                ...
                if size(data,2) ~= numberOfInputChannels ||...
                        size(data,1) ~= 1 ||...
        ...
        function doOutputSingleScan(obj,dataToOutput)
            %Validate the data
            numberOfOutputChannels = obj.Channels.countOutputChannels();
            ...
            % Validate Data
            if ~ismatrix(dataToOutput) ||...
                    any(any(isnan(dataToOutput))) || any(any(isinf(dataToOutput)))
                obj.localizedError('daq:Session:invalidDataFormat')
            end
            ...
            for idx = 1:numel(obj.Channels)
                isAO = isa(obj.Channels(idx), 'daq.AnalogOutputChannel');
                isDO = isa(obj.Channels(idx), 'daq.DigitalChannel') &&...
                    strcmp(obj.Channels(idx).Direction, char(daq.Direction.Output));
                
                % Increment the output channel counter
                if isAO || isDO
                    outputChannelIndex = outputChannelIndex + 1;
                    dataToOutputCellArray{outputChannelIndex} = ...
                        dataToOutput(outputChannelIndex);
                end
                
                % Validate AO channels
                if isAO
                    if ~isnumeric(dataToOutput(outputChannelIndex))
                        obj.localizedError('daq:Session:invalidDataFormat')
                    end
                end
                
                % Validate DO channels
                if isDO
                    if ~isnumeric(dataToOutput(outputChannelIndex)) &&...
                            ~islogical(dataToOutput(outputChannelIndex))
                        obj.localizedError('daq:Session:invalidDataFormat')
                    end
                    
                    % g880462: Check for negative and non-integer values
                    if dataToOutput(outputChannelIndex) < 0 ||...
                            floor(double(dataToOutput(outputChannelIndex))) ~= dataToOutput(outputChannelIndex)
                        obj.localizedError('daq:Session:digitalChannelInvalidDataFormat')
                    end
                    
                    numLines = obj.Channels(idx).GroupChannelCount;
                    
                    % Doubles cannot safely represent uint64 values > 2^52
                    if isnumeric(dataToOutput) &&...
                            dataToOutput(outputChannelIndex) > 2^52 &&...
                            ~isa(dataToOutput(outputChannelIndex), 'uint64')
                        obj.localizedError('daq:Session:valueRequiresUINT64')
                    end
                    
                    % Convert all logical or numeric data to uint64
                    if islogical(dataToOutput(outputChannelIndex)) ||...
                            isnumeric(dataToOutput(outputChannelIndex))
                        dataToOutputCellArray{outputChannelIndex} = ...
                            uint64(dataToOutput(outputChannelIndex));
                    end
                    
                    % Verify leading bits are all zeros
                    if dataToOutput(outputChannelIndex) >= 2^numLines
                        obj.localizedError('daq:Session:invalidDIOParamOutOfRange', ...
                            num2str(outputChannelIndex), num2str(numLines), num2str(2^numLines-1))
                    end
                end
            end
        ...
        function doRelease(obj,isExplicitCommandByUser)
            ...
            % Call the template method to give the vendor the chance to
            % release resources associated with an operation
            obj.releaseHook();
        end
        ...
        function handleProcessAcquiredData(obj,triggerTime,timestamps,dataAcquired)
            ...
            obj.AcquisitionQueue.writeScansToQueue(dataAcquired,timestamps)
    ...
    methods (Sealed, Access = protected)
        ...
        function sessionPropertyBeingChanged(obj,propertyName,newValue)
            ...
            % change
            obj.sessionPropertyBeingChangedHook(obj,propertyName,newValue)
        end
        ...
        function throwUnknownChannelIDError(obj,device,unknownChannelID,validChannelIDs)
            ...
            if ~isa(device,'daq.DeviceInfo') || ~ischar(unknownChannelID) || ~iscellstr(validChannelIDs)
            ...
            obj.localizedError('daq:Session:unknownChannelID',device.ID,unknownChannelID,validChannelIDs)
        end
    end
    ...
    methods(Access = private)
        ...
        function configureAcquisitionBuffersForStart(obj)
            ...
            % Initialize the queue containing acquired data and timestamps.
            % Choose an initial capacity of 10 times the size of the
            % NotifyWhenDataAvailableExceeds value (typically about 1
            % second worth of data)
            obj.AcquisitionQueue = daq.internal.ScanQueue(...
                obj.Channels.countInputChannels(),...
                10 * double(obj.NotifyWhenDataAvailableExceeds));
        end
        ...
        function fireDataAvailableIfNeeded(obj,emptyAcquisitionQueue)
            ...
            if obj.AcquisitionQueue.NumberOfScans == 0
                return
            end
            
            while obj.AcquisitionQueue.NumberOfScans >= obj.NotifyWhenDataAvailableExceeds ||...
                    (emptyAcquisitionQueue && obj.AcquisitionQueue.NumberOfScans > 0)
                
                % Retrieve NotifyWhenDataAvailableExceeds scans, create the
                % DataAvailableInfo object, and fire the event
                [data,timeStamps] = obj.AcquisitionQueue.readScansFromQueue(double(obj.NotifyWhenDataAvailableExceeds));
                
                % Create the DataAvailableInfo object, and fire the event
                dataAvailableInfo = daq.DataAvailableInfo(obj.TriggerTime,data,timeStamps);
                ...
                totalScansAvailable = obj.TotalScansAvailableNotified + uint64(obj.NotifyWhenDataAvailableExceeds);
                ...
                if (totalScansAvailable >= obj.NumberOfScans)
                    ...
                    if scansToNotify > 0
                        dataAvailableInfo = daq.DataAvailableInfo(obj.TriggerTime,data(1:scansToNotify, :),timeStamps(1:scansToNotify, :));
