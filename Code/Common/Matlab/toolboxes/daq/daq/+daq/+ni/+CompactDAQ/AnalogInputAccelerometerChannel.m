classdef (Hidden) AnalogInputAccelerometerChannel < daq.ni.AnalogInputAccelerometerChannel & daq.ni.CompactDAQ.CommonAIChannelAttrib
    %AnalogInputAccelerometerChannel All settings & operations
    %for an NI CompactDAQ analog input accelerometer channel.
    ...
    methods(Hidden)
        function obj = AnalogInputAccelerometerChannel(session,deviceInfo,channelID)
            %AnalogInputAccelerometerChannel All settings &
            %operations for a CompactDAQ analog input accelerometer channel
            %added to a session.
            %    AnalogInputAccelerometerChannel(SESSION,DEVICEINFO,CHANNELID) Create a
            %    analog channel with SESSION, DEVICEINFO,
            %    and CHANNELID (see daq.Channel)

            % Create the channel to get appropriate defaults
            obj@daq.ni.AnalogInputAccelerometerChannel(session,deviceInfo,channelID);
    ...
    methods(Hidden)
        ...
        function configureTask(obj,taskHandle)
            obj.configureTask@daq.ni.AnalogInputAccelerometerChannel(taskHandle);
    ...
    methods (Sealed, Access = protected)
        ...
        function channelPropertyBeingChangedHook(obj,propertyName,newValue)
            ...
            obj.channelPropertyBeingChangedHook@daq.ni.AnalogInputAccelerometerChannel(propertyName,newValue);
