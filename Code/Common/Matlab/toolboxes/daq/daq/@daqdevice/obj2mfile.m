...
%    saved.  The MAT-file has the same name as the file containing the 
%    object code.  
%
...
%    as the file containing the object code.
%
%    To recreate OBJ, if OBJ is a device object, type the name of the
%    MATLAB file.  To recreate OBJ, if OBJ is a channel or line object, type
%    the name of the MATLAB file with a device object as the only input.
%
%    Example:
...
%    $Revision: 1.5.2.12 $  $Date: 2009/12/31 18:44:46 $
...
% Parse the input.
try
   [syntax,definelist,filename,file,path] = daqgate('getfileinfo',varargin{:});
  matfilename = [filename 'at'];
catch e
   error('daq:obj2mfile:unexpected', e.message)
end
...
try
   for objindex = 1:numObjs
      ...
      % constructed file.  Define if addchannel or addline will be called.
      ...
      if isempty(indexCompare)
         ...
         if createchild
            ...
            if ~isempty(child)
               ...
               fprintf(fid, '\n');
               try
                  daqgate('privatewritechild',fid,syntax,definelist,child,...
...
% Geck 281433 & 284943:  In order to make NIDAQmx and NIDAQ adaptors look like a
% single adaptor, we handle nidaq specially
if(strcmpi(drivername,nidaqmxAdaptorName))
    drivername = nidaqAdaptorName;
end
...
if ~isempty(prop) 
   ...
   if ~isempty(modeindex) || ~isempty(configindex)
      if strcmpi(get(obj,'BufferingMode'), 'Auto')
         % Remove BufferingConfig and list BufferingMode.  
         if ~isempty(configindex)
            prop(configindex) = [];
         end
      elseif strcmpi(get(obj,'BufferingMode'), 'Manual')
...
case 'set'   
   for i = 1:length(prop)
      % Set the UserData property separately since it is being set to 
      % a variable.
...
if ~isempty(index)
   if any(strcmpi(get(obj, 'ChannelSkewMode'), {'Equisample', 'Minimum'}))
      list(index) = [];
   end
end   
...
if ~isempty(skewindex)
   if ~strcmpi(get(obj, 'ChannelSkewMode'), 'Manual')
...
for i = 1:length(readonly_prop)
   index = find(strcmpi(readonly_prop{i}, [prop{:}]));
   if ~isempty(index)
      prop(index)=[];
      index = [];
   end
...
% *************************************************************
% Create the comments for the beginning of the MATLAB file.
function localSetComment(fid,file,class,name,prefix)

fprintf(fid, [ '%%' upper(file) ' MATLAB Code for creating ' prefix ' ' class ' object.\n',...
   '%%    \n',...
   '%%    This is the machine generated representation of ' prefix ' ' class  ' object.\n',...
   '%%    This MATLAB file, ' upper(file) '.M, was generated from the OBJ2MFILE function.\n',...
   ...
   '%%    will have the same name as the MATLAB file but with a .MAT extenstion.  To \n',...
   '%%    recreate this ' class ' object, type the name of the file, ' upper(file) ', \n',...
   '%%    at the MATLAB command prompt.\n',...
   '%%    \n',...
   '%%    The MATLAB file, ' upper(file) '.M and its associated MAT-file, ',...
