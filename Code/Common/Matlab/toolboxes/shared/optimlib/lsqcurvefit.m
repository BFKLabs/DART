...
%   Jacobian option to specify that FUN also returns a second output
%   argument J that is the Jacobian matrix at the point X. If FUN returns a
%   vector F of m components when X has length n, then J is an m-by-n
%   matrix where J(i,j) is the partial derivative of F(i) with respect to
%   x(j). (Note that the Jacobian J is the transpose of the gradient of F.)
%
...
[options, optionFeedback] = prepareOptionsForSolver(options, 'lsqcurvefit');
