...
if isempty(objIMAQ) && isempty(objDACInfo0) 
    ...
    % (in Test Files) and set that as the image acquisition object
    if isTest
        a = load('TestStack');
        objIMAQ = a.I;
    end
...
% global variables
set(handles.figFlyRecord,'Visible','off'); pause(0.05);
...
if (isempty(exptType))  
    ...
    set(handles.figFlyRecord,'visible','on')
    return
else
    % sets the data structs into the GUI
    if (isTest)
...
if (runTrack)
    ...
    set(handles.menuFile,'enable','off')
    set(handles.menuAdaptors,'enable','off') 
    set(handles.menuCalibTrack,'enable','off')   
    set(handles.checkShowMarkers,'enable','on','value',1)   
    ...
    set(handles.menuFile,'enable','on')
    set(handles.menuAdaptors,'enable','on')   
    set(handles.menuCalibTrack,'enable','on')
    set(handles.checkShowMarkers,'enable','off','value',0)
...
set(handles.toggleStartTracking,'enable','off')
...
if (isfield(handles,'menuRTTrack'))
    if (strcmp(get(handles.menuRTTrack,'visible'),'on'))
        set(handles.menuRTTrack,'enable','off')
        set(handles.toggleStartTracking,'enable','off')    
    end
end
...
% enables the real-time tracking menu item (if it exists)
if (isfield(handles,'menuRTTrack'))
    set(handles.menuRTTrack,'enable','on')
end
