classdef RunExptObj < handle
    ...
    methods
        ...
        if (ishandle(obj.hProg)) 
            set(obj.hProg.hFig,'WindowStyle','normal'); 
        end
        ...
        if obj.isRT
            ...
            set(hCheck,'value',0,'enable','off')   
            hMark = getappdata(obj.hMain,'hMark');
            for i = 1:length(hMark)
                cellfun(@(x)(set(x,'visible','off')),hMark{i})
            end 
