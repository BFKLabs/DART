    ...
    case ('Ch2Tube') % case is connecting a channel to a fly
        ...
        if (nwVal == 0)
            ...
            if (any(isMatch))
                ...
                % determines 
                [C2T,nFlyR] = deal(rtP.Stim.C2T,getFlyCount(iMov,1));
...
if (~isempty(hObj))
    cellfun(@(x)(set(x,'enable',eStr{1+pVal})),hObj);
end
...
switch (get(get(handles.panelStimType,'SelectedObject'),'tag'))
    case ('radioContStim') % case is connecting channel to apparatus                
        set(handles.textFixedDur,'enable','off') 
        set(handles.editFixedDur,'enable','off') 
        rtP.Stim.sType = 'Cont';                
    case ('radioSingleStim') % case is connecting channel to tube        
        set(handles.editFixedDur,'enable','on') 
        set(handles.textFixedDur,'enable','on') 
...
switch (get(hGUI,'tag'))
    case ('figFlyRecord')
        ...
        if (~isempty(rtP.Stim) && isTracking)
            set(hGUIMain.menuExptProto,'enable','on')
        else
            set(hGUIMain.menuExptProto,'enable','off')
        end  
    case ('figFlyTrack')
        isRTPChange = true;
end
...
if (strcmp(get(handles.buttonUpdate,'enable'),'on'))
    uChoice = questdlg('Do you want to update the tracking parameters?',...
                       'Update Tracking Parameters?','Yes','No','Cancel','Yes');
    switch (uChoice)
...
if (~iMov.isSet)
    ...
    set(handles.tableCombActivity,'visible','off') 
    setPanelProps(handles.panelRegionActivity,'off')       
    return
end
