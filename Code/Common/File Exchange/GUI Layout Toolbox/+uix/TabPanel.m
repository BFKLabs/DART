classdef TabPanel < uix.Container & uix.mixin.Panel
    ...
    methods
        ...
        function set.TabWidth( obj, value )
            ...
            % Check
            assert( all(isa( value, 'double' )) && ...
                all(isreal( value )) && all(~isinf( value )) && ...
                all(~isnan( value )) && all(value ~= 0), ...
                'uix:InvalidPropertyValue', ...
...
%                 tabs(ii).Position = [1 + (ii-1) * tW + ii * dW, tY, tW, tH];
                tabs(ii).Position = [1 + wOfs + ii * dW, tY, tW(ii), tH];
                wOfs = wOfs + tW(ii);
            end
    ...
    methods( Access = private )
        ...
        function redrawTabs( obj )
            ...
            for ii = 1:d
                ...
                % Insert
                allCData(1:tH,(ii-1)*(dW)+wOfs+(1:dW),:) = cData; % center
                if ii > 1 % extend left under transparent uicontrol edge
                    allCData(1:tH,(ii-1)*(dW)+wOfs,:) = cData(:,1,:);
                end
                if ii < d % extend right under transparent uicontrol edge
                    allCData(1:tH,(ii-1)*(dW)+wOfs+dW+1,:) = cData(:,end,:);
                    wOfs = wOfs + obj.Tabs(ii).Position(3);
                end                                
            end
