function [legend_h,object_h,plot_h,text_strings] = columnlegend(hPlot,numcolumns, str, varargin)
%
...
%create the legend
[legend_h,object_h,plot_h,text_strings] = legend(hPlot,str,'Location',location);
if (boxon); set(legend_h,'box','on'); end
...
for i=1:numlines,
    ...
    set(object_h(linenum), 'xdata', [pX*col/numcolumns+spacer pX*col/numcolumns+spacer+line_width]);
    
    set(object_h(linenum+1), 'ydata', [height-(position-1)*sheight height-(position-1)*sheight]);
    set(object_h(linenum+1), 'xdata', [pX*col/numcolumns+spacer*2.5 pX*col/numcolumns+spacer*2.5]);
    
    set(object_h(labelnum), 'position', [pX*col/numcolumns+spacer*2+line_width height-(position-1)*sheight]);
...
posF = get(legend_h, 'position');
set(legend_h, 'Color', 'None','EdgeColor',[1 1 1],'Position',[pos0(1),posF(2:end)])
uistack(legend_h,'bottom')
