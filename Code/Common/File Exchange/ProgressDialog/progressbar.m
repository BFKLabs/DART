...
if (verLessThan('matlab','8.4'))
    ...
    objType = {'matlab.ui.Figure'};
end

if nargin >= 1 && ~isempty(h)
    validateattributes(h, objType, {'scalar'});
...
if isempty(h)  % no handle given, create dialog
    if usejavactrl && usejava('swing')  % Java swing implementation
        ...
        uicontrol(h, ...
            'Style', 'text', ...
            'BackgroundColor', 0.941*ones(1,3), ...
            'Units', 'pixels', ...
            'FontSize', 10, ...
            'String', defaultmsg, ...
            'Visible', 'off');
...
% 
width = (2*(bOfs+xyOfs)+bWid);
height = 2*bOfs+2*bHgt;

h = figure( ...
    'Units', 'pixels', ...
    'Color', 0.941*ones(1,3), ...
    'DockControls', 'off', ...
    'IntegerHandle', 'off', ...
    'MenuBar', 'none', ...
