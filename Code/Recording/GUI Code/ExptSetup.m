...
[axLimMax,mType,dyMax,isUpdating,isCreateBlk] = deal(300,0,2.5,false,false);
...
%             sID = getGlobalStimID([], objDACInfo, 1:length(iStim.nChannel));
        ...
        switch mType
            ...
            case 2 % case is a signal block is being placed
                ...
                if strcmp(getProtoTypeStr(getappdata(hFig,'pType')),'S')
                    set(handles.buttonTestTrain,'enable','on')
                end                
            ...
            case {3,5} % case is a signal block has been selected and moved
                ...
                if ~isReleased && isOverAxes(get(hFig,'CurrentPoint'))
                    ...
                    if iProto(nProto)
                        cellfun(@(x)(set(x,'visible','on')),hDL);
                    else
                        cellfun(@(x)(set(x,'visible','on')),hDL);
                    end
                end
...
%                 % distance line markers visible
%                 if ~isReleased
%                     cellfun(@(x)(set(x,'visible','on')),hDL(:,1));
%                 end                
...
switch mType
    case {3,5} % case is a signal block has been selected and moved
        ...
        uData = get(hSigSel,'UserData');
        cellfun(@(x)(set(x,'visible','off')),uData{indFcn('hDL')});                       
...
if (fIndex ~= 0)
    ...
    switch fExtn
        case '.exp'
            ...
            [iExpt,sTrainEx] = ...
                        convertExptData(handles,sTrainS,fData.iExpt); 
...
[fName,fDir,fIndex] = uiputfile(...
    {'*.expp','Stimulus Playlist Files (*.expp)'},...
    'Save Experiment Protocol File',dDir);
if (fIndex ~= 0)
...
if ~isempty(hMain)
    ...
    set(hMainH.menuFile,'enable','on');
    set(hMainH.menuExpt,'enable','on');
...
% FINISH ME!
% isRTBatch = strcmp(get(handles.menuEnableBP,'checked'),'on');
isRTBatch = false;
...
if (sum(iExpt.Video.Dmax) == 0)
    ...
    waitfor(errordlg(eStr,'Incorrect Video Duration','modal'))
...
iProto = getProtocolIndex(hFig);
set(handles.toggleZoomAxes,'enable',eStr{1+(~iProto(1))});
...
setObjEnableProps(hFig,'buttonClearAll',eStr{1+hasStim});
setObjEnableProps(hFig,'buttonAddSig',eStr{1+hasStim});
...
if chkEditValue(nwVal,nwLim,isInt)
    ...
    if checkSignalDur(hFig,sParaS,sPara.tDur)
...
function buttonUseMinSig(hObject, eventdata, handles)
...
setObjEnableProps(hFig,'buttonClearAll',eStr{1+anyStim});
setObjEnableProps(hFig,'buttonSaveTrain',eStr{1+anyStim});
setObjEnableProps(hFig,'buttonUseMinSig',eStr{1+anyStim});

% enables the stimuli train test (short-term protocol only)
if strcmp(getProtoTypeStr(getappdata(hFig,'pType')),'S')
    set(handles.buttonTestTrain,'enable',eStr{1+anyStim})
end
...
if ~ischar(eventdata)
    set(hList,'Max',2,'Value',[],'enable','on')
end

% enables the stimuli train test (short-term protocol only)
setObjEnable(getProtoObj(hFig,'buttonUseMinSig'),'off')
if strcmp(getProtoTypeStr(getappdata(hFig,'pType')),'S')
    set(handles.buttonTestTrain,'enable','off')
end
...
if isempty(sTrainP)
    ...
    if any(strcmp(sName,sTrainC.sName))
        ...
        eStr = ['Stimuli train name already exists! Re-enter a unique ',...
            ...
            'to the stored list.'];
        waitfor(errordlg(eStr,'Duplicate Train Names','modal'))
...
% initialisations
tDurMax = 0;
sPara = field2cell(sTrainC.blkInfo,'sPara',1);
...
if ~ok
    ...
    waitfor(errordlg(eStr,'Infeasible Stimuli Train Parameters','modal'))
end
...
% When first opening GUI, the pType flag must be set correctly
%  - should be Short-Term, but was Experimental?
...
cellfun(@(x)(set(x,'visible','on')),hDLnw);
...
if chkEditValue(nwVal,nwLim,isInt)
    ...
    if checkExptStimTrainDur(hFig,sPara)% && ...
       %checkExptBlockLimits(hFig,sPara)
...
if tExp == 0
    ...
    waitfor(errordlg(eStr,'Infeasible Experiment Duration','modal'))  
...
elseif tExp >= tMin
    ...
    resetExptTimeAxes(handles,TexpU0)   
...
if checkExptStimTrainDur(hFig,sPara)
    ...
    if isempty(hSigSel)
        ...
        hEdit = findobj(hTab,'UserData','nCount');
        editExptStimPara(hEdit, '1', handles, sType(1))
...
chCol = getChannelCol(devType,nCh);
...
if isInit
    ...
    for i = 2:nProto-1
        ...
        % sets the duration string (dependent on protocol type)
        switch dType{i}
            case 'S'
                % case is the short-term protocols
                tDur = 's';

            case 'L'
                % case is the long-term protocols
                tDur = 'h';
        ...
        set(hTextDur,'String',sprintf('Total Train Duration (%s): ',tDur));
        ...
        for j = 1:length(tStrS)
            ...
            createParaObj(handles,hTab{i}{j},sPara,dType{i},nParaMax);
        end
...
if hasStim    
    ...
    for j = 2:nProto
        ...
        hDL{j} = initPlotAxis(handles,hAx{j},tLim(i),devType,chInfo,...
                              dStr{i},isInit);  
        setAxesZoomMotion(hZoom,hAx{j},'horizontal');                          
    end    
    ...
    arrayfun(@(x)(jTab.setEnabledAt(x-1,1)),2:nProto-1)
    setExptTabProps(hFig)          
...
% updates the zoom toggle button enabled properties 
iProto = getProtocolIndex(hFig);
set(handles.toggleZoomAxes,'enable',eStr{1+(hasStim && ~iProto(1))});
...
if ~isInit; set(hFig,'Visible','on'); end
...
if isExpt
    ...
    % initialisations
    [isChange,eStr] = deal(false,{'off','on'});
...
for i = 1:length(hTab)
    ...
    if isExpt
        ...
        % case is the short/long-term protocols
        sParaS = eval(sprintf('sPara.%s',hTabTitle));
        sParaDefS = eval(sprintf('sParaDef.%s',hTabTitle));
...
if ~isExpt
    ...
    set(hButReset,'enable',eStr{1+isChange})
end
...
for i = 0:nCh
    ...
    plot(axLimMax*[-1,1],(i+yGap)*[1,1],'k:')
    ...
    if i < nCh
        ...
        if i > 0
            if sID(i+1) ~= sID(i)
                hPlt(end+1) = plot(axLimMax*[-1,1],i*[1,1],...
                                   'k','linewidth',2);
            else
                plot(axLimMax*[-1,1],i*[1,1],'k:')
            end
        end
        
        % creates the new patch
        yy = [(i+yGap),(i+1)];
        patch(xx(ii),yy(jj),[0,0,0],'UserData',i+1,...
...
plot([0,0],yLim,'k','tag','hLine0')
plot(tLim*[1,1],yLim,'k','tag','hLine1')
...
if strcmp(get(hAx,'tag'),'axesProtoS')
    plot([0,0],yLim,'r','linewidth',2,'tag','hSigTest','visible','off');
end
...
for i = 1:length(dType)
    ...
    if ~isempty(sTrainP)     
        ...
        set(handles.buttonAddSigEx,'enable','on')
    ...
    setPanelProps(hTabEx{i},eStr{1+(~isempty(sTrainP))}) 
    setPanelProps(hPanelEx,eStr{1+(~isempty(sTrainP))}) 
    setPanelProps(hPanel,eStr{1+(~isempty(sTrainP))})        
end

% sets the add button enabled properties
setObjEnableProps(hFig,'buttonAddSig',eStr{1+any(hasTrain)})
...
function createParaObj(handles,hTabS,sPara,dType,nParaMax)
...
set(findobj(hDL,'tag','top line'),'visible','off')
...
set(handles.radioFixedDur,'enable',eStr{1+hasStim})
set(handles.radioBtwnStim,'enable',eStr{1+hasStim})
set(handles.checkProtoFeas,'enable',eStr{1+hasStim})
set(handles.checkStimFeas,'enable',eStr{1+hasStim})
...
set(handles.menuSaveProto,'enable',eStr{isFeas+1})
set(handles.buttonSaveProto,'enable',eStr{isFeas+1})
set(handles.menuStartExpt,'enable',eStr{isFeas+1})
...
switch pType
    case 'Experiment Stimuli Protocol'
        ...
        set(findobj(hAx,'tag','chFill'),'visible','off');
    ...
    otherwise % case is the short/long-term protocols
        ...
        % sets up the position/userdata for the imrect object
        rPos = [x0,((iCh-1)+yGap),sParaS.tDur,1-yGap];
...
set(findobj(hRect,'type','Line'),'Visible','off')
...
%     end
    ...
    tOfsNw = tOfs*getTimeMultiplier(sPara.tOfsU,sPara.tDurU);   
...
hAx = get(hFig,'CurrentAxes');
set(findobj(hAx,'tag','chDist'),'visible','off');
set(findobj(hAx,'tag','chFill'),'visible','off');
setappdata(hFig,'iCh',-1)
...
function ok = checkSignalDur(hFig,sPara,tDur)

% global variables
global hSigSel

% initialisations
[ok,eType] = deal(true,0);
...
if ~ok
    ...
    switch eType
        case 1
            ...
            eStr = sprintf(['Error! The signal offset (%.2f%s) lies ',...
                'an existing signal block. Re-select another value ',...
                'which is feasible.'],tOfs,tDurU);
        case 2
            % case is the upper end exceeds the allowable limits
            eStr = sprintf(['Error! The signal offset (%.2f%s) and signal ',...
                ...
                ' * Increase the "Total Signal Duration"\n',...
    ...
    waitfor(errordlg(eStr,'Infeasible Signal Parameters','modal'))
end

% --- determines if the stimuli 
function ok = checkExptStimTrainDur(hFig,sPara)
...
if tDurBlk > vec2sec(getPopupDurValues(hPanelP))
    ...
    waitfor(errordlg(eStr,'Invalid Stimuli Offset Duration','modal'))
    ok = false;
    return
end
...
if isempty(hSigSel)   
    ...
    if isempty(tLimF)
        ...
        eStr = sprintf(['There are no feasible locations to include a ',...
                'new stimuli block of this type/size. To remedy this ',...
                'issue either:\n\n * Increase the experiment duration\n ',...
                '* Decrease the stimuli interval/repetition count']);        
    end
else
    % otherwise, retrieves the time limits for the current block
    sBlk = getSignalBlock(hFig);
    sPos = hSigSel.getPosition();
    tLim = getValidStimBlockLimits(hFig,hSigSel,sPos,tDurFull);
    
    % determines if the limits for the selected blocks are sufficient to
    % include the size of the stimuli block
    if diff(tLim) < tDurStim
        % if not, then create an error message for screen
        ok = false;
        eStr = sprintf(['The signal properties are such that the ',...
                'currently selected block can''t feasibly fit within its ',...
                'current limits. To remedy this issue either:\n\n',...
                ' * Decrease the stimuli interval/repetition count\n',...
                ' * Expand the time limits on the currently selected block']);
    else        
        % otherwise, determine if the stimuli+offset is less than the upper
        % block temporal limit
        ok = roundP(tOfs+tDurStim,0.01) <= tLim(2);        
        if ~ok
            % if not, then create an error message for screen
...
%     eStr = sprintf(['Error! The stimuli train duration (%.2f) must be ',...
%         'less than the allowable experiment duration (%.2f). ',...
%         'To overcome this issue either:\n\n',...
%         ' * Reduce the "Stimuli Repetition Count"\n',...        
%         ' * Increase the "Experiment Duration"'],...
%         tDurStim,tDurFull);
...
if ~ok
    ...
    waitfor(errordlg(eStr,'Infeasible Signal Duration','modal'))
end
...
sTrain = struct('sName',[],'chName',[],'tDur',[],'tDurU','h','blkInfo',[]);
...
for iBlk = 1:nBlk
    ...
    rPos = [tOfs,((iCh-1)+yGap),sPara.tDur,1-yGap];
...
set(getProtoObj(hFig,'buttonClearAll'),'enable','on');
...
switch pType
    case 'S' % case is a short-term protocol
        [sName,tUnits] = deal('Short-Term Signal #1','s');
        
    case 'L' % case is a long-term protocol
        [sName,tUnits] = deal('Long-Term Signal #1','h');
        
end

% creates the parameter struct
sPara = struct('Square',[],'Ramp',[],'Triangle',[],'SineWave',[],...
               'tDur',10,'tDurU',tUnits,'sName',sName);
...
sPara.Square = struct('sAmp',100,'tDurOn',0.5,'tDurOff',0.5,'tOfs',0,'nCount',3);
sPara.Ramp = struct('sAmp0',0,'sAmp1',100,'tCycle',1,'tOfs',0,'nCount',3);
sPara.Triangle = struct('sAmp0',0,'sAmp1',100,'tCycle',1,'tOfs',0,'nCount',3);
sPara.SineWave = struct('sAmp0',0,'sAmp1',100,'tCycle',1,'tOfs',0,'nCount',3);

% calculates the signal durations for each of the signal types
pFld = fieldnames(sPara);
for i = 1:length(pFld)    
    if strcmp(pFld{i},'Square')
        % case is the square waveform
        sPara.Square.tDur = sPara.Square.tOfs + ...
            sPara.Square.tDurOn*sPara.Square.nCount + ...
            sPara.Square.tDurOff*(sPara.Square.nCount-1);
    else
        % case is the other waveform types
        pStr0 = sprintf('sPara.%s',pFld{i});
        if isfield(eval(pStr0),'tCycle')
            eval(sprintf('%s.tDur = %s.tOfs + %s.tCycle*%s.nCount;',...
                         pStr0,pStr0,pStr0,pStr0));
        end
    end
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%    PARAMETER TEMPORAL UNITS    %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% square wave temporal units
[sPara.Square.tDurU,sPara.Square.tOfsU] = deal(tUnits);
[sPara.Square.tDurOnU,sPara.Square.tDurOffU] = deal(tUnits);

% ramp temporal units
[sPara.Ramp.tCycleU,sPara.Ramp.tDurU,sPara.Ramp.tOfsU] = deal(tUnits);

% triangle wave temporal units
[sPara.Triangle.tCycleU,sPara.Triangle.tDurU,...
                        sPara.Triangle.tOfsU] = deal(tUnits);

% sine wave temporal units
[sPara.SineWave.tCycleU,sPara.SineWave.tDurU,...
                        sPara.SineWave.tOfsU] = deal(tUnits);
...
% sPara = struct('tDur',[0,12,0,0],'tDurU','Hour','tStart',[],...
%                'fixedT0',0,'S',[],'L',[]); 
...
for i = 1:nTrain
    ...
    sParaEx.tOfs = iExpt0.Timing.Tws*getTimeMultiplier('h','m');
...
function [sTrainS,devType,nCh] = convertStimData(fData)
...
% intialisations
iStim = fData.iStim;
nDev = length(iStim.nChannel);
[nCh,sType] = deal(iStim.nChannel,'Square');
devType = repmat({'Motor'},nDev,1);
...
sTrainS.blkInfo = repmat(blkInfo,nBlk,1);
...
for iCh = 1:nCh
    % memory allocation
    nBlk = iStim.nCount(iCh);
    [tBlk0,tBlkDur] = getStimBlockTiming(iStim.iPara{iCh},nBlk);
    
    % sets the information for the signal block
    tOfs = 0;
    for iBlkCh = 1:nBlk
        % retrieves the parameter struct for the channel block
        iPara = iStim.iPara{iCh}(iBlkCh);
        
        % sets the time offset
        if iBlkCh == 1
            % case if for the first stimuli block
            tOfs = iPara.iDelay.pVal;
        else
            % case is for the other stimuli blocks
            iParaPr = iStim.iPara{iCh}(iBlkCh-1);
            tOfs = tOfs + (sPara.tDur + iParaPr.sDelay.pVal);
        end

        % sets up the signal block userdata/position
        sPara = sP0;
        sPara.sAmp = 100*iPara.pAmp.pVal;
        sPara.tDurOn = iPara.pDur.pVal;
        sPara.tDurOff = iPara.pDelay.pVal;
        sPara.nCount = iPara.pCount.pVal;
        ...
        sTrainS.blkInfo(iBlk).t0 = tBlk0(iBlkCh);
    ...
    sTrainS.tDur = max(sTrainS.tDur,ceil(tBlk0(end)+tBlkDur(end)));
end
...
for iBlk = 1:nBlk
    ...
    if iBlk == 1
        ...
        t0Nw = t0(iBlk-1) + tDur(iBlk-1) + iPara(iBlk).sDelay.pVal;
    end
    
    % sets the time offset of the current block
    t0(iBlk) = t0Nw + iPara(iBlk).iDelay.pVal;
    
    % calculates the duration of the blocks
    tDurOn = iPara(iBlk).pDur.pVal;
    tDurOff = iPara(iBlk).pDelay.pVal;
    nCount = iPara(iBlk).pCount.pVal;
...
if iChNw ~= iCh
    ...
    hAx = get(hFig,'CurrentAxes');
    set(findobj(hAx,'tag','chFill','UserData',iCh),'visible','off') 
    
    % turns on the channel highlight
    if ~iProto(nProto); set(hHover(isChP),'visible','on'); end
end
...
if ~isempty(uData)
    ...
    if ~isMatch
        ...
        objOff = false;
        if mType == 2; set(hHover,'visible','on'); end
        createTempSignalObject(hFig,iCh)
    end
end
...
rPos(1) = sParaS.tOfs*getTimeMultiplier(sParaS.tDurU,sParaS.tOfsU);
rPos(3) = sParaS.tDur;
hSigSel.setPosition(rPos);

% retrieves the scaled signal values
[xS,yS] = setupScaledStimuliSignal(hAx,sParaS,iCh,sType,1);
set(hSigObj,'xData',xS,'yData',yS)

% updates the userdata for the selected object
uData{indFcn('pDX')} = xS(1)-rPos(1);
...
if isempty(tLimF)
    ...
    if userIA
        ...
        waitfor(errordlg(eStr,'Infeasible Block Placement','modal'))
    end
elseif userIA            
...
function resetExptTimeAxes(handles,TexpU0)
        ...
        if ~isempty(sigBlk)
            if ~isempty(sigBlk{nProto})
                ...
                for i = 1:length(sBlk) 
                    ...
                    hSigObj = uData{indFcn('hSigObj')};                
                    arrayfun(@(x,pdx)(set(x,'xdata',(pdx+rPos(1))+tMlt*...
                            get(x,'xdata'))),hSigObj,uData{indFcn('pDX')})         
                end
            end
        end
    end
...
% retrieves the start time vector
hFig = handles.figExptSetup;
iExpt = getappdata(hFig,'iExpt');
...
function ok = updateExptParaFields(hFig,sTrainC,dType)
...
[~,tUnits,tOfsB,tDurB] = ...
            calcExptStimDuration(hFig,sTrainC,sPara,tDurEx);
tDurS = max(tOfsB+tDurB);
...
%
if tDurEx < tDurS
    % case is that the stimuli duration exceeds the 
    ...
    tDurInt = vec2time(sPara.tStim,sPara.tDurU);
    if tDurS > tDurInt
        % if the stimuli time exceeds that of the interval, then reset the
        % time interval so that equals the stimuli time
        tDurInt = tDurS;
        sPara.tStim = time2vec(tDurS,sPara.tDurU);
    else
        sPara.tStim = time2vec(max(1,tDurS),sPara.tDurU);
    end
    
    %    
    tDurNw = (sPara.nCount-1)*tDurInt + tDurS;
    if tDurNw > tDurEx
        sPara.nCount = floor((tDurNw - tDurS)/tDurInt) + 1;
        tDurNw = (sPara.nCount-1)*tDurInt + tDurS;
    end
...
%
if ~isempty(eStr)
    waitfor(errordlg(eStr,'Stimuli Train Error','modal'))
    ok = false;
end
...
hZoom = zoom;
set(hZoom,'Enable','off','ActionPostCallback',{@updateAxesTicks,hFig});   
...
if (get(handles.panelRecordType,'SelectedObject') == handles.radioFixedDur)
    ...
    iExpt = getappdata(handles.figExptSetup,'iExpt');
    if (any(field2cell(iExpt.Stim,'nCount',1)>0))
        % if the parameters have been set, then enable the button
        set(handles.buttonOptPlace,'enable','on')    
    else
        % parameters are not set, then disable the button
        set(handles.buttonOptPlace,'enable','off')
    end
else
...
if (strcmp(eStr,'on'))
    ...
    if (length(fStr) == 1); set(handles.popupFrameRate,'enable','off'); end
    ...
    if ((iExpt.Video.FPS > 0) && (sum(iExpt.Video.Dmax) > 0))
        ...
        [isCheck,resetFields] = deal(all(field2cell(iExpt.Stim,'nCount',1) > 0),false); 
    else
