function varargout = ProgParaRecord(varargin)
% Last Modified by GUIDE v2.5 12-Jan-2014 23:39:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ProgParaRecord_OpeningFcn, ...
    'gui_OutputFcn',  @ProgParaRecord_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ProgParaRecord is made visible.
function ProgParaRecord_OpeningFcn(hObject, eventdata, handles, varargin)

% global variables
global forceUpdate

% Choose default command line output for ProgParaRecord
...
if (isempty(ProgDef))
    ProgDef = struct('DirMov',[],'StimPlot',[],'DirPlay',[],'CamPara',[]);
end

% sets the cancel enable properties based on whether update is being forced
forceUpdate = length(varargin) == 3;
setObjEnable(handles.buttonCancel,~forceUpdate)
...
% UIWAIT makes ProgParaRecord wait for user response (see UIRESUME)
set(hObject,'WindowStyle','modal');
uiwait(hObject);

% --- Outputs from this function are returned to the command line.
function varargout = ProgParaRecord_OutputFcn(hObject, eventdata, handles)
