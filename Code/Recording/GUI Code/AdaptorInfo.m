...
set(handles.textVidResolution,'enable','off')
...
if DAConly
    ...
    set(handles.buttonConnect,'enable',eStr{1+all(strcmp(crStr,'All'))})
...
if isTest
    ...
    if ~hasDevice
        ...
        % if there are no devices, then prompt the user if they still want
        % to continue
        uChoice = questdlg({'No devices attached to the computer';...
            'Do you still wish to continue?'},...
            'No Devices Attached?','Yes','No','Yes');
        if ~strcmp(uChoice,'Yes')
            % deletes the loadbar
            delete(h)
            
            % cancels the GUI with an empty array (should exit program)
            buttonExit_Callback(handles.buttonExit, [], handles)
            return
        else
            % disables the other radio buttons (can't record with stimuli)
            set(handles.radioRecordStim,'enable','off')
...
if IMAQonly || get(handles.radioRecordOnly,'value')
    ...
    if isempty(vSelDAC) || any(nChannel(vSelDAC) == 0)
        set(handles.buttonConnect,'enable','off')
    else
        set(handles.buttonConnect,'enable','on')
    end
...
if (~isa(eventdata,'char') && ~isTest)
    set(handles.textVidResolution,'enable','on')
...
switch exptType
    ...
    otherwise % case is the other buttons
        ...
        cellfun(@(x)(set(x,'Enable','Inactive')),hCheck)
end
...
if ~DAConly
    ...
    if isTest
        ...
        imaqInfoDev = getappdata(hFig,'imaqInfoDev');
...
%                 (vIndIMAQ(vSelIMAQ,2)).VideoDeviceConstructor;
            vConStr = imaqInfoDev{vIndIMAQ(vSelIMAQ,1)}...
...
cellfun(@(x)(set(x,'enable',state)),hEdit)
...
set(handles.buttonConnect,'enable',eStr{1+allOK})
