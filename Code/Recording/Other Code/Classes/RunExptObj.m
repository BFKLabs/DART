...
%                     end
        ...
        function initCameraProperties(obj)
            ...
            if obj.isTest
                obj.hTimer = [];            
            elseif obj.isWebCam && ~isempty(obj.hTimer)
                obj.hTimer = [];
            end
            ...
            [fRate,iSel] = getFrameRateSelection(obj);
        ...
        function finishExptObj(obj)
            ...
            if ~obj.isStart
                ...
                wState = warning('off','all');
                try close(getLogFile(obj)); catch; end
                warning(wState);
...
%                 figure(obj.hExptF)    
    ...
    %             obj.forceStopDevices()   
                ...
                if obj.hasIMAQ
                    ...
                    logFileT = getLogFile(obj);
                    fName = get(logFileT,'FileName');
        ...
        function timerExptFcn(obj)
            ...
            if obj.objP.hButC.Value
                ...
                if obj.hasIMAQ
                    if ~(obj.isWebCam || obj.isMemLog)
                        % camera is triggered, so stop recording
...
%                         warning(wState)
            ...
            try
                obj.indFrmNw = get(obj.hTimerExpt,'TasksExecuted');
            catch
            end
...
%                                 (x,chInfo,1/sRate)),sTrainS,'un',0);
        ...
        function triggerVideoInput(obj)
            ...
            while ~isDeviceLogging(obj)
                ...
                if isa(obj.objIMAQ,'videoinput'); trigger(obj.objIMAQ); end
                nTrig = nTrig + 1;
            end
