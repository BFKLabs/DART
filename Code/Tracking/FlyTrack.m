...
setappdata(hObject,'postWindowSplit',@postWindowSplit)
...
% sets the time stamp array
if ~isfield(iData,'Tv')
    setTime = true;
else   
    setTime = isempty(iData.Tv);
end
...
if exist(sFile,'file')
    ...
    iVid = str2double(regexp(solnData.fData.name,'\d*','match','once'));        
...
if isCalib     
    ...
    stop(getappdata(handles.figFlyTrack,'vidTimer'))
else
...
if ~isempty(hMark)    
    for i = 1:nApp
        ...
        if isCalib       
            updateIndivMarker(handles,hMark{i},...
                                hDir{i},fPosTmp,i,pltLoc,pltAng,true)                     
        else
            updateIndivMarker(handles,hMark{i},...
                                hDir{i},pData,i,pltLoc,pltAng) 
        end        
    end
end
...
iMov = struct('pos',[1 1 1 1],'posG',[],'Ibg',[],'ddD',[],'dTube',false,...
              'nRow',[],'nCol',[],'nTube',[],'nTubeR',[],'isUse',[],...              
              'iR',[],'iC',[],'iRT',[],'iCT',[],'xTube',[],'yTube',[],...
              'sgP',[],'Status',[],'tempName',[],'autoP',[],'pStats',[],'bgP',[],...
              'isSet',false,'ok',true,'tempSet',false,'isOpt',false,...
              'sRate',5,'nDS',1,'useRot',false,'rotPhi',0,...
              'calcPhi',false,'vGrp',[],'nPath',trkP.nPath);           
iMov.sgP = iData.sgP;          
