...
if (yMax == 2)
    ...
    set(handles.buttonExptAdd,'enable','on')
    set(handles.buttonExptRemove,'enable','off')           
end
...
if (yMax == 2)
    ...
    % retrieves the indices of the selected item
    set(handles.buttonExptAdd,'enable','off')
    set(handles.buttonExptRemove,'enable','on')           
end
...
if (iExptAdd.nCount == 0)
    set(handles.buttonExptReset,'enable','on')
    set(handles.menuRestartBatch,'enable','on')
...
switch (uChoice)
    ...
    case ('Added Files') % case is clearing all the added file
        ...
        set(handles.listExptAll,'value',1,'enable','on')
...
set(handles.menuRestartBatch,'enable','off')
...
if (nExp == 0)
    ...
    % sets the listbox title/object properties    
    set(handles.listExptAdded,'String',iExptAdd.fName,'enable','on')
end
...
set(handles.figMultiBatch,'visible','off')

% initialises the solution file panel objects
setPanelProps(handles.panelBatchDir,'on')
set(handles.buttonExptRemove,'enable','off')
set(handles.buttonExptReset,'enable','off')
set(handles.buttonMoveUp,'enable','off')
set(handles.buttonMoveDown,'enable','off')
set(handles.figMultiBatch,'visible','on')

% updates the information fields
set(handles.listExptAdded,'enable','on','backgroundcolor',[1 1 1])
...
if (nargin == 1)
    ...
    set(handles.checkRedoPP,'enable','on')
...
[iExptAll,iExptAdd] = deal(struct('nCount',0,'bpData',[],'isSeg',[],...
                                  'fName',[]));
