...
set(handles.buttonGoto,'enable','off')
...
set(hGUI.figFlySolnView,'visible','off')
...
eStr = {'off','on'};
set(handles.buttonGoto,'enable',eStr{1+(any(eventdata.Indices(2) == [4,5]))})
...
% updates the goto button enabled properties (frame index selection only)
eStr = {'off','on'};
set(handles.buttonGoto,'enable',eStr{1+(eventdata.Indices(2) == 4)})
...
set(hGUI.figFlySolnView,'visible','on');
...
set(handles.figDiagCheck,'visible','off')
...
if (all(nDCount == 0))
    ...
    set(handles.tableFrmDist,'visible','off')      
    txPos = [txPos(1) (2*yDel+txPos2(4)) txPos(3:4)];
    edPos = [edPos(1) (2*yDel+txPos2(4)+3) edPos(3:4)];
    set(handles.textDispLbl,'position',[txPos2(1) yDel txPos2(3:4)],'visible','on')
else
    ...
    set(handles.textDispLbl,'visible','off')
...
set(handles.figDiagCheck,'visible','on')
...
if (all(nNaNCount == 0))
    ...
    % makes the table invisible 
    set(handles.tableNaNCount,'visible','off')   
else
    % determines the regions which have NaN values     
    nGrp = sum(nNaNCount(:));
