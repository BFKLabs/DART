...
if ~isempty(cGrp)
    if pressCtrl
        ...
        set(handles.buttonMerge,'enable',eStr{1+canMerge})
        set(handles.buttonSplit,'enable',eStr{1+~canMerge})
        set(handles.textGroupSelCount,'string',num2str(sum(fOtherOn)+dfAlpha))
    else
        %
        set(handles.buttonSplit,'enable','on')
        set(handles.buttonMerge,'enable','off')
...
for i = 1:length(hObjNw)
    set(eval(sprintf('handles.%s',hObjNw{i})),'enable',State)
end
...
set(handles.buttonSplit,'enable',eStr{1+(diff(iData.vGrp(cGrp(1),:))>50)})
...
cellfun(@(x)(set(findall(hRect,'tag',x),'visible','off')),rmvStrS)                            

% removes the corner markers
for i = 1:2
	rmvStrC = cellfun(@(x)(sprintf('%s %s %s',sStr{i},x,mStr{2})),sStr(3:4),'un',0);
    cellfun(@(x)(set(findall(hRect,'tag',x),'visible','off')),rmvStrC)                            
end
...
set(handles.editGrpStart,'string',num2str(fLim(1)))
set(handles.editGrpFinish,'string',num2str(fLim(2)))
set(handles.buttonSplit,'enable',eStr{1+(diff(fLim)>50)})
