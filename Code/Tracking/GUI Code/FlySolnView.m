...
[hAxI,hAxS] = deal(handles.axesImg,handles.axesStim);
...
switch iApp
    case 0 % case is the average velocity             
        ...
        ylabel(hAxI,'Region Index','fontweight','bold','fontsize',lblSz)      
        
    otherwise % case is the positional traces
        yLimT = [1 nFly] + 0.5*[-1 1];
        ylabel(hAxI,'Sub-Region Index','fontweight','bold',...
                   'fontsize',lblSz)
end
...
title(hAxI,'Average Velocity','fontweight','bold','fontsize',tSz)
xlabel(hAxI,'Time (min)','fontweight','bold','fontsize',lblSz)
...
set(hAxI,'xlim',T([1 end])*Tmlt-[pDel,0]','ylim',yLimT'-[0.001;0],...
         'yticklabel',yStr,'fontweight','bold','fontsize',axSz,'box','on',...
...
if showStim
    ...
    dType = fieldnames(iData.stimP);
    stimPC0 = cellfun(@(x)(getStructField(iData.stimP,x)),dType,'un',0);
    ...
    dcInfo = cell2cell(cellfun(@(x,y,z,i)([repmat({x},sum(i),1),y(i),z(i)]),...
                        dType,chName,chCol,hasStim,'un',0));
    if isempty(dcInfo)
        showStim = false;
    else
        stimPC = cellfun(@(x)(getStructField(iData.stimP,x{1},x{2})),...
        ...
        sStr = cellfun(@(x)(regexp(x,'\d','match','once')),dcInfo(:,1),'un',0);    
        yStrS = cellfun(@(x,y,z)(sprintf('%s (%s%s)',x,y(1),z)),...
                                          yStrS0,dcInfo(:,1),sStr,'un',0);

        % sets the experimental stimuli train parameter struct
        sPara = iData.sTrainEx;
    %     sPara = evalin('base','sPara'); % REMOVE ME!
    ...
    %     A = importdata('MM (4Ch).expp','-mat'); sPara = A.sTrain.Ex;      % REMOVE ME LATER
        ...
        for i = 1:nStim
            ...
            for j = 1:length(indS)
                ...
                cellfun(@(x,y)(createPatchObject(hAxS,x,y,pCol)),xS,yS)         
            end
        ...
        resetObjPos(handles.panelStim,'Height',axH+2*dY)
        resetObjPos(handles.panelImg,'Bottom',axH+4*dY)
        ...
        set(hAxS,'xlim',T([1 end])*Tmlt-[pDel,0]','ylim',yLimS'-[0.001;0],...
             'yticklabel',yStrS,'fontweight','bold','fontsize',axSz-6,...
             'TickLength',[0 0],'LineWidth',1.5,'ytick',yTickS,'UserData',2,...
             'Color',axCol,'box','on');
...
%
if ~showStim
    % if not, hide the stimuli info axes panel
    ...
    resetObjPos(handles.panelImg,'Bottom',dY)
end
...
% resets the objet property units
...
patch(hAx,x,y,c,'LineStyle','none','FaceAlpha',0.5);
...
switch (iApp)
    case (0) % case is the average velocity
        ...
        ylabel(hAx,'Region Index','fontweight','bold','fontsize',lblSz)        
