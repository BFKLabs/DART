...
% Last Modified by GUIDE v2.5 01-Oct-2016 02:01:03
...
if (isDetecting)
    set(handles.uiDataTool,'enable','off') 
end
...
if (is2D)    
    ...
    set(handles.menuPlotData,'enable','off')
...
switch (get(hObject,'state'))
    case ('on') % case is the zoom is turned on        
        ...
        set(handles.uiDataTool,'enable','off')                
        set(handles.uiDataTool,'state','off')
    ...
    case ('off') % case is the zoom is turned off        
        ...
        % if detecting, then don't allow the data tool
        if (~isDetecting)
            set(handles.uiDataTool,'enable','on')    
        end
end
...
switch (get(hObject,'state'))
    ...
    case ('off') % case is the data tool is turned off
        ...
        set(findobj(handles.figFlySolnView,'tag','hMark'),'visible','off')
        set(findobj(handles.figFlySolnView,'tag','hText'),'visible','off')
...
set(get(hAx,'xlabel'),'string','Time (min)')
...
% retrieves the axes handle and time vector
hAx = findall(handles.figFlySolnView,'type','axes');
set(get(hAx,'xlabel'),'string','Frame Index')
...
hAx = findall(h.panelImg,'type','axes');
[axSize,lblSize,tSize] = detSolnViewFontSizes(h);

% resets the title, label and axis font sizes
set(hAx,'FontSize',axSize)
set(get(hAx,'Title'),'FontSize',tSize)
set(get(hAx,'xLabel'),'FontSize',lblSize)
...
set(handles.menuDiagCheck,'enable',mStr)
...
% sets the number of flies and apparati
[Tmlt,T] = deal(1/60,setupTimeVector(handles));
[nFly,nApp] = deal(max(getFlyCount(iMov,1)),length(iMov.iR));
NN = max(nFly,nApp);

% sets up the time labels
setappdata(handles.figFlySolnView,'T',T)

% retrieves the font-sizes
[axSize,lblSize,tSize] = detSolnViewFontSizes(handles);
...
% sets the x-axis label + properties
title(hAx,'Average Velocity','fontweight','bold','fontsize',tSize)
xlabel(hAx,'Time (min)','fontweight','bold','fontsize',lblSize)
...
if (~isempty(iData.stimP))
    if (~isempty(iData.stimP(1).Ts0))    
        [Ts,Tf] = field2cell(iData.stimP,{'Ts0','TsF'},1);
        yLim = [1 NN] + 0.5*[-1 1];
        for i = 1:length(Ts)
            xPatch = [Ts(i)*[1 1] Tf(i)*[1 1]];
            yPatch = yLim([1 2 2 1]);
            fill(xPatch*Tmlt,yPatch,0.5*[1 1 1],'facealpha',0.8,...
                 'parent',hAx,'EdgeColor',0.5*[1 1 1],'hittest','off')
        end
    end
end
...
for i = 1:nApp
    ...
    plot(hAx,NaN,NaN,'color',pCol,'tag','hLinePop','UserData',i,...
...
for i = 1:NN    
    plot(hAx,NaN,NaN,'b','tag','hLineInd','UserData',i,'Linewidth',1);
    if (is2D)        
        plot(hAx,NaN,NaN,'r','tag','hLineInd2',...
                            'UserData',i,'Linewidth',1,'hittest','off');
    end
    
    % adds in the seperator lines
    if (i ~= NN)
        plot(hAx,T([1 end])*Tmlt-[pDel,0]',(i+0.5)*[1 1],'k','linewidth',1,'hittest','off')
    end    
end

% sets the axis properties
set(hAx,'xlim',T([1 end])*Tmlt-[pDel,0]','ylim',yLimT'-[0.001;0],...
        'yticklabel',yStr,'fontweight','bold','fontsize',axSize,'box','on',...
        'TickLength',[0 0],'LineWidth',1.5,'ytick',yTick,'UserData',1);

setappdata(handles.figFlySolnView,'tTick0',get(hAx,'xTick'))
setappdata(handles.figFlySolnView,'tTickLbl0',get(hAx,'xTickLabel'))    
...
set(hText,'tag','hText','parent',hAx,'visible','off','FontSize',8,...
            'FontWeight','bold','EdgeColor','k','LineWidth',1,...
            'BackgroundColor','y')

% sets up the marker line
hMark = plot(hAx,NaN,NaN,'k','linewidth',1,'tag','hMark');

% Create menu items for the uicontextmenu
c = uicontextmenu;
set(hMark,'UIContextMenu',c)
uimenu(c,'Label','Goto Video Frame','Callback',{@updateMainFrame,handles});
...
% retrieves the font-sizes
[~,lblSize,~] = detSolnViewFontSizes(handles);
...
% parameters
[yDel,vStr] = deal(0.05,{'off','on'});

% retrieves the menu 
hMenu = findobj(handles.menuPlotMetrics,'type','uimenu','checked','on');
[hAx,iApp] = deal(handles.axesImg,get(hMenu,'UserData'));
set(handles.menuYData,'enable',vStr{1+(iApp~=0)})
...
switch (iApp)
    case (0) % case is the average velocity
        ...
        set(findobj(hAx,'tag','hLineInd'),'visible','off')
        set(get(hAx,'Title'),'string','Average Velocity')
        if (is2D); set(findobj(hAx,'tag','hLineInd2'),'visible','off'); end
...
set(hAx,'yticklabel',yStr,'ytick',yTick,'xlim',[-pDel max(get(hAx,'xlim'))]);
...
function [axSize,lblSize,tSize] = detSolnViewFontSizes(handles)
...
if (ismac)
    [axSize,lblSize,tSize] = deal(20*fR,26*fR,30*fR);
else
    [axSize,lblSize,tSize] = deal(14*fR,18*fR,22*fR);    
end
