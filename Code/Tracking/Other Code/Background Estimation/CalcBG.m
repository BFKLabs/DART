classdef CalcBG < handle
    ...
    methods
            ...
            switch selType                    
                case 'normal'
                    ...
                    if obj.iClose > 0
                        ...
                        uData = obj.hMarkAll.UserData(obj.iClose,1:2);
                        uListNw = [obj.iPara.cFrm,uData,obj.iPara.cPhase];
                        ...
                        if ~isempty(obj.uList)                            
                            if any(cellfun(@(x)(isequal(x,uListNw)),...
                                ...
                                eStr = ['Manual marker already exists ',...
                                    'for this sub-region. Delete the ',...
                                    'existing marker from the list ',...
                                    'before proceeding.'];
                                waitfor(msgbox(eStr,...
                ...
                case 'alt'
                    ...
                    if obj.iClose > 0
                        obj.removeManualHighlight(obj.iClose);
                        obj.iClose = -1;
                    end                    
            end
        ...
        function addManualMarker(obj,uListNw)
            ...
            pNw = [obj.hMarkAll.XData(obj.iClose),...
                   obj.hMarkAll.YData(obj.iClose)];
            
            % creates the new marker
            hold(obj.hAx,'on')
            hPlt = scatter(obj.hAx,pNw(1),pNw(2),'k','tag','hManual',...
        ...
        function updateManualTrackTable(obj)
            ...
            set(obj.hGUI.tableFlyUpdate,'Data',Data);
        ...
        function updateMarkerProps(obj)
            ...
            mPos = get(obj.hAx,'CurrentPoint');

            % sets the marker coordinates and ambiguity flags
            pData = [obj.hMarkAll.XData;obj.hMarkAll.YData]';
            isAmbig = logical(obj.hMarkAll.CData(:,1));            
            
            % calculates the distances between the mouse and the markers
            D = pdist2(pData,mPos(1,1:2));
            D(~isAmbig) = Dtol + 1;            
            
            % determines if there are any points close to the mouse
            isClose = D <= Dtol;
            if any(isClose)
                % sets the closest marker to being on 
                iCloseNw = argMin(D);
                if obj.iClose ~= iCloseNw
                    % reset
                    iClose0 = obj.iClose;                    
                    obj.iClose = iCloseNw;
                    
                    % if the previous marker is still highlighted, then
                    % remove the highlight from that object
                    if iClose0 > 0
                        obj.removeManualHighlight(iClose0)
                    end
                    
                    % updates the marker colour and uses the hand pointer 
                    try
                        obj.hMarkAll.CData(obj.iClose,:) = [1 1 0];
                        set(obj.hFig,'Pointer','hand');
                    catch
                        obj.iClose = -1;
                        return
                    end
            ...
            elseif obj.iClose > 0
                % if a marker is still highlighted, then disable it
                iClose0 = obj.iClose;
                obj.iClose = -1;
                                
                % removes the highlight from the marker
                obj.removeManualHighlight(iClose0)   
                set(obj.hFig,'Pointer','arrow')
                
            else
                % resets the close item index
                obj.iClose = -1;                
            end
            
        end
        
        % --- removes the highlight for a particular marker
        function removeManualHighlight(obj,iClose0)
            
            % if so, retrieve the details of these markers
            pData = [obj.hMarkAll.XData(iClose0),...
                     obj.hMarkAll.YData(iClose0)];
            indG = obj.hMarkAll.UserData(iClose0,:);

            % determines if the marker represents the likely object for
            % that particular sub-region
            fPosG = obj.fPos{obj.iPara.cPhase}...
                    {indG(1),obj.iPara.cFrm}(indG(2),:);
            isBest = isequal(pData,fPosG);

            % resets the marker colour
            obj.hMarkAll.CData(iClose0,:) = double([~isBest,0,isBest]);
        ...
        function initObjectCallbacks(obj)
            ...
            scObj = {'panelTrackingMarkers', 'panelImageType'};
        ...
        function initClassFields(obj)
            ...
            if isfield(obj.iMov,'bgP')
                ...
                obj.iMov.bgP = DetectPara.initDetectParaStruct();                
            end
            ...
            if strContains(obj.iMov.bgP.algoType,'single')
                ...
                obj.trkObj = MultiTrack(obj.iData);
            end
            ...
            [obj.isAllUpdate,obj.nManualMx] = deal(true,13);             
        ...
        function initObjProps(obj)
            ...
            setPanelProps(hgui.panelTrackingMarkers,'off')
            ...
            if ~initDetectCompleted(imov)
                ...
                set(hgui.checkFlyMarkers,'enable','off')
                ...
                set(hgui.textPhaseStatus,'string',' ')    
                ...
                set(hgui.checkFlyMarkers,'value',1,'enable','on')  
                ...
                if ~isempty(obj.fPos)
                    ...
                    obj.fPos = cellfun(@(x)(x(:,1)),obj.fPos,'un',0);
                    obj.fPosAll = cellfun(@(x)(x(:,1)),obj.fPosAll,'un',0); 
                    ...
                    if ~isempty(pData)                    
                        ...
                        [obj.fPos,obj.fPosAll] = deal(cell(nPhase,1));
                        ...
                        for i = 1:nPhase
                            ...
                            obj.fPosAll{i} = cellfun(@(x)...
                                    (num2cell(x,2)),obj.fPos{i},'un',0);
                        end    
                    end
                end
        ...
        function initPotentialPlotMarkers(obj)
            ...
            % if there are no potential coordinates then exit
            if isempty(fPosF); return; end                
        ...
        function closeBGAnalysis(obj)
            ...
            % determines if calibrating
            if obj.isCalib
                % if so, then restart the video feed 
                objIMAQ = getappdata(obj.hFig,'objIMAQ');
                updateVideoFeedImage(obj.hFig,objIMAQ)     
            else
                % otherwise, update the axes with the original image
            ...
            if ~isempty(obj.hManual)
                % if there are manual placement markers, then remove them
                try; delete(obj.hManual); end
                obj.hManual = [];
            end                                                           
...
%             % re-enables the apparatus and frame select panels
%             setPanelProps(obj.hGUI.panelAppInfo,'on')
%             if obj.isCalib
%                 set(obj.hGUI.editFrameStat,'enable','inactive')
%                 set(obj.hGUI.textStatus,'enable','on')
%                 set(obj.hGUI.textStatusStr,'enable','on')
%             else
%                 setPanelProps(obj.hGUI.panelFrmSelect,'on');
%                 set(obj.hGUI.editFrameStat,'enable','inactive');
%             end
        ...
        function updateMainImage(obj,varargin)
            ...
            if ~isempty(obj.hManual) && get(obj.hGUI.radioAllObject,'Value')
                ...
                isShow = (uData(:,1) == ipara.cFrm) & ...
                         (uData(:,4) == ipara.cPhase);
...
%                         obj.ImgFrm{iPhase} = obj.ImgFrm{iPhase} + dI;
                ...
                case ('radioBackEst') % case is the background estimate        
                    ...
                    if strcmp(getDetectionType(imov),'General')
        ...
        function updateObjMarkers(obj)
            ...
            fposall = obj.fPosAll{iPhase}; 
            
            % updates the position markers for all
            if ~isempty(fposall)
                [iPosAll,fPosAllT,pColAll] = ...
                    obj.getAllMarkerColours(fposall(:,iFrm),fpos(:,iFrm));
                set(obj.hMarkAll,'xData',fPosAllT(:,1),...
                                 'yData',fPosAllT(:,2),...
                                 'cData',pColAll,'UserData',iPosAll);                
            end
        ...
        function setButtonProps(obj,Type)
            ...
            pType = {'Low','Medium','High','Invalid','Small','Direct'};                        
            ...
            ttStr = {sprintf(['* Low pixel intensity variance within phase.\n',...
                     ...
                     sprintf(['* Medium pixel intensity variance within phase.\n',...
            ...
            if strcmp(Type,'Phase')
                ...
                if ~isempty(obj.fPosAll) && initDetectCompleted(imov)
                    anyAmbig = any(cellfun(@(y)(any(cellfun(@(x)...
                                (size(x,1)>1),y))),obj.fPosAll{cX}(:,1)));
                    setObjEnable(obj.hGUI.radioAllObject,anyAmbig);

                    % if the there is no ambiguity, but the all object 
                    % button is selected, then reset the object properties
                    if ~anyAmbig && get(obj.hGUI.radioAllObject,'Value')
                        hPanel = obj.hGUI.panelTrackingMarkers;
                        set(obj.hGUI.radioLikelyObject,'Value',1)
                        panelTrackingMarkers(obj, hPanel, [])
                    end
                end
                
                % sets the phase count and variance type
                set(hgui.editPhaseCount,'string',num2str(cX),...
                                        'enable',eStr{1+(nX>1)});
        ...
        function resetGUIDimensions(obj,openBG)
            ...
            resetObjPos(hgui.figFlyTrack,'width',sum(axPos([1,3]))+dx)            
...
%             setObjVisibility(obj.hGUI.menuRTCalib,~openBG && obj.isCalib)
        ...
        function buttonUpdateStack(obj, hObject, eventdata)
            ...
            if obj.isCalib
                % sets the candidate images for the background estimation
                if getappdata(hgui.figFlyTrack,'isTest')
                    % case is the test, so load the image stack and set the
                    % indices for the next Nframe image frames
                    ImgTmp = getappdata(hgui.figFlyTrack,'objIMAQ');
                    ii = mod((vFrm-1)+((1:ipara.nFrm0)-1),length(ImgTmp))+1;

                    % sets the sub-struct
                    Img = ImgTmp(ii); clear ImgTmp
                else
                    % retrieves the camera the required number of snapshots
                    objIMAQ = getappdata(obj.hFig,'objIMAQ');
                    Img = getCameraSnapshots(objIMAQ,ipara.nFrm0);     
                    if isempty(Img)
                        return
                    end
                end
                ...
                [simgs,imgfrm] = deal(setSubImageStruct(imov,Img),Img(1));  
                simgs.iFrm = (1:ipara.nFrm0)';

                % runs the phase detection algorithm over the image stack
                obj.iMov.iPhase = detVideoPhase(idata,imov,Img,simgs.iFrm);
                if size(obj.iMov.iPhase,1) > 1
                    % if there is major fluctuation, then output an error
                    tStr = 'Inconsistent Image Quality';
                    eStr = ['There is a fluctuation in the image quality ',...
                            'of the image stack. Ensure the video ',...
                            'quality is consistent or alter the ',...
                            'Background Detection parameters.'];
                    waitfor(errordlg(eStr,tStr,'modal'))
                    return                    
                else
                    % otherwise, set the phase detection value 
                    obj.iMov.vPhase = 1;
                end

                % disables the stack selection type and the current image frame
                obj.ImgFrm = imgfrm;
                
            else
                % retrieves the current frame from file                                
                [imov,simgs,Img] = ...
                            getEstimateImageStack(idata,imov);
            ...
            [obj.fPosAll,obj.frameSet] = deal([],true);
            ...
            set(hgui.radioBackEst,'enable','off')
            set(hgui.radioRes,'enable','off')
            ...
            if ~all(obj.iMov.vPhase == 4)
                ...
                set(hgui.buttonUpdateEst,'enable','on')    
            end

            % sets the enables properties of the phase selection objects
            setPanelProps(hgui.panelPhaseSelect,'on');     
            if length(simgs) > 1
                % sets the 1st frame for each phase as the background image
                imgfrm = cellfun(@(x)(x{roundP(length(x)/2)}),Img,'un',0);
                obj.ImgFrm = imgfrm; 
            else
                obj.ImgFrm = Img{1}(1);
            end
            ...
            set(hgui.checkFlyMarkers,'value',0,'enable','off');
            obj.checkFlyMarkers(hgui.checkFlyMarkers, [])
        ...
        function checkFlyMarkers(obj, hObject, eventdata)
            ...
            fok = obj.iMov.flyok;
            isAll = logical(get(obj.hGUI.radioAllObject,'value'));

            % sets the panel properties
            setPanelProps(obj.hGUI.panelTrackingMarkers,vStr{1+isOK})
            obj.setManualDetectEnable()
            
            % sets the marker visibility for all apparatus
            for i = 1:length(obj.hMark)
                for j = 1:length(obj.hMark{i})
                    isOn = isOK && fok(j,i);
                    if isOn
                        setObjVisibility(obj.hMark{i}{j},~isAll);                    
                        if ~isempty(obj.hMarkAll)
                            setObjVisibility(obj.hMarkAll,isAll);
                        end                        
                    else
                        setObjVisibility(obj.hMark{i}{j},false);                    
                        if ~isempty(obj.hMarkAll)
                            setObjVisibility(obj.hMarkAll,false);
                        end
                    end
                end
            end
        ...
        % --- Executes when selected object is changed in panelTrackingMarkers.
        function panelTrackingMarkers(obj, hObject, eventdata)

            % initialisations
            hRadio = get(hObject,'SelectedObject');
            isAll = strcmp(get(hRadio,'tag'),'radioAllObject'); 
            
            % sets the visibility of the likely markers
            cellfun(@(y)(cellfun(@(x)(setObjVisibility...
                                    (x,~isAll)),y)),obj.hMark);
               
            % sets the visibility of potential markers (if they exist)
            if ~isempty(obj.hMarkAll)
                setObjVisibility(obj.hMarkAll,isAll)
            end
           
            % updates the menual detection button enabled properties                    
            obj.setManualDetectEnable()
        end
        ...
        function buttonUpdateEst(obj, hObject, eventdata)
            ...
            if initDetectCompleted(obj.iMov)
                ...
                uChoice = questdlg({['This action will overwrite the ',...
                            'current background images.'];['Do you want ',...
                            'to continue?']},tStr,'Yes','No','Yes');
                if ~strcmp(uChoice,'Yes')
            ...
            if ok     
                ...
                obj.fPosAll = cellfun(@(x)(x.pMaxG),obj.trkObj.fObj,'un',0);                
                ...
                set(hgui.checkFlyMarkers,'value',1,'enable','on') 
                set(hgui.radioLikelyObject,'value',1,'enable','on')                 
        ...
        function tableFlyUpdate(obj, hObject, eventdata)
            ...
            if (obj.uList(iVal,1) ~= obj.iPara.cFrm) || ...
               (obj.uList(iVal,4) ~= obj.iPara.cPhase)
                % updates the phase/frame count
                set(hgui.editFrameCount,'string',obj.uList(iVal,1))
                set(hgui.editPhaseCount,'string',obj.uList(iVal,4))
        ...
        function buttonAddManual(obj, hObject, eventdata)
            ...
            % runs the manual fly tracking sub-program
%             waitfor(msgbox('FINISH ME! (Manual Tracking)'))
%             manualFlyTracking(getappdata(handles.figBackEst,'hgui'),handles.figBackEst)
        ...
        function setManualDetectEnable(obj)
            ...
            % object handles
            hRadio = obj.hGUI.radioAllObject;
            
            % determines the enabled state of the button
            vP = obj.iMov.vPhase(obj.iPara.cPhase);
            validPhase = ~any(vP==[3,4]);                        
            useManual = validPhase && ...
                get(hRadio,'value') && strcmp(get(hRadio,'Enable'),'on');
            ...
            if ~isempty(obj.uList)
                ...
                % markers for this frame are shown (otherwise disabled)
                if useManual
                    isShow = (uData(:,1) == obj.iPara.cFrm) & ...
                             (uData(:,4) == obj.iPara.cPhase);
                    setObjVisibility(obj.hManual(isShow),'on')
                    setObjVisibility(obj.hManual(~isShow),'off')   
                else
                    setObjVisibility(obj.hManual,'off')  
                end
            end
            
            % updates the manual detection button
            setObjEnable(obj.hGUI.buttonAddManual,useManual)
            setObjEnable(obj.hGUI.buttonUpdateManual,...
                                        useManual && ~isempty(obj.uList))
    ...
    methods (Static)
        ...
        % --- 
        function [iPosAll,fPosAllT,pColAllT] = ...
                                        getAllMarkerColours(fPosAll,fPos)
            
            % initialisations
            fCol0 = {[1 0 0];[0 0 1]};
            nApp = num2cell(1:length(fPosAll))';
            
            % sets the global region/sub-region mapping indices
            iPosAll = cell2mat(cell2cell(cellfun(@(x,y)(cellfun(@(z,zz)...
                ([repmat([y,zz],size(z,1),1),(1:size(z,1))']),x(:),...
                num2cell(1:length(x))','un',0)),fPosAll(:),nApp,'un',0)));
            
            % memory allocation
            fPosAll = cell2cell(fPosAll);
            fPos = num2cell(cell2cell(fPos),2);
            pColAll = cell(length(fPosAll),1);
            
            % sets the 
            isUniq = cellfun(@(x)(size(x,1)),fPosAll) == 1;
            pColAll(isUniq) = {[0 1 0]};
            pColAll(~isUniq) = cellfun(@(x,y)(cell2cell(fCol0(1+...
                cellfun(@(z)(double(isequal(x,z))),num2cell(y,2))))),...
                fPos(~isUniq),fPosAll(~isUniq),'un',0);
                
            % sets the combined arrays
            pColAllT = cell2cell(pColAll);
            fPosAllT = cell2cell(fPosAll);
