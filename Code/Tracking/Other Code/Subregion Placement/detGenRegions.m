...
for j = 1:szF(1)
    yPF(j,:) = calcPolyFitValues(xP0(j,:),yP0(j,:),IxcP(j,:));
end

% fits the sub-region centroids in the vertical direction
for j = 1:size(xP0,2)
    xPF(:,j) = calcPolyFitValues(yP0(:,j),xP0(:,j),IxcP(:,j)); 
end
...
while 1
    ...
    if D(iMx) > dLim
        ...
        yPF(yMx,:) = calcPolyFitValues(...
                        xP0(yMx,:),yP0(yMx,:),IxcP(yMx,:),ok(yMx,:));
        xPF(:,xMx) = calcPolyFitValues(...
                        yP0(:,xMx),xP0(:,xMx),IxcP(:,xMx),ok(:,xMx)); 
...
function yF = calcPolyFitValues(x0,y0,W,ok)

%
rsTol = 0.8;
if ~exist('ok','var'); ok = true(length(x0),1); end
...
[~,tPk,~,P] = findpeaks(Z/max(Z),'MinPeakHeight',pkTol);
