...
if ~isempty(iExpt)
    ...
    if (fIndex == 1)          
        ...
        % loads the summary file and retrieves the 
        [fDir,~,~] = fileparts(fName);
        snFile = fullfile(fDir,'Summary.mat');     
        
        % sets the time of the previous 
    ...
    Tofs = roundP(Tofs);
    [Tvid,Tnow] = deal(datevec(addtodate(datenum(TT.T0),Tofs,'second')),clock);

    % while the wait time is greater than zero, keep updating the waitbar
    Twait = ceil(max(0,calcTimeDifference(Tvid,Tnow)));
    while (Twait > 0)
        ...
        % sets the 
        wStrNw = sprintf(['Waiting For Next Video (Approx Time Remain ',...
                          '= %s:%s:%s)'],C{2},C{3},C{4});
        pW = max(0,(Ttot-Twait)/Ttot);
