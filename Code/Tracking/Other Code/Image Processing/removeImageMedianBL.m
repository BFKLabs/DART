function [ImgBL,ImgBG] = removeImageMedianBL(Img,isLoVar,is2D,h0)
...
ImgBL = removeImagePhaseBL(Img,ImgBG,hG);
if returnArr; ImgBL = ImgBL{1}; end

% removes the median baseline from the image
function ImgRmv = removeImagePhaseBL(Img,ImgBG,hG)         

if length(ImgBG) == length(Img)
    ImgRmv = cellfun(@(x,Ibg)(...
            removeImageBL(x,Ibg,hG)),Img,ImgBG,'un',0);
else
    ImgRmv = cellfun(@(x)(...
            removeImageBL(x,ImgBG{1},hG)),Img,'un',0);    
end   
