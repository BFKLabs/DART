    ...
    methods
        ...
        function calcFullObjPos(obj)
            ...
            for i = 1:obj.nApp
                ...
                IRLmx = cellfun(@(x)(max(x(:))),IRLnwT);
                IRLmxN = IRLmx/max(IRLmx(:));
                            
                % matches the residual maxima to the thresholded binaries
                Bmx = cellfun(@(x)(imregionalmax(x)),IRLnwT,'un',0);
                BRLnwT = cellfun(@(x,y)...
                                (y.*(x>pW*max(x(:)))),IRLnwT,Bmx,'un',0);
                iGrpT = cellfun(@(x)(...
                                cell2mat(getGroupIndex(x))),BRLnwT,'un',0);
                
                % calculates the coordinates for each sub-region/frame
                for j = 1:obj.nTube(i)
                    if obj.iMov.flyok(j,i)
                        if all(IRLmxN(j,:) < pTolMax) ...
                                                    && ~isempty(obj.prData)
                            ...
                            % determines the number of thresholded groups
                            nGrpT = cellfun(@length,iGrpT(j,:));
                            for k = find(nGrpT>1)
                                % if more than one potential object, then
                                % use the point with the higher residual
                                imx = argMax(IRLnwT{j,k}(iGrpT{j,k}));
                                iGrpT{j,k} = iGrpT{j,k}(imx);
                            end

                            % calculates the final coordinates of the object
                            % (within the current sub-region) for all frames
                            szT = size(IRLnwT{j,1});
                            [yP,xP] = ind2sub(szT,cell2mat(iGrpT(j,:)'));
                            fPosNw = [xP,yP];
                        end
...
%                         Ixc = normxcorr2(Isub1.*obj.Iopt,Isub2.*obj.Iopt);
            ...
            nTubeR0 = obj.iMov.nTubeR(iRow,iCol);
            
            % sets the object counts (feasible frames only)
            fok0 = obj.iMov.flyok(1:nTubeR0,iApp);
            fok = find(fok0);
            nTubeR = length(fok);
            Ngrp = cellfun(@(x)(size(x,1)),p_Max{1}(fok));             
            
            % calculates the object shape (if not already calculated)
            if isnan(obj.szObj)
                % retrieves the likely position of the objects
                pMaxTmp = cellfun(@(x)(x(:)),obj.pMax(iApp,:),'un',0);
                pMaxApp = cell2cell(pMaxTmp,0);                
                    ...
                    for i1 = 1:obj.nImg                        
                        ...
                        for k = find(isAmbig(:))'
                            ...
                            elseif ~all(isnan(NsT(indG{k},:)))
                                % sorts the metrics in ascending order
                                NsTG = NsT(indG{k},:);
                ...
                for i = 1:nImgL
                    ...
                    if mean(ii) > obj.pTol
                        ...
                        if isempty(iGrp{iImg})
                            % if there are no stored indices, then flag 
                            % adding data
                            iAdd = [];
                        else
            ...
            for iImg = 1:obj.nImg
                ...
                % tube region offset
                fPosF = NaN(nTubeR0,2);     
                fokF = fok(hasObj{iImg});
                
                %
                yOfs = num2cell(obj.y0{iApp}(fokF)); 
                pMaxF = cell2mat(cellfun(@(x,y0)(x+repmat([0,y0],...
                     size(x,1),1)),p_Max{iImg}(fokF),yOfs,'un',0));
            
                % sets the coordinates for each potential grouping
                fPosF(fokF,:) = pMaxF(iGrp{iImg}{iS(1)},:);
            ...
            %
            isStat = (obj.iStatus(1:nTubeR0,iApp) == 0) & fok0;                       
            for iT = find(isStat(:)')
                fPosT = sortrows(cell2mat(cellfun(@(x)...
                                    (x(iT,:)),obj.fPos(iApp,:)','un',0)));
                if any(range(fPosT,1) > dX)
                    % if there is a major shift in position, then
                    % recalculate the 
                    B = find(any(abs(diff([-dX*[1,1];fPosT],[],1)) > 5,2));
                    indT = [B,[B(2:end)-1;obj.nImg]];
                    iMx = argMax(diff(indT,[],2));
                    
                    %
                    pMn = nanmean(fPosT(indT(iMx,1):indT(iMx,2),:),1);
                    for j = 1:obj.nImg
                        iMn = argMin(pdist2(obj.pMax{iApp,j}{iT},pMn));
                        if ~isnan(iMn)
                            obj.fPos{iApp,j}(iT,:) = ...
                                        [0,obj.y0{iApp}(iT)] + ...
                                         obj.pMax{iApp,j}{iT}(iMn,:);
                        else
                            obj.fPos{iApp,j}(iT,:) = roundP(pMn);
                        end
                    end
                end
            end
            ...
            for iT = iTubeS(:)'
                ...
                for iImg = 1:obj.nImg
                    fPosT = cell2mat(cellfun(@(x)...
                                (x(iT,:)),obj.fPos(iApp,:)','un',0));
                    dPosT = sqrt(sum(diff(fPosT,[],1).^2,2));
                            
                    if max(dPosT) > sqrt(sum(obj.szObj.^2))/2
                        obj.iStatus(iT,iApp) = 3;
                    end
                end
            end            
            ...
            if obj.vPh == 1            
                ...
                for iApp = 1:obj.nApp                
                    ...
                    for iT = find(isSet)
                        ...
                        fP = cellfun(@(x)...
                                (x(iT,:)-pOfs),obj.fPos(iApp,:),'un',0);
                        
                        %
                        IBGtmp = cellfun(@(I,fP)...
                             (obj.setBGImage(I,roundP(fP))),IL0,fP,'un',0); 
        ...
        function estimateRegionBG(obj,iApp)
            ...
            for iImg = 1:obj.nImg
                [IappBG{iImg},BMaxT{iImg},isOK{iImg}] = ...
                            obj.setBGImage(Iapp{iImg},roundP(fPosT{iImg}));
            end
    ...
    methods (Static)
        ...
        % --- 
        function Isub = getPointSubImage(I,p)
