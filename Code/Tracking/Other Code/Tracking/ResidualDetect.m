classdef ResidualDetect < handle
    ...
    methods
        ...
        function segmentRegions(obj,iApp)
            ...
            nTubeR = obj.iMov.nTubeR(iRow,iCol);
            fok = num2cell(obj.iMov.flyok(1:nTubeR,iApp));
            ...
            ImgBG = imov.Ibg{obj.iPhase}{iApp}.*Bw;            
            ...
            %
            IRmx = cellfun(@(x)(max(x(:))),IResL); 
            IRmx(~obj.iMov.flyok(:,iApp),:) = NaN;
            pIRmx = IRmx/nanmedian(IRmx(:));                        
            
            % determines if there are any residuals that appear to have a
            % low residual value (relative to the median intensity)
            isLoRes = pIRmx<pW;
            isMiss = mean(isLoRes,2);            
            if any(isMiss>0) && ~obj.is2D
                % if so, then determine if their locations are 
                ImgT = obj.Img;
                hasMD = false(length(ImgT),1);                
            
                for i = find(isMiss(:)' > 0)
                    if isMiss(i) > pWTol 
                        % if there are a significant number of frames with
                        % low residuals, then the object is probably hiding                        
                        fP = obj.prData.fPos{iApp}(i,:)-[0,obj.y0{iApp}(i)];
                        fPos0{i} = repmat({fP},1,length(fPos0{i}));
                        
                    else
                        % case is the residuals are only low for a few
                        % number of frames
                        for j = find(isLoRes(i,:))
                            if j == 1
                                % if the this is the first frame, then use
                                % the position from the previous frame
                                pOfsL = [0,iRT{i}(1)-1];
                                fPos0{i}{j} = ...
                                    obj.prData.fPos{iApp}(i,:) - pOfsL;
                                
                            else            
                                % otherwise, calculate the median baseline
                                % remove image
                                if ~hasMD(j)
                                    hasMD(j) = true;
                                    ImgT{j} = removeImageMedianBL(...
                                                   ImgT{j},0,obj.is2D,h0);
                                end

                                % calculates the total residual
                                Ir = imfilter(ImgT{j}(iR(iRT{i}),iC)...
                                        -obj.iMov.IbgT{iApp}(iRT{i},:),hG);
                                    
                                % determines the position of the hidden
                                % object (relative to the previous frame)
                                [X,Y] = calcHiddenObjPos(Ir,fPos0{i}{j-1});
                                fPos0{i}{j} = [X,Y];
                            end
                        end
                    end
