    ...
    methods 
        ...
        function calcInitEstimate(obj,iMov,hProg)
            ...
            switch rFlag
                case 0
                    ...
                    for i = 1:nPhase
                        ...
                        obj.fObj{i}.setClassField('szObj',szObj)
                    end
            ...
            for i = 1:nPhase
                ...
                obj.fObj{i}.runDetectionAlgo(isempty(obj.prData0));
            ...
            szObj = cell2mat(cellfun(@(x)(x.szObj),obj.fObj,'un',0));
...
%                             %
%                             y0 = obj.iMov.iRT{i}{j}(1)-1;
%                             fPosNw(:,2) = fPosNw(:,2) + y0;
            ...
            for i = 1:nImg
                ...
                Ixc(iT,iApp) = nanmean(cellfun(@(x)(max(max(...
                          normxcorr2(IsubNw{ii(i)},x)))),IsubNw(ii(jj))));
                      
                %
                if Ixc(iT,iApp) < xcTol
                    IbgTmp{iT,iApp} = [];
                    sFlag(iT,iApp) = 0;
                    
                    for iPhase = 1:nPhase
                        for iFrm = 1:size(fPos{iPhase},2)
                            fPos{iPhase}{iApp,iFrm}(iT,:) = NaN;
                        end
                    end
                end
            end            
        ...
        function [fPosNw,sFlag,IbgT,IsubNw] = ...
            ...
            for i = 1:nFrm
                ...
                dImgL0(:,i) = cellfun(@(x)(ImgL{i}-x),ImgL(:),'un',0);
            ...
            if isempty(obj.prData0)                      
                ...
                [fPosNw,IsubF,isOK] = obj.calcMovingObjCoords(dImgMx,ImgL);
        ...
        function [fPos,IsubF,ok] = calcMovingObjCoords(obj,dImgQ,ImgL)
            ...
            if ~obj.is2D        
                for i = 1:length(fPos0)                                
                    ...
                    if nGrp(i) == 0
                        % if there are no valid groups within the 
                        % sub-region, then exit the function
                        [fPos,ok] = deal([],false);
                        return                    
                    end
                end
...
%             jGrp = jGrp(isOK);            
    ...
    methods (Static)
        ...
        function ImgL = setTempBGImage(ImgL,fPos)
            ...
            nDil = 2;
            pW = 0.5;
            pTol = ImgL(sub2ind(size(ImgL),fPos(2),fPos(1)))*pW;
