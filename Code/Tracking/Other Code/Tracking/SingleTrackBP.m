classdef SingleTrackBP < handle
    ...
    methods
        ...
        function ok = batchProcessDir(obj,iDir)
            ...
            [obj.iMov0,obj.iData0,obj.pData0] = deal(obj.iMov,obj.iData,[]);            
            ...
            if isnan(obj.iFile0)
                ...
                [~,obj.fNameBase0,~] = fileparts(obj.bData.mName{j0});                 
            end                        
            ...
            while i <= obj.nFile
                ...
                if ~obj.isSeg(i) || (obj.nFile == 1)                     
                    ...
                    if obj.bData(iDir).movOK(i) == 1
                        ...
                        if obj.bData(iDir).movOK(i) == 1                                                                                    
                            switch obj.calcInitObjLocations(iDir,i)
                                case 1
                        ...
                        % outputs the final data to file (if required)
                        if obj.isOutput
            ...
            fldStr = {'iMov','iData','iMov0','iData0','pData0','p0Pr'};
        ...
        function ok = segObjLocations(obj)            
            ...
            trkObjF.segEntireVideo(obj.hGUI,obj.iMov,obj.pData);
        ...
        function sFlag = calcInitObjLocations(obj,iDir,iFile)
        ...
        function openNewSolnFile(obj,iDir,iFile)
            ...
            if ~exist(obj.sFileNw,'file')
                ...
                if iFile == 1
                    ...
                    if (isempty(BT.pData))
                        ...
                        obj.isCalcBG = ~any(cellfun...
                                            (@(x)(any(x)),BT.pData.frmOK));
                    end
                end
            end             
            ...
            if ~isempty(hSolnT)
                try
                    ...
                    set(obj.hGUI.checkShowMark,'value',1)
                    setappdata(hSolnT,'pData',[])
                    iFunc = getappdata(hSolnT,'initFunc');
                    iFunc(guidata(hSolnT),1)
        ...
        function ok = getNextRecordedVideo(obj,iDir,iFile)
            ...
            hh0 = getHandleSnapshot(findall(obj.hProg));
            ...
            if ~exist(obj.outDir,'dir')
                ...
                if isempty(sName)
                    ...
                    while cont
                        ...
                        if isfield(B,'pData'); obj.pData0 = B.pData; end
        ...
        function ok = videoRecordWait(obj,bdata)
            ...
            A = load(bdata.sName,'-mat','iExpt');
            if ~waitForRecordedFile(...
                            bdata.mName{obj.ind0},A.iExpt,obj.hProg,wOfs)
                % if the user cancelled, or there was a timeout, then exit
                ok = false;
                return 
        ...
        function updateBPFile(obj,bData)
            ...
            eval(sprintf('save(obj.bFile,''bData'')'))
...
%                 end    
        ...
        function initPosDataStruct(obj, iDir)
            ...
            summFile = load(obj.bData(iDir).sName);

            % creates an empty position solution struct 
            % and outputs it to file               
            obj.pData = setupPosDataStruct(...
                        obj.iMov,summFile.tStampV{iVid});  
