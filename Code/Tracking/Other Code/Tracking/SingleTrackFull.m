    ...
    methods 
        ...
        function segEntireVideo(obj,hGUI,iMov,pData)
            ...
            if obj.calcOK
                ...
                if obj.pData.calcPhi && obj.calcOK
        ...
        function checkFinalSegSolnF(obj)
            ...
            [obj.pData,obj.iMov,obj.calcOK] = checkFinalSegSoln(...
                    obj.hGUI,obj.iData,obj.pData,obj.iMov,obj.hProg);
        ...
        function segVideoPhase(obj,iPhase)
            ...
            elseif ~isempty(fPosPr)
                ...
                prData = obj.getPrevPhaseData(obj.fObj{iPhase-1}); 
...
%                 iFrmLast = obj.sProg.iFrmR{max(1,nCountNw)}(1)-1;
                ...
                prData = obj.setupPrevPhaseData(iFrmLast);                                 
            end
            ...
            for i = (obj.pData.nCount(iPhase)+1):nStack            
                ...
                obj.fObj{iPhase}.runDetectionAlgo(prData);   
                ...
                prData = obj.updateTrackingSoln(iPhase,i,nStack);                                
        ...
        function prData = updateTrackingSoln(obj,iPhase,iStack,nStack)
            ...
            for iApp = 1:obj.nApp    
                ...
                for iT = 1:obj.nTube(iApp)
                    ...
                    % sets the orientation angle (if calc0ulated)
                    if obj.iMov.calcPhi
        ...
        function resetData = resetPosDataStruct(obj)
            ...
            % prompts the user to enter the restart stack
            inStr = {'Enter video phase to restart from:';...
                     'Enter image stack you wish to restart from:'};
            A = num2cell([obj.iPhase0,obj.pData.nCount(obj.iPhase0)+1]);     
            defVal = cellfun(@num2str,A,'un',0);                 
            
            % keep looping prompting until a feasible answer is input
            while 1
                % prompts the user for the start video/stack
                ...
                nwVal = cellfun(@str2double,nwVal);
                if chkEditValue(nwVal(1),[1 obj.iPhase0],1) && ...
                   chkEditValue(nwVal(2),[1 obj.pData.nCount(nwVal(1))],1)
                    % case is the user chose to restart the video 
                    if isequal(nwVal,[1;1])
            ...
            obj.iPhase0 = nwVal(1);
            obj.iStack0 = nwVal(2);
            obj.pData.isSeg((nwVal(1)+1):end) = false;
            obj.pData.nCount((nwVal(1)+1):end) = 0;
            obj.pData.nCount(obj.iPhase0) = nwVal(2)-1;                

            % loads the progress data file
            pFile = fullfile(obj.tDir,'Progress.mat');
            A = load(pFile); obj.sProg = A.sProg;                                
            i0 = (nwVal(2)-1)*obj.sProg.nFrmS+obj.iMov.iPhase(nwVal(1),1);
        ...
        function initPosDataStruct(obj)
            ...
            if resetData
                ...
                if exist(sumFile,'file')
                    ...
                    % if the summary file exists, then set the frame time-stamps
                    vidSum = load(sumFile);
                    obj.pData.T = vidSum.tStampV{iVid};
                else
                    ...
                    try
                        ...
                        % old version of function (obsolete on newer Matlab versions)
